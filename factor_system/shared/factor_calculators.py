...
class SharedFactorCalculators:
    ...
    def calculate_stoch(
        ...
        if not self.has_talib:
            ...
            return self._stoch_python(high, low, close, fastk_period, slowk_period, slowd_period)
    ...
    def calculate_candlestick_pattern(
        ...
        if self.has_talib:
            try:
                ...
                result = talib_function(open_price.values, high.values, low.values, close.values)
    ...
    def _calculate_candlestick_pattern_fallback(
        ...
        if pattern_name == 'CDL2CROWS':
            ...
            result = np.where(condition1 & condition2 & condition3 & condition4 & condition5, -100, 0)
            ...
            result = np.where(condition1 & condition2 & condition3 & condition4, -100, 0)
    ...
    def _stoch_python(
        self, high, low, close, fastk_period, slowk_period, slowd_period
    ):
        """STOCH纯Python实现"""
        ...
        fastk = 100 * (close - lowest_low) / (highest_high - lowest_low).replace(0, 1e-10)
    ...
    def _willr_python(self, high, low, close, period):
        ...
        willr = -100 * (highest_high - close) / (highest_high - lowest_low).replace(0, 1e-10)
        return willr
...
def calculate_macd(
    ...
    """MACD计算便捷函数"""
    return SHARED_CALCULATORS.calculate_macd(close, fastperiod, slowperiod, signalperiod)
