...
class VBTIndicatorAdapterOptimized:
    ...
    def __init__(
        ...
        if enable_smart_selection:
            self.smart_adapter = SmartIndicatorAdapter(
                price_field=price_field, min_ic_threshold=ic_threshold
            )
    ...
    def _build_core_parameters(self) -> Dict[str, List[Dict]]:
        ...
        return {
            ...
            # 波动率类 - 去除数学重复
            "ATR": [{"window": 14, "rationale": "经典ATR，2周周期"}],
            "BBANDS": [
    ...
    def compute_all_indicators(
        self, df: pd.DataFrame, target: Optional[pd.Series] = None
    ) -> pd.DataFrame:
        """
        计算优化后的指标 - 智能参数选择
