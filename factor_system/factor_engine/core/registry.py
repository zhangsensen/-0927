...
class FactorRegistry:
    ...
    def _extract_enhanced_metadata(self, factor_class: Type[BaseFactor], factor_id: str) -> Dict:
        """
    ...
    def _validate_factor_id(self, factor_id: str) -> None:
        ...
        logger.debug(f"因子ID {factor_id} 格式验证通过，支持的格式: {supported_patterns}")
    ...
    def _parse_parameterized_factor_id(self, factor_id: str) -> Tuple[str, Dict]:
        ...
        if factor_id.startswith('TA_AROON_') and factor_id.endswith('_up'):
            ...
            if match:
                return 'TA_AROON', {'timeperiod': int(match.group(1)), 'direction': 'up'}
        ...
        if factor_id.startswith('TA_AROON_') and factor_id.endswith('_down'):
            ...
            if match:
                return 'TA_AROON', {'timeperiod': int(match.group(1)), 'direction': 'down'}

        # 特殊处理长格式STOCHRSI
        if factor_id.startswith('TA_STOCHRSI_'):
            # TA_STOCHRSI_fastd_period3_fastk_period5_timeperiod14_K
            match = re.match(r'^TA_STOCHRSI_fastd_period(\d+)_fastk_period(\d+)_timeperiod(\d+)_(K|D)$', factor_id)
            if match:
                return 'TA_STOCHRSI', {
        ...
        if factor_id.startswith('TA_ULTOSC_'):
            ...
            match = re.match(r'^TA_ULTOSC_timeperiod(\d+)_timeperiod(\d+)_timeperiod(\d+)$', factor_id)
            if match:
                return 'TA_ULTOSC', {
        ...
        if factor_id.startswith('TA_APO_'):
            ...
            match = re.match(r'^TA_APO_fastperiod(\d+)_matype(\d+)_slowperiod(\d+)$', factor_id)
            if match:
                return 'TA_APO', {
        ...
        # 特殊处理布林带变体
        if factor_id.startswith('BB_') and any(suffix in factor_id for suffix in ['_Upper', '_Middle', '_Lower', '_Width']):
            # BB_20_2_0_Upper -> BB with parameters
            match = re.match(r'^BB_(\d+)_(\d+)_(\d+)_(Upper|Middle|Lower|Width)$', factor_id)
            if match:
                return 'BB', {
        ...
        mapping_rules = {
            ...
            # MACD变体 - 使用下划线命名
            r'^MACD_(\d+)_(\d+)_(\d+)$': ('MACD', {'fast_period': int, 'slow_period': int, 'signal_period': int}),

            # STOCH变体
            r'^STOCH_(\d+)_(\d+)$': ('STOCH', {'fastk_period': int, 'slowk_period': int}),
