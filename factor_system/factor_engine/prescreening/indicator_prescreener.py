...
class IndicatorPrescreener:
    ...
    def prescreen_indicators(self, 
                           indicator_df: pd.DataFrame,
                           target_series: pd.Series,
                           reference_date: Optional[pd.Timestamp] = None) -> Dict[str, any]:
        """
        预筛选指标 - 多维度质量评估
        ...
        quality_report = self._generate_quality_report(
            indicator_df, basic_qualified, predictive_qualified, 
            stability_qualified, diversity_qualified, target_series
        )
        
        return {
            "qualified_indicators": diversity_qualified,
            "rejected_indicators": list(set(indicator_df.columns) - set(diversity_qualified)),
            "quality_metrics": self._calculate_quality_metrics(indicator_df[diversity_qualified], target_series),
            "quality_report": quality_report,
            "reduction_ratio": (len(indicator_df.columns) - len(diversity_qualified)) / len(indicator_df.columns)
        }
    
    def _basic_quality_screen(self, indicator_df: pd.DataFrame, target_series: pd.Series) -> List[str]:
        """基础质量筛选"""
        qualified = []
        ...
        for column in indicator_df.columns:
            ...
            if missing_ratio > self.max_missing_ratio:
                logger.debug(f"{column} 被拒绝: 缺失率 {missing_ratio:.2%} > {self.max_missing_ratio:.2%}")
                continue
            ...
            if series.dtype in ['float64', 'int64']:
                ...
                if non_zero_ratio < self.min_coverage:
                    logger.debug(f"{column} 被拒绝: 覆盖率 {non_zero_ratio:.2%} < {self.min_coverage:.2%}")
                    continue
            ...
            if len(valid_samples) < self.min_samples:
                logger.debug(f"{column} 被拒绝: 样本数 {len(valid_samples)} < {self.min_samples}")
                continue
    ...
    def _predictive_power_screen(self, indicator_df: pd.DataFrame, target_series: pd.Series) -> List[str]:
        """预测力筛选 - 基于IC和统计显著性"""
        qualified = []
        ...
        for column in indicator_df.columns:
            ...
            if not np.isnan(ic) and abs(ic) >= self.min_ic_threshold:
                # t检验
                t_stat, p_value = stats.pearsonr(aligned_data['indicator'], aligned_data['target'])
                
                if p_value < 0.05:  # 5%显著性水平
                    ic_ir = abs(ic) / max(1e-6, np.std(aligned_data['indicator']))  # 简化IC_IR
                    
                    if ic_ir >= self.min_ic_ir_threshold:
                        qualified.append(column)
                        logger.debug(f"{column} 通过预测力筛选: IC={ic:.4f}, IC_IR={ic_ir:.4f}, p={p_value:.4f}")
                    else:
                        logger.debug(f"{column} IC_IR不足: {ic_ir:.4f} < {self.min_ic_ir_threshold}")
                else:
    ...
    def _stability_screen(self, indicator_df: pd.DataFrame, reference_date: pd.Timestamp) -> List[str]:
        """稳定性筛选 - 时间序列稳定性分析"""
        qualified = []
    ...
    def _diversity_screen(self, indicator_df: pd.DataFrame) -> List[str]:
        ...
        for col in indicator_df.columns:
            # 这里简化处理，实际应该传入IC得分
            ic_scores[col] = abs(indicator_df[col].corr(indicator_df.iloc[:, -1])) if len(indicator_df.columns) > 1 else 0.1
        ...
        for indicator, ic_score in sorted_indicators:
            ...
            for selected_indicator in selected:
                if selected_indicator in corr_matrix.columns and indicator in corr_matrix.columns:
                    correlation = abs(corr_matrix.loc[indicator, selected_indicator])
                    if correlation > self.correlation_threshold:
                        is_redundant = True
                        logger.debug(f"{indicator} 与 {selected_indicator} 高相关({correlation:.3f})，被去除")
                        break
    ...
    def _calculate_quality_metrics(self, indicator_df: pd.DataFrame, target_series: pd.Series) -> Dict[str, IndicatorQualityMetrics]:
        """计算指标质量评估"""
        metrics = {}
        ...
        for column in indicator_df.columns:
            ...
            if window_size >= 10:
                for start in range(0, len(aligned_data) - window_size + 1, window_size//2):
                    end = start + window_size
                    if end <= len(aligned_data):
                        window_ic = aligned_data.iloc[start:end]['indicator'].corr(aligned_data.iloc[start:end]['target'])
            ...
            positive_ratio = np.mean([ic_val > 0 for ic_val in ic_values]) if ic_values else (ic > 0)
    ...
    def _generate_quality_report(self, indicator_df: pd.DataFrame,
        ...
        report = {
            "summary": {
                ...
                "final_selection_rate": len(diversity_qualified) / total_indicators if total_indicators > 0 else 0
            },
            "rejection_reasons": {
                "basic_rejected": list(set(indicator_df.columns) - set(basic_qualified)),
                "predictive_rejected": list(set(basic_qualified) - set(predictive_qualified)),
                "stability_rejected": list(set(predictive_qualified) - set(stability_qualified)),
                "diversity_rejected": list(set(stability_qualified) - set(diversity_qualified))
            },
            "quality_distribution": self._analyze_quality_distribution(indicator_df[diversity_qualified], target_series)
        }
        
        return report
    
    def _analyze_quality_distribution(self, qualified_df: pd.DataFrame, target_series: pd.Series) -> Dict[str, any]:
        """分析质量分布"""
        ...
        avg_ic = np.mean([m.ic_mean for m in avg_metrics.values()]) if avg_metrics else 0
        avg_ic_ir = np.mean([m.ic_ir for m in avg_metrics.values()]) if avg_metrics else 0
        
        return {
            "indicator_type_distribution": {k: len(v) for k, v in indicator_types.items()},
            "average_ic": avg_ic,
