...
class StrictTemporalProvider(DataProvider):
    ...
    def __init__(
        ...
        money_flow_columns: Optional[List[str]] = None,
    ):
        """
        初始化严格时序安全包装器
        ...
        self.money_flow_columns = (
            money_flow_columns or self._default_money_flow_columns()
        )

        logger.info(
            f"初始化StrictTemporalProvider，监控{len(self.money_flow_columns)}个资金流字段"
        )
    ...
    def _default_money_flow_columns(self) -> List[str]:
        ...
        return [
            ...
            "main_net_winsorized",
            "main_net_zscore",
            "retail_net_winsorized",
            "retail_net_zscore",
            "total_net_winsorized",
            "total_net_zscore",
        ]
    ...
    def load_price_data(
        ...
        """
        # 调用被包装的提供者
        data = self.wrapped_provider.load_price_data(
    ...
    def _apply_temporal_safety(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        # 检测数据中存在的资金流字段
        money_flow_cols_present = [
            col for col in self.money_flow_columns if col in data.columns
        ]
    ...
    def load_fundamental_data(
        ...
        """加载基本面数据（直接转发）"""
        return self.wrapped_provider.load_fundamental_data(
