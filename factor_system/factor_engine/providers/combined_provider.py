...
class CombinedMoneyFlowProvider(DataProvider):
    ...
    def _align_and_merge(
        ...
        # 检测资金流列（排除元数据列）
        mf_cols = [c for c in money_flow_data.columns if c not in ['value_unit', 'temporal_safe']]
            ...
            if "value_unit" in combined.columns:
                ...
                if len(unit_values) > 0:
                    if "yuan" not in unit_values:
                        logger.error(f"⚠️ 单位异常：value_unit={unit_values}，预期'yuan'")
                        raise ValueError("资金流数据单位不是'yuan'，可能存在二次缩放风险")
                    else:
