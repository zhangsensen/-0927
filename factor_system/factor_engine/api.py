...
def _register_core_factors(registry: FactorRegistry):
    ...
    try:
        ...
        # 导入自动生成的所有因子
        from factor_system.factor_engine.factors import GENERATED_FACTORS, FACTOR_CLASS_MAP
        ...
        for factor_class in GENERATED_FACTORS:
            try:
                ...
                logger.debug(f"✅ 注册因子: {factor_class.factor_id}")
                registered_count += 1
        ...
        logger.info(f"🎯 FactorEngine现在拥有 {len(registry.factors)} 个因子，与factor_generation完全一致")
