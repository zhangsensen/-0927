...
def batch_screen_market_parallel(market: str):
    ...
    print(
        f"📈 总任务数: {len(stocks)} × {len(ALL_TIMEFRAMES)} = {len(stocks) * len(ALL_TIMEFRAMES)}\n"
    )
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # 提交所有任务
        future_to_symbol = {
            executor.submit(screen_single_stock_worker, task): task[0] for task in tasks
        }
    ...
    if results:
        print(f"\n📈 成功股票详细统计:")
        full_success = sum(
            1 for _, (sc, _) in results.items() if sc == len(ALL_TIMEFRAMES)
        )
        partial_success = success_count - full_success
        print(
            f"  - 全部时间框架成功: {full_success} ({full_success/len(stocks)*100:.1f}%)"
        )
        print(
            f"  - 部分时间框架成功: {partial_success} ({partial_success/len(stocks)*100:.1f}%)"
        )
...
def main():
    ...
    print(
        f"  - 预计耗时: {total_stocks * 2 / MAX_WORKERS / 60:.1f} 分钟 (假设每股2分钟)\n"
    )
    ...
    print(f"{'='*100}")
    print(
        f"⏱️  总耗时: {overall_duration:.1f}秒 ({overall_duration/60:.1f}分钟 / {overall_duration/3600:.1f}小时)"
    )
    print(
        f"📊 HK市场: {hk_result['success']}/{hk_result['total']} 成功 ({hk_result['success']/hk_result['total']*100:.1f}%)"
    )
    print(
        f"📊 US市场: {us_result['success']}/{us_result['total']} 成功 ({us_result['success']/us_result['total']*100:.1f}%)"
    )
