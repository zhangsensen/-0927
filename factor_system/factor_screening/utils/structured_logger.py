...
class StructuredLogger:
    ...
    @contextmanager
    def log_performance(self, operation: str, **kwargs: Any):
        """
        性能监控上下文管理器
        ...
        self.info(
            f"开始操作: {operation}", operation=operation, phase="start", **kwargs
        )
        ...
        try:
            ...
            if exception_occurred:
                ...
                self.error(f"操作失败: {operation}", **performance_data)
            else:
                self.info(f"操作完成: {operation}", **performance_data)
    ...
    def log_metric(
        ...
        self.info(f"指标记录: {metric_name} = {value} {unit or ''}", **metric_data)

    def log_alert(
        self, alert_type: str, message: str, severity: str = "warning", **kwargs: Any
    ) -> None:
        """
        记录告警
        ...
        Args:
            ...
            **kwargs: 额外的告警上下文
        """
        alert_data = {"alert_type": alert_type, "severity": severity, **kwargs}

        log_method = getattr(self, severity.lower(), self.warning)
        log_method(f"[ALERT] {message}", **alert_data)
