...
def construct_factor_file_path(
    data_root: Path, symbol: str, timeframe: str, file_suffix: str = "factors"
) -> Path:
    """
    ...
    if mapped_timeframe != timeframe:
        search_patterns.append(
            f"{clean_symbol}{market}_{mapped_timeframe}_{file_suffix}_*.parquet"
        )
        ...
        for pattern in search_patterns:
            ...
            if matching_files:
                ...
                if len(matching_files) > 1:
                    ...
                    logger.debug(
                        f"找到多个匹配文件，选择最新的: {matching_files[0].name}"
                    )
    ...
    available_sample = [
        f.relative_to(market_dir) for f in available_files[:5]
    ]  # 显示相对路径
...
def construct_price_file_path(data_root: Path, symbol: str, timeframe: str) -> Path:
    """
    构建价格数据文件路径
    ...
    if not matching_files:
        raise FileNotFoundError(f"未找到价格文件: {market_dir / pattern}")

    # 如果有多个文件，选择最新的
    if len(matching_files) > 1:
        logger.warning(f"找到多个价格文件: {len(matching_files)}个，使用最新的")
        matching_files.sort(key=lambda x: x.stat().st_mtime, reverse=True)
