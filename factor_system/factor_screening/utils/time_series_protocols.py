...
from typing import Protocol, runtime_checkable, TypeVar, Generic, Union, Optional, List
...
class SafeTimeSeriesProcessor(Generic[T]):
    ...
    def calculate_ic_safe(self, 
        ...
        try:
            ...
            common_index = factor_data.index.intersection(return_data.index)
            aligned_factor = factor_data.loc[common_index]
            aligned_return = return_data.loc[common_index].shift(horizon)  # 注意：这里是正数horizon
...
def safe_ic_calculation(factor_data: pd.Series, 
                       return_data: pd.Series, 
                       horizon: int) -> float:
    """便捷的IC计算函数"""
