...
def analyze_timeframe_results(before_dir: Path, after_dir: Path) -> pd.DataFrame:
    ...
    # 获取所有时间框架
    timeframes = ['1min', '2min', '3min', '5min', '15min', '30min', '60min', '2h', '4h', '1day']
    ...
    for tf in timeframes:
        ...
        # 计算改进百分比
        row['sig_improve'] = ((after_stats.get('significant_factors', 0) - 
                               before_stats.get('significant_factors', 0)) / 
                              max(before_stats.get('significant_factors', 1), 1) * 100)
        
        row['high_score_improve'] = ((after_stats.get('high_score_factors', 0) - 
                                      before_stats.get('high_score_factors', 0)) / 
                                     max(before_stats.get('high_score_factors', 1), 1) * 100)
...
def generate_comparison_report(df: pd.DataFrame, output_path: Path):
    ...
    report.append("")
    report.append(f"{'时间框架':<10} {'修复前':<10} {'修复后':<10} {'改进%':<10} {'Alpha前':<10} {'Alpha后':<10}")
    report.append("-" * 70)
    ...
    report.append("")
    report.append(f"{'时间框架':<10} {'Tier1(前)':<12} {'Tier1(后)':<12} {'Tier2(前)':<12} {'Tier2(后)':<12}")
    report.append("-" * 60)
