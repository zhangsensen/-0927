...
def load_factors_v2(self, symbol: str, timeframe: str) -> pd.DataFrame:
    ...
    try:
        # 使用新的路径构建工具
        factor_file = construct_factor_file_path(self.data_root, symbol, timeframe)
        ...
        self.logger.info(
            f"因子数据加载成功: 形状={factors.shape}, " f"内存={initial_memory:.1f}MB"
        )
        ...
        try:
            ...
            if market_dir.exists():
                self.logger.error(
                    f"可用时间框架目录: {[d.name for d in market_dir.iterdir() if d.is_dir()]}"
                )
            else:
...
def load_price_data_v2(
    ...
    try:
        ...
        self.logger.info(f"✅ 价格数据提取成功: 形状={price_data.shape}")
        self.logger.info(
        ...
        try:
            ...
            if market_dir.exists():
                ...
                if tf_dir.exists():
                    ...
                    self.logger.error(
                        f"可用因子文件: {[f.name for f in available_files[:5]]}"
                    )
        except Exception:
            pass
...
def patch_data_loader(screener_instance):
    ...
    screener_instance.load_factors = types.MethodType(
        load_factors_v2, screener_instance
    )

    # 替换load_price_data方法
    screener_instance.load_price_data = types.MethodType(
        load_price_data_v2, screener_instance
    )
