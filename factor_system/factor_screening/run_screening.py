...
def run_batch_screening(market: str = None, limit: int = None, workers: int = 8):
    ...
    if market:
        ...
        logger.info(f"🚀 开始批量筛选 - {market}市场 (并发度: {MAX_WORKERS}进程, 股票: {len(stocks)}只)")
        if limit:
            logger.info(f"⚠️ 测试模式: 前 {limit} 只股票")
        logger.info(f"📊 时间框架: {ALL_TIMEFRAMES}, 总任务: {len(stocks) * len(ALL_TIMEFRAMES)}")
            ...
            for future in as_completed(future_to_symbol):
                ...
                try:
                    ...
                    if succ_count > 0:
                        ...
                        status = f"❌ 失败: {error_msg[:50] if error_msg else 'Unknown'}"
        ...
        logger.error("批量筛选必须指定市场：--market HK 或 --market US 或 --all-markets")
        return None
...
示例：
  ...
  python run_screening.py --batch --all-markets
    ...
    parser.add_argument('--timeframes', nargs='+', help='多个时间框架 (如: 5min 15min 60min)')
    
    # 批量筛选参数
    parser.add_argument('--market', type=str, choices=['HK', 'US'], help='市场代码 (HK或US)')
    parser.add_argument('--limit', type=int, help='限制筛选的股票数量（用于测试）')
    parser.add_argument('--workers', type=int, default=8, help='并行工作进程数（默认8）')
