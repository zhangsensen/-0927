...
class VectorizedFactorAnalyzer:
    ...
    def calculate_multi_horizon_ic_batch(
        ...
        logger.info(f"开始矩阵化 IC 计算: {len(factors.columns)} 因子 × {len(horizons)} 周期")
        ...
        logger.info(f"过滤零方差因子后剩余: {len(factors_clean.columns)}/{len(valid_factors)} 因子")
        ...
        for horizon in horizons:
            ...
            # 中心化秩
            factors_ranks_centered = current_factors_ranks - current_factors_ranks.mean(axis=0)
            ...
            numerator = (factors_ranks_centered.T @ returns_ranks_centered) / (n_valid - 1)
    ...
    def calculate_rolling_ic_vbt(
        ...
        if n_samples > 20000:  # 20k行以上
            ...
            logger.info(f"大数据集降采样: {n_samples} -> {len(factors_sampled)} (-{n_samples-len(factors_sampled)}行)")
        else:
            factors_sampled = factors
            returns_sampled = returns

        logger.info(f"🚀 VectorBT 滚动 IC (优化版): {len(factors_sampled.columns)} 因子, 窗口={window}")
        ...
        # 4. 过滤异常值
        rolling_ics_clean = rolling_ics_df.replace([np.inf, -np.inf], np.nan).dropna(how='all')
        ...
        # 一致性（向量化符号判断）
        consistency = (np.sign(rolling_ics_clean) == np.sign(rolling_ic_mean)).mean(axis=0)
        ...
        for factor_name in factors_clean.columns:
            ...
            results[factor_name] = {
                ...
                'ic_sharpe': float(rolling_ic_mean[factor_name] / (rolling_ic_std[factor_name] + 1e-8)),
            }
    ...
    def calculate_vif_batch(
        ...
        if original_count > 100:
            ...
            high_variance_factors = factor_stds[factor_stds > factor_stds.median()].index
            factors = factors[high_variance_factors]

            if len(factors.columns) < original_count:
                logger.info(f"预筛选低变化因子: {original_count} -> {len(factors.columns)} (-{original_count-len(factors.columns)}个)")
        ...
        # 2. 标准化
        factors_std = (factors_clean - factors_clean.mean()) / (factors_clean.std() + 1e-8)
        ...
        while iteration < max_iterations:
            ...
            current_vif = {
                factor: float(vif)
                for factor, vif in zip(remaining_factors, vif_values)
            }
    ...
    def calculate_information_increment_batch(
        ...
        # 5. 批量计算所有因子的信息增量
        test_factors = [col for col in factors_clean.columns if col not in available_base]
        ...
        # 相关系数（向量化）
        numerator = (combined_centered.T @ returns_centered) / (len(returns_centered) - 1)
    ...
    def calculate_momentum_persistence_batch(
        ...
        """
        start_time = time.perf_counter()
        logger.info(f"🚀 批量动量持续性分析: {len(factors.columns)} 因子, {len(windows)} 窗口")
        ...
        for window in windows:
            ...
            if len(current_factors) > 20 and forward_returns_sums.size > 20:
                ...
                numerator = ((factor_ranks - factor_mean_ranks) *
                           (returns_ranks_broadcast - returns_mean_rank)).sum(axis=0)  # (F,)
                denominator = (n - 1) * factor_std_safe * returns_std_safe

                momentum_corrs = numerator / denominator  # (F,)

                # 向量化p值计算
                t_stats = momentum_corrs * np.sqrt((n - 2) / (1 - momentum_corrs**2 + 1e-12))
                momentum_p_values = 2 * (1 - stats.t.cdf(np.abs(t_stats), n - 2))

                # 向量化一致性计算（修复广播）
                forward_returns_sums_broadcast = forward_returns_sums[:, np.newaxis]  # (M, 1)
                ...
                # 5. 存储结果（避免因子级循环）
                for idx, factor_name in enumerate(factors_clean.columns):
                    if not np.isnan(momentum_corrs[idx]) and momentum_p_values[idx] < 0.05:
                        if factor_name not in momentum_analysis:
                            momentum_analysis[factor_name] = {
                                'momentum_persistence': float(momentum_corrs[idx]),
                                'momentum_consistency': float(momentum_consistencies[idx]),
                            ...
                            # 选择最佳窗口
                            if abs(momentum_corrs[idx]) > abs(momentum_analysis[factor_name]['momentum_persistence']):
    ...
    def calculate_volatility_sensitivity_batch(
        ...
        # 确保所有数组长度一致
        min_length = min(len(factors_aligned.columns), len(vol_sensitivity), len(stability_scores),
                        len(high_vol_stds), len(low_vol_stds), len(high_vol_counts), len(low_vol_counts))
