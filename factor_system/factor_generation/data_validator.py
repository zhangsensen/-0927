...
class DataValidator:
    ...
    def validate_resampled_data(self, original_data: pd.DataFrame, 
        ...
        try:
            ...
            missing_cols = [col for col in required_cols if col not in resampled_data.columns]
            if missing_cols:
                result['status'] = 'FAIL'
            ...
            invalid_high = (resampled_data['high'] < resampled_data[['open', 'close']].max(axis=1)).sum()
            invalid_low = (resampled_data['low'] > resampled_data[['open', 'close']].min(axis=1)).sum()
            ...
            result['metrics'] = {
                ...
                'compression_ratio': len(original_data) / len(resampled_data) if len(resampled_data) > 0 else 0,
    ...
    def compare_with_existing_data(self, resampled_file: Path, 
                                 original_file: Path,
                                 symbol: str,
                                 timeframe: str) -> Dict:
        """
        ...
        try:
            ...
            available_cols = [col for col in common_cols if col in resampled_common.columns and col in original_common.columns]
            ...
            for col in available_cols:
                # 计算相对差异
                diff = np.abs(resampled_common[col] - original_common[col]) / (original_common[col] + 1e-8)
                avg_diff = diff.mean()
                max_diff = diff.max()
            ...
            logger.debug(f"{symbol} {timeframe} 对比完成: 相似度 {result['similarity_score']:.4f}")
