...
class EnhancedFactorCalculator:
        ...
        try:
            ...
            if self.config.enable_rsi and "RSI" in available_indicators:
                ...
                def create_rsi_calc(window: int) -> Callable[[], pd.Series]:
                    def calc() -> pd.Series:
                        # 使用共享计算器确保与factor_engine、hk_midfreq完全一致
                        rsi = SHARED_CALCULATORS.calculate_rsi(price, period=window)
                        return rsi.rename(f"RSI{window}")
            ...
            if self.config.enable_all_periods:
                ...
                # Fixed Lookback指标 - 启用并修复rename方法问题
                if "FIXLB" in available_indicators:
                    fixlb_windows = timeframe_params["lb_windows"]
                    logger.info(f"{timeframe.value} 使用FIXLB窗口: {fixlb_windows}")

                    def create_fixlb_calc(window: int) -> Callable[[], pd.Series]:
                        def calc() -> pd.Series:
                            # FIXLB需要n参数
                    ...
                    for window in fixlb_windows:
                        factor_calculations.append(
                            (f"FIXLB{window}", create_fixlb_calc(window))
                        )

                # 统计指标 - 启用并修复rename方法问题
                if "FMAX" in available_indicators:
                    stat_windows = timeframe_params["stat_windows"]
                    logger.info(f"{timeframe.value} 使用统计指标窗口: {stat_windows}")

                    # FMAX (Maximum Price)
                    def create_fmax_calc(window: int) -> Callable[[], pd.Series]:
                        def calc() -> pd.Series:
                    ...
                    # FMEAN (Mean Price)
                    def create_fmean_calc(window: int) -> Callable[[], pd.Series]:
                        def calc() -> pd.Series:
                    ...
                    # FMIN (Minimum Price)
                    def create_fmin_calc(window: int) -> Callable[[], pd.Series]:
                        def calc() -> pd.Series:
                    ...
                    def create_fstd_calc(window: int) -> Callable[[], pd.Series]:
                        def calc() -> pd.Series:
                ...
                if self.config.enable_all_periods:
                    ...
                    for window in rand_windows:
                        # RAND - 随机指标
                        def create_rand_calc(w: int) -> Callable[[], pd.Series]:
                            def calc() -> pd.Series:
                                # RAND需要input_shape参数
                                try:
                        ...
                        # RANDX - 随机指标交叉
                        def create_randx_calc(w: int) -> Callable[[], pd.Series]:
                            def calc() -> pd.Series:
                                # RANDX需要entries参数
                                try:
                                    # 创建简单的entries信号
                        ...
                        # RANDNX - 随机指标归一化
                        def create_randnx_calc(w: int) -> Callable[[], pd.Series]:
                            def calc() -> pd.Series:
                    ...
                    for window in rprob_windows:
                        # RPROB - 上涨概率指标
                        def create_rprob_calc(w: int) -> Callable[[], pd.Series]:
                            def calc() -> pd.Series:
                                # RPROB需要prob参数，但是有Series歧义问题，需要特殊处理
                                try:
                                    # 使用简单的布尔值避免Series歧义
                                    prob_param = 0.5
                        ...
                        # RPROBX - 概率指标交叉
                        def create_rprobx_calc(w: int) -> Callable[[], pd.Series]:
                            def calc() -> pd.Series:
                                try:
                    ...
                    for window in st_windows:
                        # STX - 统计趋势指标
                        def create_stx_calc(w: int) -> Callable[[], pd.Series]:
                            def calc() -> pd.Series:
                                # STX需要ts和stop参数
                                try:
                        ...
                        # STCX - 统计趋势累积指标
                        def create_stcx_calc(w: int) -> Callable[[], pd.Series]:
                            def calc() -> pd.Series:
                                # STCX需要ts和stop参数
                                try:
            ...
            if hasattr(vbt, "talib") and self.config.enable_all_periods:
                ...
                def expand_param_grid(param_config: Dict[str, Any]):
                    if not param_config:
                        yield {}
                        return
                    ...
                    values_list = []
                    for key in keys:
                        value = param_config[key]
                        if isinstance(value, (list, tuple, set)):
                            values_list.append(list(value))
                        else:
                ...
                for talib_name, params in talib_params.items():
                    try:
                        ...
                        input_kwargs = {}
                        missing_inputs = []
                        for input_name in talib_indicator.input_names:
                            resolved = resolve_talib_input(input_name)
                            if resolved is None:
                                missing_inputs.append(input_name)
                            else:
                                input_kwargs[input_name] = resolved

                        if missing_inputs:
                            logger.warning(
                                f"跳过TA指标 {talib_name}: 缺少必要输入 {missing_inputs}"
                            )
                            continue

                        param_combos = list(expand_param_grid(params))
                        if not param_combos:
                            param_combos = [{}]

                        for combo in param_combos:
                            suffix = build_param_suffix(combo)
    ...
    def _process_indicator_result(self, name: str, result, factor_data: Dict) -> int:
        ...
        if "CDL" in name.upper():
            try:
                ...
                if extracted is not None:
                    factor_data[name] = extracted.rename(name)
                ...
                for attr in ["labels", "integer", "real", "values"]:
                    if hasattr(result, attr):
                        ...
                        logger.debug(f"CDL模式指标 {name} 使用属性 {attr} 提取成功")
                        return 1

                logger.warning(f"CDL指标 {name} 无法提取有效数据")
                return 0
            except Exception as e:
                logger.warning(f"CDL指标 {name} 处理失败: {e}")
                return 0
        ...
        extracted_values = extract_vbt_indicator(result, name)
        if extracted_values is not None:
            factor_data[name] = to_series(extracted_values, name)
            return 1

        extracted_labels = extract_vbt_labels(result, name)
...
# 测试函数 - 必须提供配置
def test_enhanced_calculator(config: IndicatorConfig):
    """测试增强版因子计算器 - 必须提供配置"""
...
if __name__ == "__main__":
    # 必须提供配置，不再有默认值
    raise ValueError("直接运行此脚本需要提供配置。请使用主程序入口。")
