...
class IntegratedResampler:
    ...
    def resample_ohlcv(self, data: pd.DataFrame, target_timeframe: str) -> pd.DataFrame:
        ...
        logger.debug(
            f"重采样完成: {len(data)} -> {len(resampled)} 行 ({normalized_tf})"
        )

        return resampled

    def find_missing_timeframes(
        self, stock_files: Dict[str, str], required_timeframes: List[str]
    ) -> List[str]:
        """
        找出缺失的时间框架
    ...
    def generate_missing_data(
        ...
        for tf in missing_timeframes:
            try:
                ...
                logger.info(
                    f"{stock_symbol}: 生成 {tf} 数据 -> {output_filename} ({len(resampled_data)} 行)"
                )
    ...
    def ensure_all_timeframes(
        ...
        # 找出缺失的时间框架
        missing_tfs = self.find_missing_timeframes(
            normalized_stock_files, required_timeframes
        )
        ...
        logger.info(
            f"{stock_symbol}: 完成时间框架补全，共 {len(complete_files)} 个时间框架"
        )
...
def batch_resample_from_1min(
    data_1min: pd.DataFrame, target_timeframes: List[str]
