...
class ConfigLoader:
    ...
    def _convert_yaml_to_config(config_data: Dict[str, Any]) -> ScreeningConfig:
        ...
        config = ScreeningConfig(
            ...
            base_factors=independence.get('base_factors', ["MA5", "MA10", "RSI14", "MACD_12_26_9"]),
    ...
    def validate_config(config: ScreeningConfig) -> Dict[str, Any]:
        ...
        # 检查权重
        total_weight = (config.weight_predictive + config.weight_stability + 
                       config.weight_independence + config.weight_practicality + 
                       config.weight_adaptability)
        
        if abs(total_weight - 1.0) > 0.01:
            validation_result['errors'].append(f"权重总和必须为1.0，当前为{total_weight:.3f}")
            validation_result['is_valid'] = False
