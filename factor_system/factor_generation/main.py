...
class FactorGenerationApp:
    ...
    def generate_factors(self, symbol: str, timeframes: List[str], data_path: str) -> None:
        """
        生成因子数据
        ...
        for timeframe in timeframes:
            try:
                ...
                factors = self.calculator.calculate_comprehensive_factors(resampled_data, tf_enum)

                if factors is not None:
                    # 保存结果
                    self._save_factors(factors, symbol, timeframe)
                    logger.info(f"✅ {symbol} {timeframe} 因子生成完成: {factors.shape}")
                else:
    ...
    def _save_factors(self, factors: pd.DataFrame, symbol: str, timeframe: str) -> None:
        ...
        try:
            ...
            output_dir = Path(config_data.get('output', {}).get('directory', './factor_output'))
