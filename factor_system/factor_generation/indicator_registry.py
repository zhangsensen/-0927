...
class IndicatorRegistry:
    ...
    def register(self, spec: IndicatorSpec):
        ...
        logger.debug(f"注册指标: {spec.name} (type={spec.indicator_type}, batch={spec.batch_capable})")
    ...
    def list_batch_capable(self) -> List[IndicatorSpec]:
        """列出支持批量计算的指标"""
        return [spec for spec in self.specs.values() if spec.enabled and spec.batch_capable]
    
    def get_by_type(self, indicator_type: str) -> List[IndicatorSpec]:
        """按类型获取指标"""
        return [spec for spec in self.specs.values() if spec.enabled and spec.indicator_type == indicator_type]
