...
def main():
    ...
    for file_path in sorted(factor_files):
        ...
        if result['consistent']:
            print(f"✅ {symbol:10s} {timeframe:10s}: {result['parquet_count']} 因子 (完美匹配)")
        else:
            print(f"❌ {symbol:10s} {timeframe:10s}: {result['parquet_count']} 因子 (不一致)")
            all_consistent = False
            
            if result['missing']:
                print(f"   缺失 {len(result['missing'])} 个: {result['missing'][:5]}...")
            if result['extra']:
