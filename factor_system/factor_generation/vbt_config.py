...
class VBTConfigManager:
    ...
    def configure(self, config: Dict[str, Any]):
        ...
        if config.get('n_jobs', 1) != 1:
            self._enable_parallel(config.get('n_jobs', -1), config.get('chunked', 'auto'))
        
        logger.info(f"VectorBT配置完成: cache={self.cache_enabled}, parallel={self.parallel_enabled}")
    ...
    def restore(self):
        ...
        try:
            if 'caching_enabled' in self._original_settings:
                vbt.settings.caching.enabled = self._original_settings['caching_enabled']
