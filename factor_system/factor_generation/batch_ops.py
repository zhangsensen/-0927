...
def execute_atr_batch(high: pd.Series, low: pd.Series, close: pd.Series, windows: List[int]) -> pd.DataFrame:
    """批量计算平均真实范围
...
def execute_rolling_stats_batch(price: pd.Series, windows: List[int], stat_type: str) -> pd.DataFrame:
...
class BatchExecutor:
    ...
    @staticmethod
    def execute(indicator_name: str, data: Dict[str, pd.Series], params: Dict[str, List[Any]]) -> pd.DataFrame:
        """执行批量计算
        ...
        if indicator_name == "MA":
            ...
            return execute_atr_batch(data['high'], data['low'], price, params.get('window', []))
            ...
            return execute_rolling_stats_batch(price, params.get('window', []), stat_type)
        else:
