...
class BatchFactorProcessor:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.max_workers = min(
            mp.cpu_count(), 
            perf_config.get('max_workers', mp.cpu_count())
        )
        ...
        self.temp_resample_dir = Path(self.config.get('resampling', {}).get('temp_dir', './temp_resampled'))
            ...
            for file_path in market_dir.glob("*.parquet"):
                ...
                if self.resampler:
                    ...
                    if original_timeframe != timeframe:
                        logger.debug(f"标准化时间框架: {original_timeframe} -> {timeframe}")
                else:
    ...
    def process_single_stock(self, stock_info: StockInfo) -> Tuple[str, bool, str, int]:
        ...
        try:
            ...
            if self.enable_resampling and self.resampler:
                # 获取配置中需要的时间框架
                required_timeframes = self.config.get('timeframes', {}).get('enabled', 
                    ['1min', '2min', '3min', '5min', '15min', '30min', '60min', '1day'])
                
                # 确保所有时间框架数据存在
                complete_file_paths = self.resampler.ensure_all_timeframes(
                    symbol, stock_info.file_paths, required_timeframes, self.temp_resample_dir
                )
                
                logger.debug(f"{symbol}: 完成时间框架检查，共 {len(complete_file_paths)} 个时间框架")
            
            # 初始化因子计算器 - 复用实例避免重复初始化
            from factor_system.factor_engine.batch_calculator import BatchFactorCalculator
            from pathlib import Path

            project_root = Path(self.config.get('data.root_dir', '/Users/zhangshenshen/深度量化0927/raw')).parent.parent
            ...
            for timeframe, file_path in complete_file_paths.items():
                ...
                if factors_df is not None and not factors_df.empty:
                    ...
                    if available_price_columns:
                        ...
                        logger.debug(f"{symbol} {timeframe}: 合并数据 - 价格列: {len(available_price_columns)}, 因子列: {len(factors_df.columns)}")
                    else:
                        combined_df = factors_df
                        logger.warning(f"{symbol} {timeframe}: 未找到价格数据列，仅保存因子数据")
                    
                    # 保存合并后的数据 - 按市场分别存储
                    timestamp_suffix = f"_{self.batch_timestamp}" if self.batch_timestamp else ""
                    if self.separate_by_market:
                        market_dir = self.output_dir / stock_info.market
                        output_path = market_dir / timeframe / f"{symbol}_{timeframe}_factors{timestamp_suffix}.parquet"
                    else:
                        output_path = self.output_dir / timeframe / f"{symbol}_{timeframe}_factors{timestamp_suffix}.parquet"
                    ...
                    logger.debug(f"{symbol} {timeframe}: {len(factors_df.columns)} 个因子 + {len(available_price_columns)} 个价格列")
            ...
            logger.info(f"✅ {symbol}: {total_factors} 个因子, 内存: {memory_used:.1f}MB")
    ...
    def process_batch(self, stocks: Dict[str, StockInfo], 
                     batch_size: Optional[int] = None) -> ProcessingStats:
        """批量处理所有股票"""
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            with tqdm(total=len(valid_stocks), desc="处理股票") as pbar:
                for future in as_completed(future_to_symbol):
                    ...
                    try:
                        symbol_result, success, error_msg, factor_count = future.result()
        ...
        if self.failed_stocks:
            ...
            logger.error(f"处理失败 {len(self.failed_stocks)} 只股票 (前5条): {preview}")

            # 输出完整失败列表供排查
            failed_summary = '\n'.join(f"{sym}: {err}" for sym, err in self.failed_stocks)
            logger.error(f"处理失败股票明细:\n{failed_summary}")

        # 清理临时重采样文件
        if self.enable_resampling and self.config.get('resampling', {}).get('cleanup_temp', True):
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"成功率: {self.stats.processed_stocks/self.stats.total_stocks*100:.1f}%")
...
def main():
    ...
    parser.add_argument("--raw-dir", default="/Users/zhangshenshen/深度量化0927/raw",
                       help="原始数据目录")
