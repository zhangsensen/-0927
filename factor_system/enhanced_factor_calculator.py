...
INDICATOR_DATA_REQUIREMENTS = {
    # 基础移动平均线类
    'SMA': ['close'], 'EMA': ['close'], 'WMA': ['close'], 'DEMA': ['close'], 'TEMA': ['close'],
    'TRIMA': ['close'], 'KAMA': ['close'], 'T3': ['close'],

    # 动量指标类
    'MOM': ['close'], 'ROC': ['close'], 'RSI': ['close'], 'TRIX': ['close'],

    # 波动率指标类
    'STDDEV': ['close'], 'VAR': ['close'], 'ATR': ['high', 'low', 'close'],
    'NATR': ['high', 'low', 'close'],
    ...
    # 布林带类
    'BBANDS': ['close'], 'MIDPOINT': ['high', 'low'], 'MIDPRICE': ['high', 'low'],
    'SAR': ['high', 'low'], 'SAREXT': ['high', 'low'],

    # 随机指标类
    'STOCH': ['high', 'low', 'close'], 'STOCHF': ['high', 'low', 'close'], 'STOCHRSI': ['close'],
    ...
    'DI': ['high', 'low', 'close'], 'DX': ['high', 'low', 'close'],
    'MINUS_DI': ['high', 'low', 'close'], 'PLUS_DI': ['high', 'low', 'close'],
...
def calculate_indicator_with_validation(talib_indicator, output_name, talib_name, df, *args, **kwargs):
    ...
    try:
        ...
        if talib_name in ['SAR', 'SAREXT']:
            ...
            result = talib_indicator.run(**kwargs, **{
...
class EnhancedFactorCalculator:
                        ...
                        if not params:
                            ...
                            def make_unified_calc():
                                def calc():
                                    return calculate_indicator_with_validation(
                                        talib_indicator, f'TA_{talib_name}', talib_name,
                                        price, high, low, df['open'], volume, df.index
                                    )
                                return calc
                            factor_calculations.append((f'TA_{talib_name}', make_unified_calc()))
                            ...
                            for period in periods:
                                def make_unified_period_calc(p=period):
                                    def calc():
                                        return calculate_indicator_with_validation(
                                            talib_indicator, f'TA_{talib_name}_{p}', talib_name,
                                            price, high, low, df['open'], volume, df.index,
                                            timeperiod=p
                                        )
                                    return calc
                                factor_calculations.append((f'TA_{talib_name}_{period}', make_unified_period_calc()))
    ...
    def _process_indicator_result(self, name: str, result, factor_data: Dict) -> int:
        ...
        # BOLB指标 (Binary Oscillator with Lower Bounds) - 特殊处理VectorBT对象
            ...
            processed_count = 1
        # 布林带系列指标
        elif hasattr(result, 'upper') and hasattr(result, 'middle') and hasattr(result, 'lower'):
