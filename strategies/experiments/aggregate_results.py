...
def generate_summary_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        汇总统计 DataFrame
    """
    summary = df.groupby(['top_n', 'fee']).agg({
...
def plot_sharpe_topn_heatmap(
    ...
    # 计算每个 Top-N 的平均夏普
    pivot = df_filtered.groupby('top_n')['sharpe'].agg(['mean', 'max', 'std']).reset_index()
...
def plot_sharpe_fee_curve(df: pd.DataFrame, output_path: Path, top_n: int = 8):
    ...
    # 计算每个费率的统计
    fee_stats = df_filtered.groupby('fee')['sharpe'].agg(['mean', 'max', 'std']).reset_index()
    ...
    plt.errorbar(x, y_mean, yerr=y_std, fmt='o-', capsize=5, label='Mean ± Std', linewidth=2)
...
def compare_with_history(
    ...
    comparison['Improvement'] = (
        ...
        comparison['History Best'].abs() * 100
    )
