...
def run_single_experiment(
    config_path: Path, script_path: Path, dry_run: bool = False
) -> Dict:
    """运行单个实验
...
def find_experiment_configs(
    config_dir: Path, pattern: Optional[str] = None
) -> List[Path]:
    """查找实验配置文件
...
def main():
    parser = argparse.ArgumentParser(description="实验管线运行器")

    parser.add_argument("--config", type=str, help="单个实验配置文件路径")
    parser.add_argument(
        "--pattern", type=str, help="实验配置文件名模式（如：p0_*.yaml）"
    )
    parser.add_argument("--all", action="store_true", help="运行所有实验配置")
    ...
    for config_file in config_files:
        result = run_single_experiment(config_file, script_path, dry_run=args.dry_run)
