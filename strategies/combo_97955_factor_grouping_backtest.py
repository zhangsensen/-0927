...
FACTOR_GROUPS = {
    'short_term': {
        'name': '短期（<20日）',
        'patterns': ['_5', '_6', '_9', '_10', '_12', '_14', 'PRICE_POSITION_10', 'RSI', 'MFI', 'STOCH', 'CCI', 'WILLIAMS'],
...
def group_factors(factors: List[str], weights: np.ndarray) -> Dict[str, Dict]:
    ...
    for group_name, info in grouped.items():
        group_info = FACTOR_GROUPS.get(group_name, {'name': group_name, 'description': 'Unknown'})
...
def calculate_persistence_layers(
...
def run_combination_sensitivity_test(
    ...
    test_combinations = {
        ...
        'mid_20_30_long': ['mid_term_20', 'mid_term_30', 'long_term', 'volatility_filter'],
    ...
    for combo_name, group_names in test_combinations.items():
        ...
        print(f"  组合 {combo_name}: {len(all_factors)}个因子（来自{len(group_names)}个分组）")
...
def calculate_theme_persistence_indicator(
    ...
    # 标准化到[0, 1]
    persistence_index = (persistence_index - persistence_index.min()) / (persistence_index.max() - persistence_index.min())
...
def create_visualizations(
    ...
    for _, row in combination_results_df.iterrows():
        values = [
            ...
            row['win_rate'],
            1 - row['turnover'] / combination_results_df['turnover'].max()  # 换手率越低越好
        ]
        values += values[:1]
    ...
    ax.bar(x - width/2, false_signal_rates, width, label='假信号率', color='red', alpha=0.7)
    ...
    if len(persistence_index) > 0:
        ...
        ax.plot(persistence_index.index, persistence_index.values, linewidth=1.5, alpha=0.8)
        ...
        ax.axhline(q75, color='green', linestyle='--', label=f'Q75 (强持续): {q75:.3f}', alpha=0.7)
        ax.axhline(q50, color='orange', linestyle='--', label=f'Q50 (中等): {q50:.3f}', alpha=0.7)
        ax.axhline(q25, color='red', linestyle='--', label=f'Q25 (转折): {q25:.3f}', alpha=0.7)
        ...
        ax.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig(output_dir / 'persistence_timeseries.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    assert total_grouped == len(factors), f"🚨 因子覆盖缺口: {total_grouped}/{len(factors)}"
    ...
    print(f"✅ 数据加载完成: {len(factors)}个因子, {len(price_pivot)}个交易日, {len(price_pivot.columns)}个标的")
    ...
    print(group_results_df[[
        'group_name', 'n_factors', 'sharpe', 'annual_return', 
        'max_drawdown', 'win_rate', 'turnover', 'n_trades'
    ...
    print(combination_results_df[[
        'combination', 'n_factors', 'sharpe', 'annual_return',
        'max_drawdown', 'win_rate', 'turnover', 'hit_rate_signal', 'false_signal_rate'
    ]].to_string(index=False))
    ...
    if 'volatility_filter' in grouped_factors:
        persistence_index = calculate_theme_persistence_indicator(
            factor_panel,
            grouped_factors['volatility_filter']['factors']
        )
        ...
        if len(persistence_index) > 0:
            ...
            print(f"   范围: [{persistence_index.min():.4f}, {persistence_index.max():.4f}]")
    ...
    group_results_df[[
        'group_name', 'n_factors', 'annual_return', 'max_drawdown',
        'sharpe', 'calmar', 'win_rate', 'turnover', 'n_trades'
    ...
    combination_results_df[[
        'combination', 'n_factors', 'annual_return', 'max_drawdown',
        'sharpe', 'calmar', 'win_rate', 'turnover', 'n_trades',
        'hit_rate_signal', 'false_signal_rate', 'net_cumulative_return'
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write("---\n\n")
        f.write("## 3. 组合敏感度测试结果\n\n")
        f.write(combination_results_df[[
            'combination', 'n_factors', 'sharpe', 'annual_return',
            'max_drawdown', 'win_rate', 'turnover', 'hit_rate_signal', 'false_signal_rate'
        ]].to_markdown(index=False))
        f.write("\n\n")
