...
class ComboWFOOptimizer:
    ...
    def _test_combo_single_window(
        ...
        pos_size: int = 2, commission_rate: float = 0.0002
    ):
        """
        测试单个 WFO 窗口的因子组合表现
        ...
        oos_return = _compute_rebalanced_return(
            signal_oos, returns_oos, best_freq, pos_size, commission_rate
        )
    ...
    def run_combo_search(self, factors_data, returns, factor_names, top_n=100,
        ...
        results = Parallel(n_jobs=self.config.n_jobs, verbose=0)(
            delayed(self._test_combo_impl)(
                combo, factors_data, returns, windows, pos_size, commission_rate
            )
