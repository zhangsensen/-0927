...
@njit
def _rolling_yield_slope_numba(short_term: np.ndarray, long_term: np.ndarray, window: int) -> np.ndarray:
    """
    计算滚动收益率曲线斜率（长端 - 短端）
...
@njit
def _rolling_duration_proxy_numba(prices: np.ndarray, rates: np.ndarray, window: int) -> np.ndarray:
    """
    久期代理：价格对利率变化的敏感性
...
class BondFactors:
    ...
    def duration_proxy_60d(
        self, 
        close: pd.Series, 
        benchmark_rate: Optional[pd.Series] = None
    ) -> pd.Series:
        """
    ...
    def bond_vol_regime(self, close: pd.Series) -> pd.Series:
        ...
        vol_regime = vol_20.rolling(window=60, min_periods=60).apply(rolling_percentile, raw=False)
        return vol_regime
    ...
    def compute_all(self, prices: Dict[str, pd.DataFrame], symbols: List[str]) -> pd.DataFrame:
        """
        批量计算所有债券因子
        ...
        df.columns = pd.MultiIndex.from_tuples(
            [(col[1], col[0]) for col in df.columns],
            names=["factor", "symbol"]
        )
...
class CommodityFactors:
    ...
    def usd_inverse_mom_20d(self, close: pd.Series, usd_proxy: Optional[pd.Series] = None) -> pd.Series:
        """
    ...
    def compute_all(
        ...
        for symbol in symbols:
            ...
            results[(symbol, "GOLD_SAFE_HAVEN_SCORE")] = self.gold_safe_haven_score(close, market_close)
            results[(symbol, "COMMODITY_VOL_ADJ_MOM")] = self.commodity_vol_adj_mom(close)
...
class CategoryFactorManager:
    ...
    def get_factors_for_category(self, category: str) -> List[str]:
        """获取指定类别的因子列表"""
        return [name for name, cat in self.factor_category_map.items() if cat == category]
