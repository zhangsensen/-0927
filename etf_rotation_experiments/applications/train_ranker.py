...
⚠️  注意: 本脚本保留用于向后兼容
推荐使用新的统一Pipeline入口: applications/run_ranking_pipeline.py
...
使用方式:
  # 传统单数据源训练 (本脚本)
  python applications/train_ranker.py --wfo-dir results/run_xxx --backtest-dir results_combo_wfo/xxx
...
from strategies.ml_ranker.data_loader import build_training_dataset, find_latest_wfo_run, find_latest_backtest_run
...
from strategies.ml_ranker.evaluator import generate_evaluation_report, create_ranking_comparison_df


def parse_args():
    parser = argparse.ArgumentParser(
        description="训练LTR排序模型 (单数据源)",
        epilog="推荐使用: python applications/run_ranking_pipeline.py --config configs/ranking_datasets.yaml"
    )
    parser.add_argument(
        "--wfo-dir",
        type=str,
...
def main():
    ...
    if args.use_pipeline and PIPELINE_AVAILABLE:
        ...
        print(f"   python applications/run_ranking_pipeline.py --config configs/ranking_datasets.yaml\n")
    ...
    from strategies.ml_ranker.data_loader import load_wfo_features, load_real_backtest_results
