...
EXPERIMENTS_ROOT = _HERE.parents[1]
...
def _compute_or_load_daily_ic_memmap(factors_data_full: np.ndarray, returns: np.ndarray, stable_rank: bool) -> np.ndarray:
    ...
    环境变量:
        RB_DAILY_IC_MEMMAP_DIR: 目录（默认项目根下 .cache）
    ...
    """
    memmap_dir_env = os.environ.get("RB_DAILY_IC_MEMMAP_DIR", "").strip()
    if memmap_dir_env:
        memmap_dir = Path(memmap_dir_env).expanduser()
    elif CACHE_ROOT is not None:
        memmap_dir = CACHE_ROOT
    else:
        memmap_dir = (PROJECT_ROOT / "etf_rotation_experiments" / ".cache").resolve()
    ...
    path = memmap_dir / file_name

    if path.exists():
        mm = np.memmap(str(path), dtype=np.float32 if use_fp32 else np.float64, mode="r", shape=(T, F_total))
    ...
    for _ in range(50):  # 最多等待 ~5s
        try:
            ...
            time.sleep(0.1)
    if not got_lock and path.exists():
        mm = np.memmap(str(path), dtype=np.float32 if use_fp32 else np.float64, mode="r", shape=(T, F_total))
    ...
    try:
        ...
        mm = np.memmap(str(path), dtype=arr_to_store.dtype, mode="w+", shape=(T, F_total))
        mm[:] = arr_to_store[:]
        del mm  # 关闭文件句柄
...
def backtest_no_lookahead(
    ...
    *,
    ...
    """
    ⚠️ 严格无未来函数的回测 (优化版 + V10止损机制 + Light Timing)
    ...
    # 执行延迟修正 (消除前视偏差)
    # 信号在 T 日生成，T+1 日才生效（时间对齐：用 T 日因子预测 T+1 日收益）
    # 原因：returns[t] = close[t]/close[t-1]-1 是 day t 全天收益，但因子在 day t 收盘后计算
    # 立即应用会导致反向因果偏差（用收盘后因子预测已发生的当天收益）
    ...
    # === V10 止损机制状态跟踪 ===
    # entry_prices[n] = 入场时的累计净值基准（用于计算持仓收益）
    # 使用 cumulative return 而非 price 避免数据对齐问题
    entry_cumret = np.ones(N, dtype=np.float64)  # 每个ETF入场时的累计收益基准
    etf_cumret = np.ones(N, dtype=np.float64)    # 当前每个ETF的累计收益 (1 + sum returns)
    ...
    for offset, day_idx in enumerate(range(start_idx, T)):
        # 在计算收益前，先检查是否有待应用的 pending（延迟应用逻辑）
        if pending_ready and pending_weights is not None:
            # 记录新买入ETF的入场基准
            new_positions = (pending_weights > 0) & (current_weights == 0)
        ...
        if stop_loss_enabled:
            ...
            if np.any(held_mask):
                ...
                if np.any(stop_triggered):
                    ...
                    # 重新归一化剩余仓位（保持满仓）
                    remaining_weight = np.sum(current_weights)
                    if remaining_weight > 1e-12:
                        current_weights = current_weights / remaining_weight
                    # 注：如果全部被止损，current_weights 全为0，当日收益为0
    ...
    result = {
        ...
        "stop_loss_events": stop_loss_events if len(stop_loss_events) < 100 else stop_loss_events[:100],  # 防止过大
        # 详细数据
...
def main():
    ...
    global CACHE_ROOT
    cache_dir_cfg = (config.get("data") or {}).get("cache_dir")
    CACHE_ROOT = Path(cache_dir_cfg).expanduser().resolve() if cache_dir_cfg else None
    ...
    # experiments 树内的默认 results 目录
    candidate_roots.append((EXPERIMENTS_ROOT / "results").resolve())
    candidate_roots.append((EXPERIMENTS_ROOT / "results_combo_wfo").resolve())
    # 🔥 移除白名单路径推断逻辑（已无白名单机制）
    # 去重
    unique_roots = []
    seen = set()
    ...
    def _backtest_single_combo(
        ...
        try:
            result = backtest_no_lookahead(
                ...
                etf_stop_loss=etf_stop_loss_cfg,
            )
