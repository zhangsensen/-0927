...
def main():
    ...
    # 计算main_net
    main_net_raw = (
        raw_mf["buy_lg_amount"]
        + raw_mf["buy_elg_amount"]
        - raw_mf["sell_lg_amount"]
        - raw_mf["sell_elg_amount"]
    ) * 10000  # 转换为元
    ...
    data = combined.load_price_data(
        [symbol], "daily", datetime(2024, 8, 20), datetime(2024, 9, 10)
    )
    ...
    if first_valid_idx:
        ...
        # 向前查找最近的交易日
        while (
            prev_date not in main_net_raw.index
            and prev_date >= main_net_raw.index.min()
        ):
    ...
    if first_valid_idx:
        ...
        while (
            two_days_before not in main_net_raw.index
            and two_days_before >= main_net_raw.index.min()
        ):
