...
class TestSessionResample:
    ...
    def test_daily_bar_count(self, provider, test_symbol, date_range, timeframe, expected_bars):
        """测试每日K线数是否符合预期"""
        ...
        assert abs(avg_bars - expected_bars) < 0.1, \
    ...
    def test_no_cross_lunch_bars(self, provider, test_symbol, date_range):
        ...
        lunch_stamps = [
            ts for ts in resampled.index 
            if ts.hour == 12 or (ts.hour == 11 and ts.minute > 30)
        ]
        
        assert len(lunch_stamps) == 0, \
            f"发现跨午休时间戳: {lunch_stamps}"
