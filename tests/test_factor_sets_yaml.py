...
class TestFactorSetsYAML:
    ...
    def test_daily_default_research_composition(self, registry):
        ...
        for factor in tech_mini:
            assert (
                factor in factor_ids
            ), f"daily_default_research缺少tech_mini的: {factor}"
        ...
        for factor in money_flow:
            assert (
                factor in factor_ids
            ), f"daily_default_research缺少money_flow_core的: {factor}"
        ...
        print(
            f"   包含: tech_mini({len(tech_mini)}) + money_flow_core({len(money_flow)}) + 其他"
        )
    ...
    def test_all_factors_expansion(self, registry):
        ...
        print(
            f"   注册表: factors={len(registry.factors)}, metadata={len(registry.metadata)}"
        )
