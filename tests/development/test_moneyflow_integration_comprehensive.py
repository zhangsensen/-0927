...
def test_moneyflow_integration():
    ...
    try:
        ...
        print(f"   数据覆盖率: {(1 - result.isnull().sum().sum() / result.size) * 100:.2f}%")
        ...
        if hasattr(result.index, 'levels'):
            for symbol in test_symbols:
                try:
                    ...
                    print(f"   {symbol}: {len(symbol_data)}天, 有效率{valid_ratio:.2f}%")
                except:
                    print(f"   {symbol}: 无数据")
    ...
    try:
        ...
        for set_id in factor_sets:
            ...
            if factor_set:
                print(f"   - {set_id}: {factor_set.get('name')} ({len(factor_set.get('factors', []))}个因子)")
    ...
    for test_name, result in results.items():
        if result is not None and not result.empty:
            ...
            print(f"  有效率: {(1 - result.isnull().sum().sum() / result.size) * 100:.2f}%")
