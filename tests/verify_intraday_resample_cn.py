...
def main():
    parser = argparse.ArgumentParser(
        description="Verify A-share session-aware resample"
    )
    ...
    base = provider._load_timeframe_file(
        args.symbol,
        provider.market_dirs[provider._detect_market(args.symbol)],
        "1min",
        start,
        end,
    )
    if base.empty:
        ...
        return
    print(
        f"✅ 1min loaded: {len(base)} rows, range: {base.index.min()} ~ {base.index.max()}"
    )
