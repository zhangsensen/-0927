...
def find_positive_net_return_strategies(df, min_net_return=0.02):
    ...
    if len(positive_strategies) > 0:
        print("\n前10名净收益策略:")
        cols = ['combo_idx', 'annual_return', 'annual_cost', 'net_return', 'net_sharpe', 'turnover', 'sharpe']
...
def beta_optimization_pareto(df, beta_range=[0.001, 0.003, 0.005, 0.007, 0.01]):
    ...
    for beta in beta_range:
        ...
        print(f"β={beta}: 最佳{best_strategy['combo_idx']}, "
...
def realistic_strategy_selection(df):
    ...
    if len(realistic_df) > 0:
        print("\n前5名现实可行策略:")
        cols = ['combo_idx', 'net_return', 'net_sharpe', 'turnover', 'concentration', 'annual_cost', 'sharpe']
        ...
        print(f"\n对比分析:")
        print(f"原始最佳(62774): 毛夏普{original_best['sharpe']:.4f}, 净收益{original_best['net_return']:.3f}")
        print(f"现实最佳({realistic_best['combo_idx']}): 净夏普{realistic_best['net_sharpe']:.4f}, 净收益{realistic_best['net_return']:.3f}")
...
def main():
    ...
    # 保存结果
    if len(positive_strategies) > 0:
        positive_strategies.to_csv('/Users/zhangshenshen/深度量化0927/strategies/results/positive_net_strategies.csv', index=False)
        print(f"\n正净收益策略已保存: {len(positive_strategies)}个")
    
    beta_results.to_csv('/Users/zhangshenshen/深度量化0927/strategies/results/beta_optimization_results.csv', index=False)
    
    if realistic_strategies is not None and len(realistic_strategies) > 0:
        realistic_strategies.to_csv('/Users/zhangshenshen/深度量化0927/strategies/results/realistic_strategies.csv', index=False)
