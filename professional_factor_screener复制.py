...
class ProfessionalFactorScreener:
    ...
    def validate_factor_data_quality(self, factors_df: pd.DataFrame, symbol: str, timeframe: str) -> pd.DataFrame:
        """验证因子数据质量 - Linus式直接解决方案"""
        self.logger.info(f"开始数据质量验证: {symbol} {timeframe}")
        ...
        # 2. 检查缺失值
        missing_cols = factors_df.columns[factors_df.isnull().any()].tolist()
        if missing_cols:
            self.logger.warning(f"发现缺失值列: {missing_cols}")
            factors_df = factors_df.dropna(axis=1, how='any')
            issues_found.append(f"移除缺失值列: {missing_cols}")
        ...
        if inf_cols:
            self.logger.warning(f"发现无穷值列: {inf_cols}")
            factors_df = factors_df.replace([np.inf, -np.inf], np.nan)
            factors_df = factors_df.dropna(axis=1, how='any')
            issues_found.append(f"移除无穷值列: {inf_cols}")
        ...
        # 6. 检查无效列名
        invalid_cols = [col for col in factors_df.columns if not isinstance(col, str) or len(str(col).strip()) == 0]
        if invalid_cols:
            self.logger.warning(f"发现无效列名: {invalid_cols}")
            factors_df = factors_df.drop(columns=invalid_cols)
            issues_found.append(f"移除无效列名: {invalid_cols}")

        final_shape = factors_df.shape

        # 报告验证结果
        if issues_found:
            self.logger.warning(f"数据质量问题已修复:")
            for issue in issues_found:
                self.logger.warning(f"  - {issue}")
            self.logger.info(f"数据形状变化: {original_shape} -> {final_shape}")
        else:
            self.logger.info(f"数据质量验证通过: {final_shape}")
