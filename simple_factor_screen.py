...
def main():
    ...
    if factor_results:
        ...
        print("\n🏆 Top 30 因子:")
        print("-" * 100)
        print(f"{'排名':<4} {'因子名称':<40} {'IC均值':<10} {'IR':<8} {'胜率':<8} {'样本数':<8}")
        print("-" * 100)

        top_factors = factor_results[:30]
        for i, factor in enumerate(top_factors, 1):
            print(f"{i:<4} {factor['factor_name']:<40} {factor['ic_mean']:<10.4f} {factor['ic_ir']:<8.2f} {factor['positive_ratio']:<8.2%} {factor['samples']:<8}")

        # 保存结果
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = f"/Users/zhangshenshen/深度量化0927/top_30_factors_{timestamp}.json"
        ...
        # 生成简化的因子列表
        factor_list_path = f"/Users/zhangshenshen/深度量化0927/top_30_factors_list_{timestamp}.txt"
        with open(factor_list_path, 'w', encoding='utf-8') as f:
            f.write("Top 30 ETF因子列表\n")
            f.write("=" * 50 + "\n\n")
            for i, factor in enumerate(top_factors, 1):
                f.write(f"{i:2d}. {factor['factor_name']} (IC: {factor['ic_mean']:.4f}, IR: {factor['ic_ir']:.2f})\n")
