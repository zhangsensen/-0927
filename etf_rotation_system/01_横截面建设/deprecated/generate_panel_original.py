...
def calculate_factors_single(args: Tuple[str, pd.DataFrame, Dict]) -> pd.DataFrame:
    ...
    try:
        ...
        for window in config['volatility_windows']:
            factors[f'VOLATILITY_{window}D'] = (ret.rolling(window, min_periods=1).std() * np.sqrt(252)).values
        ...
        pv_divergence = np.sign(price_change) * vol_change.rolling(5, min_periods=1).mean()
...
def generate_etf_panel(
    ...
    try:
        ...
        # 加载配置
        config = load_config(config_path) if config_path else load_config('config/etf_config.yaml')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output-dir', default='etf_rotation_system/data/results/panels', help='输出目录')
