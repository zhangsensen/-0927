...
class BacktestConfig:
    ...
    # 权重网格
    weight_grid_points: List[float] = field(default_factory=lambda: [0.0, 0.25, 0.5, 0.75, 1.0])
...
class ConfigLoader:
    ...
    def load_config(self, preset_name: Optional[str] = None,
                   overrides: Optional[Dict[str, Any]] = None) -> BacktestConfig:
        """
        加载配置文件
        ...
        if preset_name and 'presets' in yaml_config:
            if preset_name not in yaml_config['presets']:
                raise ValueError(f"预设 '{preset_name}' 不存在。可用预设: {list(yaml_config['presets'].keys())}")
    ...
    def _yaml_to_config(self, yaml_config: Dict) -> BacktestConfig:
        ...
        return BacktestConfig(
            ...
            output_dir=data_paths.get('output_dir', 'etf_rotation_system/03_vbt回测/results'),
            ...
            # 权重网格
            weight_grid_points=weight_grid.get('grid_points', [0.0, 0.25, 0.5, 0.75, 1.0]),
            weight_sum_range=weight_grid.get('weight_sum_range', [0.7, 1.3]),
            max_combinations=weight_grid.get('max_combinations', 10000),

            # 复合因子计算
            standardization_method=composite_config.get('standardization_method', 'zscore'),
    ...
    def _validate_config(self):
        ...
        if self.config.primary_metric not in ['sharpe_ratio', 'total_return', 'max_drawdown']:
            raise ValueError("primary_metric 必须是 'sharpe_ratio', 'total_return' 或 'max_drawdown'")
    ...
    def apply_environment_config(self):
        ...
        os.environ.setdefault("VECLIB_MAXIMUM_THREADS", str(self.config.veclib_maximum_threads))
