...
def generate_weight_combinations_vectorized(
    ...
    valid_combos = (
        weight_combos[:max_combinations]
        if max_combinations < len(weight_combos)
        else weight_combos
    )
...
def grid_search_weights_vectorized(
    ...
    3. 内存优化
    """
    from .backtest_engine_configurable import (
        backtest_topn_rotation,
    ...
    if results:
        ...
        logger.info(
            f"最优策略: {best_weights}, top_n={best_top_n}, sharpe={best_sharpe:.3f}"
        )
