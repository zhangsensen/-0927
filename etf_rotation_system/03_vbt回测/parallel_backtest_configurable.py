...
class ConfigurableParallelBacktestEngine:
    ...
    def _vectorized_batch_backtest(
        ...
        returns = (
            price_aligned.pct_change().fillna(0.0).values
        ...
        net_returns = np.concatenate(
            [np.zeros((n_combinations, 1)), net_returns], axis=1
        )
    ...
    def parallel_grid_search(self) -> pd.DataFrame:
        ...
        self.logger.info(f"✓ 因子面板加载完成，耗时: {time.time() - stage_start:.2f}秒")

        stage_start = time.time()
        prices = self._load_price_data()
        self.logger.info(f"✓ 价格矩阵加载完成，耗时: {time.time() - stage_start:.2f}秒")
