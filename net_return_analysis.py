...
def analyze_net_returns(df, beta_values=[0.001, 0.003, 0.007, 0.01, 0.03]):
    ...
    df['net_sharpe'] = df['net_annual_return'] / (df['annual_return'] / df['sharpe'])  # 近似计算
    
    print("=== 基础净收益分析 ===")
    print(f"平均年化成本: {df['annual_turnover_cost'].mean():.3f} ({df['annual_turnover_cost'].mean()*100:.2f}%)")
    print(f"成本范围: {df['annual_turnover_cost'].min():.3f} - {df['annual_turnover_cost'].max():.3f}")
    print(f"平均净收益损失: {(df['annual_return'] - df['net_annual_return']).mean():.3f}")
...
def compare_rankings(df, original_top_n=50):
    ...
    print(f"毛收益前{original_top_n}名中，有{dropped_out}个掉出净收益前{original_top_n}")
    print(f"净收益前{original_top_n}名中，有{new_comers}个新进入者")
    
    # 具体分析掉出者
    if dropped_out > 0:
        dropped_combos = list(gross_top50 - net_top50)
        dropped_df = df.loc[dropped_combos, ['combo_idx', 'sharpe', 'net_sharpe_beta_0.007', 
                                           'annual_turnover_cost', 'gross_rank', 'net_rank']].head(10)
...
def pareto_analysis(df, beta=0.007, gamma=0.1):
    ...
    """
    # 计算综合得分
    df['composite_score'] = (df['sharpe'] - 
                            beta * df['annual_turnover_cost'] - 
                            gamma * df['concentration'])
...
def generate_cost_sensitivity_report(df):
    ...
    for cost in cost_scenarios:
        annual_cost = df['turnover'].apply(lambda x: calculate_annual_turnover_cost(x, cost))
        ...
        print(f"成本率{cost*100:.1f}%: 最佳策略{best_combo}, 净夏普{best_net_sharpe:.4f}")
...
def main():
    ...
    # 读取数据
    df = pd.read_csv('/Users/zhangshenshen/深度量化0927/strategies/results/top1000_complete_analysis.csv')
    ...
    # 保存结果
    output_file = '/Users/zhangshenshen/深度量化0927/strategies/results/net_return_analysis.csv'
    df.to_csv(output_file, index=False)
    print(f"\n分析结果已保存至: {output_file}")
    
    # 关键发现总结
    print("\n=== 关键发现 ===")
    print(f"1. 平均年化换手成本: {df['annual_turnover_cost'].mean():.3f} ({df['annual_turnover_cost'].mean()*100:.2f}%)")
    print(f"2. 成本对收益影响: 平均降低{(df['annual_return'] - df['net_annual_return']).mean():.3f}收益")
    print(f"3. 最佳毛收益策略: 62774 (夏普{df.loc[df['sharpe'].idxmax(), 'sharpe']:.4f})")
    print(f"4. 最佳净收益策略: {df.loc[df['net_sharpe_beta_0.007'].idxmax(), 'combo_idx']} (净夏普{df['net_sharpe_beta_0.007'].max():.4f})")
    print(f"5. 排名变化: 前50名中{len(set(df.nsmallest(50, 'gross_rank').index) - set(df.nsmallest(50, 'net_rank').index))}个策略位置变化")
