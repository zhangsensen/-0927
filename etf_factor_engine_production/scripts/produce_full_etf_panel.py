...
class FullPanelProducer:
    ...
    def calculate_all_factors(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 使用VBT适配器（支持vbt/pandas_ta/talib全量指标）
        ...
        logger.info("✅ 加载VBT适配器（300-500个指标：vbt+talib+自定义）")
...
def main():
    ...
    producer = FullPanelProducer(
        data_dir=args.data_dir, output_dir=args.output_dir, diagnose_mode=args.diagnose
    )
