...
# 导入增强的ETF横截面系统组件
from factor_system.factor_engine.factors.etf_cross_section.etf_cross_section_enhanced import (
    ETFCrossSectionFactorsEnhanced,
)
from factor_system.factor_engine.factors.etf_cross_section.etf_cross_section_storage_enhanced import (
    ETFCrossSectionStorageEnhanced,
)
from factor_system.factor_engine.factors.etf_cross_section.etf_cross_section_strategy_enhanced import (
    ...
    ETFCrossSectionStrategyEnhanced,
)
...
class ETFCrossSectionEnhancedDemo:
    ...
    def run_comprehensive_demo(
        self, start_date: str = "2024-01-01", end_date: str = "2025-10-14"
    ) -> Dict[str, Any]:
        """
        运行完整演示
        ...
        try:
            ...
            if not enhanced_factors.empty:
                ...
                logger.info(
                    f"包含的因子列: {[col for col in enhanced_factors.columns if col not in ['date', 'etf_code']][:10]}..."
                )

                # 保存因子数据
                save_success = self._save_demo_factors(
                    enhanced_factors, demo_etfs, start_date, end_date
                )
    ...
    def _test_system_initialization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "overall_status": all(
                    [data_manager_status, storage_status, calculator_status]
                ),
            }
    ...
    def _test_factor_registration(self) -> Dict[str, Any]:
        ...
        try:
            # 获取因子注册表
            from factor_system.factor_engine.factors.etf_cross_section.factor_registry import (
                get_factor_registry,
            )
    ...
    def _test_enhanced_storage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_save = self.storage.save_factor_cache(
                "test_cache", test_data, ttl_hours=1
            )
    ...
    def _run_strategy_backtest(
        self, start_date: str, end_date: str, etf_codes: List[str]
    ) -> Dict[str, Any]:
        """运行策略回测"""
        try:
            # 创建增强策略配置
    ...
    def _save_demo_factors(
        ...
        try:
            ...
            dynamic_factor_cols = [
                col
                for col in factors_data.columns
                if col not in ["date", "etf_code"]
                and col
                not in [
                    "momentum_21d",
                    "momentum_63d",
        ...
        if basic_results.get("initialization", {}).get("overall_status", False):
            ...
            if "system_statistics" in comprehensive_results:
                ...
                if "demo_summary" in stats:
                    ...
                    logger.info(
                        f"   • 动态因子数: {summary.get('total_dynamic_factors', 0)}"
                    )
                    logger.info(
                        f"   • 因子类别数: {summary.get('factor_categories', 0)}"
                    )
