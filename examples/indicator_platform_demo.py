...
# 导入新的指标平台组件
from factor_system.factor_engine.adapters.smart_indicator_adapter import SmartIndicatorAdapter
from factor_system.factor_engine.adapters.vbt_adapter_optimized import VBTIndicatorAdapterOptimized
from factor_system.factor_engine.prescreening.indicator_prescreener import IndicatorPrescreener
...
class IndicatorPlatformDemo:
    ...
    def create_sample_data(self, n_days: int = 500, n_etfs: int = 5) -> pd.DataFrame:
        ...
        for etf_id in range(n_etfs):
            ...
            for i, date in enumerate(dates):
                ...
                # 成交量（与价格变动相关）
                volume = int(np.random.uniform(1000000, 5000000) * (1 + abs(returns[i]) * 10))
    ...
    def generate_target_variable(self, price_series: pd.Series, forward_days: int = 5) -> pd.Series:
        """生成目标变量（未来收益率）"""
        # 未来n日收益率
    ...
    def demonstrate_optimization(self, data: pd.DataFrame):
        ...
        print(f"  总体精简比例: {(smart_result.shape[1] - final_count) / smart_result.shape[1]:.1%}")
        ...
        return {
            ...
            'total_reduction': (traditional.shape[1] - final_count) / traditional.shape[1],
...
def main():
    ...
    print(f"\n🎯 核心优化点:")
    print(f"1. 参数空间压缩: {optimization_result['traditional_count']} → {optimization_result['smart_count']} (减少{(optimization_result['traditional_count'] - optimization_result['smart_count']) / optimization_result['traditional_count']:.1%})")
    print(f"2. 质量预筛选: {optimization_result['smart_count']} → {optimization_result['final_count']} (再减少{(optimization_result['smart_count'] - optimization_result['final_count']) / optimization_result['smart_count']:.1%})")
