...
class FactorStabilityMetrics:
    ...
    def __repr__(self):
        return (f"FactorStability(factor={self.factor_name}, "
...
class FactorStabilityTester:
    ...
    def test_factor_stability(
        ...
        # 判断稳定性
        is_stable = (
            ic_mean >= self.ic_mean_threshold and
            ic_std <= self.ic_std_threshold and
            positive_rate >= self.positive_rate_threshold
        )
    ...
    def test_all_factors(
        ...
        for factor_name, factor_values in factors_dict.items():
            ...
            logger.info(f"  {factor_name:40} | IC={metrics.ic_mean:+.4f}±{metrics.ic_std:.4f} | "
        ...
        logger.info(f"\n✅ 稳定性测试完成:")
        logger.info(f"  稳定因子: {stable_count} / {len(metrics_list)} ({stable_count/len(metrics_list):.1%})")
        ...
        for metrics in metrics_list:
            if metrics.is_stable:
                if metrics.factor_name in factors_dict:
                    stable_factors[metrics.factor_name] = factors_dict[metrics.factor_name]
        
        logger.info(f"📊 过滤结果: {len(stable_factors)} / {len(factors_dict)} 因子通过稳定性测试")
