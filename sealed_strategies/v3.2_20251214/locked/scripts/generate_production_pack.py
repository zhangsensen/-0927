...
def main():
    ...
    parser.add_argument("--candidates", type=str, required=True, help="Path to final_candidates.parquet")
    parser.add_argument("--bt-results", type=str, required=True, help="Path to bt_results.parquet")
    parser.add_argument("--top-n", type=int, default=20, help="Number of top strategies to include")
    ...
    merged["diff_train_return_vec_vs_bt"] = (merged["vec_return"] - merged["bt_train_return"]).abs()
    merged["diff_holdout_return_vec_vs_bt"] = (merged["holdout_return"] - merged["bt_holdout_return"]).abs()
    ...
    r_mdd = _percentile_rank(-merged["bt_max_drawdown"])  # smaller drawdown => higher rank
    ...
    merged = merged.sort_values(["prod_score_bt", "composite_score"], ascending=[False, False])
    ...
    with open(report_path, "w") as f:
        ...
        f.write("**Source**: Leakage-Controlled Triple Validation (screening) + Backtrader Audit (ground truth)\n\n")
        
        f.write("## 1. Executive Summary\n")
        f.write("This pack is intended to be *audit-grade* (no ambiguous comparisons):\n")
        f.write("1.  **Screening (VEC + Rolling + Holdout)**: Candidate discovery + stability filtering (leakage-controlled).\n")
        f.write("2.  **Audit (BT Ground Truth)**: Final production metrics use BT-split returns (train/holdout) and BT risk stats.\n\n")
        f.write("**Key Principle**: When VEC and BT differ due to execution assumptions, BT wins.\n\n")
        ...
        for c in ["BT Train Ret", "BT Holdout Ret", "BT Total Ret", "BT MDD", "BT WinRate", "Roll Worst"]:
            display_df[c] = display_df[c].apply(lambda x: f"{x:.2%}")
        display_df["BT Calmar"] = display_df["BT Calmar"].apply(lambda x: f"{x:.2f}")
        display_df["ProfitFactor"] = display_df["ProfitFactor"].apply(lambda x: f"{x:.2f}")
        ...
        f.write("- **Ground Truth**: All production returns shown in this report are from BT (event-driven).\n")
        f.write("- **VEC vs BT**: VEC is used for fast screening; it may diverge from BT due to execution modeling differences.\n")
        f.write("- **Leakage Control**: Rolling stability metrics are train-only (no holdout segment mixed).\n")

        f.write("\n## 4. Risk Disclosures\n")
        f.write("- **Holdout Length**: 2025-05 to 2025-10 is short; monitor live drawdowns and regime shifts.\n")
        f.write("- **QDII Exposure**: Many top strategies use QDII assets; US/HK regime shifts can impact OOS.\n")
        f.write("- **Execution**: BT assumes execution at bar prices; real slippage/taxes may reduce performance.\n")
        
        f.write("\n## 5. Implementation Guide\n")
        f.write("1.  **Select**: Choose 1-3 strategies (or a small basket) from the top list.\n")
        f.write("2.  **Monitor**: Track drawdown vs BT expectation and stop deploying if behavior breaks.\n")
        f.write("3.  **Rebalance**: Follow the fixed rule: FREQ=3 trading days, POS=2, no stop-loss.\n")
