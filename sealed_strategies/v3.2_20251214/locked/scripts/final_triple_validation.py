...
# === CONFIGURATION ===
# Input Paths (Hardcoded based on previous context)
PATH_VEC = Path("results/vec_from_wfo_20251214_000418/full_space_results.parquet")
PATH_HOLDOUT = Path("results/holdout_validation_20251214_002747/holdout_validation_results.parquet")
# IMPORTANT: train-only rolling summary (end_date=training_end_date) to avoid holdout leakage into the gate.
PATH_ROLLING = Path("results/rolling_oos_consistency_20251214_011500/rolling_oos_summary.parquet")
...
def load_data(path_vec: Path, path_holdout: Path, path_rolling: Path):
    ...
    holdout_cols_to_keep = ["combo"] + [c for c in holdout.columns if c.startswith("holdout_")]
...
def main():
    parser = argparse.ArgumentParser(description="Final triple validation (Train VEC + Train-only rolling + Holdout).")
    ...
    prev_final_path = Path(args.prev_final_candidates) if args.prev_final_candidates else None
    ...
    print(f"Passed Risk Filter (exclude {sorted(EXCLUDE_FACTORS)}): {len(passed_risk)} / {total_count} ({len(passed_risk)/total_count:.2%})")
    ...
    print(f"Passed Holdout Gate (Return > {GATE_HOLDOUT_MIN_RET}): {len(final_candidates)} / {len(passed_rolling)} ({len(final_candidates)/len(passed_rolling):.2%})")
    ...
    final_candidates["score_train"] = final_candidates["vec_calmar_ratio"].rank(pct=True)
    final_candidates["score_roll"] = final_candidates["roll_all_segment_worst_return"].rank(pct=True)
    final_candidates["score_holdout"] = final_candidates["holdout_calmar_ratio"].rank(pct=True)
    ...
    with open(report_path, "w") as f:
        ...
        f.write("This run uses **train-only** rolling OOS summary as the gate input (no holdout segments).\n\n")
        ...
        f.write(f"- **Rolling Consistency Gate** (Strict): {len(passed_rolling)} (PosRate>={GATE_ROLLING_POS_RATE}, Worst>={GATE_ROLLING_WORST_RET}, Calmar>={GATE_ROLLING_CALMAR})\n")
        f.write(f"- **Holdout Gate** (Profitable): {len(final_candidates)} (Return > {GATE_HOLDOUT_MIN_RET})\n\n")
        
        f.write("## 2. Top 20 'Gold Standard' Strategies\n")
        f.write("Sorted by Composite Score (30% Train Calmar + 40% Roll Worst + 30% Holdout Calmar)\n\n")
        
        cols_to_show = [
            "combo", 
            "vec_return", "vec_calmar_ratio",  # Train
            "roll_all_segment_positive_rate", "roll_all_segment_worst_return", # Rolling
            "holdout_return", "holdout_calmar_ratio" # Holdout
        ]
        
        # Format for markdown
        display_df = final_candidates[cols_to_show].head(20).copy()
        display_df.columns = ["Combo", "Train Ret", "Train Calmar", "Roll Win%", "Roll Worst", "Holdout Ret", "Holdout Calmar"]
        
        # Rounding
        for c in ["Train Ret", "Train Calmar", "Roll Worst", "Holdout Ret", "Holdout Calmar"]:
        ...
        if len(final_candidates) > 0:
            ...
            f.write(f"- **Train**: {top['vec_return']:.2%} return, {top['vec_calmar_ratio']:.2f} Calmar\n")
            f.write(f"- **Stability**: {top['roll_all_segment_positive_rate']:.0%} quarterly win rate, worst quarter {top['roll_all_segment_worst_return']:.2%}\n")
            f.write(f"- **Holdout**: {top['holdout_return']:.2%} return in unseen data\n")
            f.write("\nThis is a **leakage-controlled** triple validation (rolling gate uses train-only data; holdout remains unseen).\n")
        else:
            f.write("No strategies passed all gates. The criteria might be too strict, or the strategy pool needs expansion.\n")
        ...
        if prev_final is not None:
            ...
            f.write(f"- Overlap: {len(inter)} ({(len(inter)/max(1, len(prev_set))):.2%} of prev; {(len(inter)/max(1, len(new_set))):.2%} of new)\n")
