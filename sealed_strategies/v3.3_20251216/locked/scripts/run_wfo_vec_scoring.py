...
def run_vec_on_window(combo_indices, data, config, start_idx, end_idx):
    ...
    # Slice data
    factors_slice = data["all_factors_stack"][slice_start:slice_end, :, :][
        ..., list(combo_indices)
    ]
    ...
    atr_slice = (
        data["atr_arr"][slice_start:slice_end]
        if data.get("atr_arr") is not None
        else None
    )
    ...
    eq_curve, *_metrics = run_vec_backtest(
        ...
        dynamic_leverage_enabled=config["backtest"]["risk_control"]["dynamic_leverage"][
            "enabled"
        ],
        vol_regime_arr=vol_slice,
        use_atr_stop=config["backtest"]["risk_control"].get("stop_method", "fixed")
        == "atr",
        trailing_stop_pct=config["backtest"]["risk_control"].get(
            "trailing_stop_pct", 0.0
        ),
        atr_arr=atr_slice,
        atr_multiplier=config["backtest"]["risk_control"].get("atr_multiplier", 3.0),
        stop_on_rebalance_only=config["backtest"]["risk_control"].get(
            "stop_check_on_rebalance_only", False
        ),
        individual_trend_arr=None,
        individual_trend_enabled=config["backtest"]["timing"]
        .get("individual_timing", {})
        .get("enabled", False),
        profit_ladders=config["backtest"]["risk_control"].get("profit_ladders", []),
        circuit_breaker_day=config["backtest"]["risk_control"]
        .get("circuit_breaker", {})
        .get("max_drawdown_day", 0.0),
        circuit_breaker_total=config["backtest"]["risk_control"]
        .get("circuit_breaker", {})
        .get("max_drawdown_total", 0.0),
        circuit_recovery_days=config["backtest"]["risk_control"]
        .get("circuit_breaker", {})
        .get("recovery_days", 5),
...
def main():
    ...
    optimizer = ComboWFOOptimizer(
        ...
        complexity_penalty_lambda=config["combo_wfo"]
        .get("scoring", {})
        .get("complexity_penalty_lambda", 0.0),
    ...
    weight_cfg = config["combo_wfo"]["scoring"].get(
        "vec_score_weights", {"ann_ret": 0.4, "sharpe": 0.3, "max_dd": 0.3}
    )
    ...
    # Ranking per window
    df_window["ann_rank"] = df_window.groupby("window")["ann_ret"].rank(
        ascending=False, method="average"
    )
    df_window["sharpe_rank"] = df_window.groupby("window")["sharpe"].rank(
        ascending=False, method="average"
    )
    df_window["dd_rank"] = df_window.groupby("window")["max_dd"].rank(
        ascending=True, method="average"
    )
    df_window["score"] = (
        w_ann * df_window["ann_rank"]
        + w_sharpe * df_window["sharpe_rank"]
        + w_dd * df_window["dd_rank"]
    )

    agg = (
        df_window.groupby("combo_id")["score"]
        .mean()
        .reset_index()
        .rename(columns={"score": "wfo_vec_score"})
    )
