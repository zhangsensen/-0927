...
def filter_risky_combos(
    df: pd.DataFrame, risky_factors: list = None, banned_combos: list = None
) -> pd.DataFrame:
    """过滤包含高风险因子的组合
...
def apply_ic_threshold(merged: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    mask = (merged["mean_oos_ic"] > ic_threshold) | (
        merged["positive_rate"] > pr_threshold
    )
    ...
    print(
        f"   通过: {len(passed)} / {len(merged)} ({len(passed)/len(merged)*100:.1f}%)"
    )
...
def apply_rolling_consistency_filter(
    ...
    *,
    ...
    missing = (
        merged["all_segment_positive_rate"].isna().sum()
        if "all_segment_positive_rate" in merged.columns
        else len(merged)
    )
    if missing:
    ...
    if "holdout_worst_ret_min" in thresholds:
        mask = mask & (
            merged["holdout_segment_worst_return"]
            >= thresholds["holdout_worst_ret_min"]
        )
    ...
    print(
        ...
        % (
            ...
            (
                ""
                if "holdout_worst_ret_min" not in thresholds
                else f", holdout_worst_ret>={thresholds['holdout_worst_ret_min']:.2f}"
            ),
        )
    )
    print(
        f"   通过: {len(passed)} / {len(merged)} ({len(passed)/max(len(merged),1)*100:.2f}%)"
    )
    return passed
...
def _build_oos_windows(config: dict) -> list[pd.Interval]:
    ...
    while idx + oos_period <= len(dates):
        ...
        if len(oos_slice) == 0:
            break
        windows.append(
            pd.Interval(left=oos_slice[0], right=oos_slice[-1], closed="both")
        )
        idx += step_size
...
def apply_regime_filter(merged: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    recent_idx = [
        ...
        if (w.left >= RECENT_START and w.right <= RECENT_END)
    ]
    if not recent_idx:
        print(
            "⚠️ 未找到位于 2025-01-01~2025-05-31 的 OOS 窗口，跳过 Regime Fitness Filter"
        )
        merged["Recent_Ret_5M"] = np.nan
        merged["Recent_MDD_5M"] = np.nan
        return merged
    ...
    def compute_recent_metrics(oos_returns) -> tuple[float, float]:
        ...
        if isinstance(oos_returns, str):
            oos_returns = np.fromstring(
                oos_returns.replace("[", " ").replace("]", " "), sep=" "
            )
    ...
    # 硬性门槛
    filtered = merged[
        (merged["Recent_Ret_5M"] >= 0.0) & (merged["Recent_MDD_5M"] <= 0.08)
    ].copy()
    ...
    print(
        f"   通过: {len(filtered)} / {len(merged)} ({len(filtered)/len(merged)*100:.1f}%)"
    )
...
def compute_composite_score(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    df["dd_rank"] = df["vec_max_drawdown"].rank(
...
def display_top_strategies(df: pd.DataFrame, top_n: int = 20):
    ...
    if has_oos_return:
        print(
            f"{'排名':^4} | {'OOS收益':^8} | {'VEC收益':^8} | {'Sharpe':^7} | {'MaxDD':^7} | {'IC':^7} | {'PR':^6} | {'得分':^6} | 组合"
        )
        print("-" * 120)

        for rank, (_, row) in enumerate(sorted_df.head(top_n).iterrows(), 1):
            combo_display = (
                row["combo"][:40] + "..." if len(row["combo"]) > 40 else row["combo"]
            )
            print(
                f"{rank:4d} | {row['mean_oos_return']:>7.2%} | {row['vec_return']:>7.2%} | {row['vec_sharpe_ratio']:>7.3f} | "
        ...
        print(
            f"{'排名':^4} | {'收益':^8} | {'Sharpe':^7} | {'MaxDD':^7} | {'Calmar':^7} | {'IC':^7} | {'PR':^6} | {'得分':^6} | 组合"
        )
        print("-" * 120)
        ...
        for rank, (_, row) in enumerate(sorted_df.head(top_n).iterrows(), 1):
            ...
            print(
                f"{rank:4d} | {row['vec_return']:>7.2%} | {row['vec_sharpe_ratio']:>7.3f} | "
...
def main():
    ...
    print(f"   止损: 禁用")
    print(
        f"   择时: {config.get('backtest', {}).get('timing', {}).get('type', 'light_timing')}"
    )
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            f"- positive_rate 门槛: > {config['selection']['positive_rate_threshold']*100:.0f}%\n"
        )
        f.write(
            f"- 综合得分权重: 收益{config['selection']['composite_weights']['return']*100:.0f}%, "
        )
        f.write(
            f"Sharpe{config['selection']['composite_weights']['sharpe']*100:.0f}%, "
        )
        f.write(
            f"回撤{config['selection']['composite_weights']['drawdown']*100:.0f}%\n\n"
        )
        ...
        f.write(
            f"- 过滤比例: {(len(merged) - len(scored)) / len(merged) * 100:.1f}%\n\n"
        )
        ...
        for rank, (_, row) in enumerate(sorted_df.head(10).iterrows(), 1):
            f.write(
                f"| {rank} | {row['vec_return']:.2%} | {row['vec_sharpe_ratio']:.3f} | "
            )
            f.write(
                f"{row['vec_max_drawdown']:.1%} | {row['mean_oos_ic']:.4f} | {row['combo']} |\n"
            )
