...
class PolicyDefenseValidator(Top1Validator):
    ...
    def apply_defense_layer(self, factors_v0):
        ...
        for i in range(5):
            ...
            factors_v1[:, :, i][
                mask_hard_val
            ] = -999.0  # Set to very low score instead of 0 to ensure not picked
    ...
    def analyze_specific_dates(self, equity_v0, equity_v1, mask_hard):
        ...
        if date_str in self.dates:
            ...
            filtered_tickers = mask_hard.loc[date_str][
                mask_hard.loc[date_str]
            ].index.tolist()
            print(f"📅 {date_str} (史诗级高开低走):")
            print(f"   V0 当日收益: {get_ret(equity_v0, date_str):.2%}")
            print(f"   V1 当日收益: {get_ret(equity_v1, date_str):.2%}")
            print(
                f"   触发过滤的ETF ({len(filtered_tickers)}只): {', '.join(filtered_tickers[:5])}..."
            )
            ...
            for t in ["512480", "512690"]:
                if t in self.tickers:
                    ...
                    if t == "512480":
                        ...
                        print(
                            f"   DEBUG 512480: Open={op}, Close={cl}, PrevClose={prev_cl}, Gap={gap:.2f}%, IsFade={cl<op}"
                        )
