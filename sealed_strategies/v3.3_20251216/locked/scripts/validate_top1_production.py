...
def main():
    ...
    std_factors = processor.process_all_factors(
        {fname: raw_factors_df[fname] for fname in factor_names_list}
    )
    ...
    print(f"|---|---|---|---|")
    print(
        f"| Ann Ret | {metrics_v0['ann_ret']:.1%} | {metrics_v3['ann_ret']:.1%} | {(metrics_v3['ann_ret'] - metrics_v0['ann_ret']):.1%} |"
    )
    print(
        f"| Max DD | {metrics_v0['max_dd']:.1%} | {metrics_v3['max_dd']:.1%} | {(metrics_v3['max_dd'] - metrics_v0['max_dd']):.1%} |"
    )
    print(
        f"| Sharpe | {metrics_v0['sharpe']:.2f} | {metrics_v3['sharpe']:.2f} | {(metrics_v3['sharpe'] - metrics_v0['sharpe']):.2f} |"
    )
    print(
        f"| 2022 Ret | {metrics_v0['yearly_rets'][2022]:.1%} | {metrics_v3['yearly_rets'][2022]:.1%} | - |"
    )
    print(
        f"| 2024 Ret | {metrics_v0['yearly_rets'][2024]:.1%} | {metrics_v3['yearly_rets'][2024]:.1%} | - |"
    )
    print(
        f"| Oct 8 DD | {metrics_v0['dd_oct8']:.1%} | {metrics_v3['dd_oct8']:.1%} | - |"
    )
    ...
    print(f"|---|---|---|---|")
    print(
        f"| 7bp | {m_7bp['ann_ret']:.1%} | {m_7bp['max_dd']:.1%} | {m_7bp['sharpe']:.2f} |"
    )
    print(
        f"| 12bp | {metrics_v3['ann_ret']:.1%} | {metrics_v3['max_dd']:.1%} | {metrics_v3['sharpe']:.2f} |"
    )
    print(
        f"| 15bp | {m_15bp['ann_ret']:.1%} | {m_15bp['max_dd']:.1%} | {m_15bp['sharpe']:.2f} |"
    )
    ...
    alternatives = (
        candidates[candidates["similarity"] < 0.5]
    ...
    print(
        alternatives[
            ["combo", "ann_return", "max_dd", "sharpe", "similarity"]
        ].to_markdown()
    )
