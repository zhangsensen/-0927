...
def main() -> None:
    ...
    md_lines.append(
        df["all_segment_count"].value_counts().head(10).to_frame("count").to_markdown()
    )
    md_lines.append("\n\n- holdout_segment_count (value_counts):\n")
    md_lines.append(
        df["holdout_segment_count"]
        .value_counts()
        .head(10)
        .to_frame("count")
        .to_markdown()
    ...
    for preset_name, gate in PRESETS.items():
        ...
        md_lines.append(
            ...
            + (
                ""
                if gate.holdout_worst_ret_min is None
                else f", holdout_worst_ret>={gate.holdout_worst_ret_min:.2f}"
            )
            + "\n"
