...
def load_data():
    ...
    df_train_top = pd.read_csv(
        ROOT / "results/vec_from_wfo_20251211_205649/top1000_composite.csv"
    )
...
def analyze_score_distribution(df):
    ...
    spearman_corr, p_value = stats.spearmanr(
        df["train_composite"], df["hold_composite"]
    )
...
def analyze_top_cohorts(df):
    ...
    # 按训练期得分排序
    df_sorted = df.sort_values("train_composite", ascending=False).reset_index(
        drop=True
    )
    ...
    for name, n in cohorts:
        ...
        print(
            f"{name:12} {n:6d} {hold_ret_mean:+11.4f} {hold_ret_median:+11.4f} {positive_pct:9.2%} {hold_comp_mean:+9.4f}"
        )
...
def analyze_factor_shift(df):
    ...
    for factor, train_pct, hold_pct, diff in diffs:
        if abs(diff) > 0.05:  # 只显示差异>5%的
            ...
            print(
                f"{factor:40} {train_pct:9.1%} {hold_pct:9.1%} {diff:+9.1%} {direction}"
            )
...
def analyze_complexity_impact(df):
    ...
    for size in sorted(df["size_train"].unique()):
        ...
        print(
            f"{size:6d} {len(subset):8d} {train_ret:+11.4f} {hold_ret:+11.4f} {decay:+11.4f} {decay_pct:9.2%}"
        )
...
def find_stable_combos(df, train_pct=0.90, hold_pct=0.70):
    ...
    if len(stable) > 0:
        ...
        print(
            f"{'排名':>4} {'组合':70} {'Hold收益':>10} {'Hold Sharpe':>12} {'Hold MaxDD':>12}"
        )
        print("-" * 110)

        for idx, row in stable.head(20).iterrows():
            print(
                f"{idx+1:4d} {row['combo']:70} {row['hold_return']:+9.4f} {row['hold_sharpe']:11.4f} {row['hold_max_dd']:11.4f}"
            )

        # 保存结果
        output_path = (
            ROOT / "results/vec_from_wfo_20251211_205649/stable_combos_dual_gate.csv"
        )
