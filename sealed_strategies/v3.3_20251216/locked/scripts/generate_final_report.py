...
print(f"\n平均差异:")
print(
    f"  VEC训练 vs BT:     {merged_df['vec_bt_diff'].mean():.2%} (标准差: {merged_df['vec_bt_diff'].std():.2%})"
)
print(
    f"  Holdout vs BT:     {merged_df['holdout_bt_diff'].mean():.2%} (标准差: {merged_df['holdout_bt_diff'].std():.2%})"
)
...
if len(margin_failures) > 0:
    print("\nTop 5 保证金失败策略:")
    print(
        margin_failures[["combo", "bt_margin_failures", "bt_return"]]
        .head()
...
if len(large_diff) > 0:
    print("\nTop 5 差异最大策略:")
    top_diff = large_diff.nlargest(5, "vec_bt_diff")[
        ["combo", "vec_return", "bt_return", "vec_bt_diff"]
    ]
    for _, row in top_diff.iterrows():
        print(
            f"  差异: {row['vec_bt_diff']*100:.1f}% | VEC: {row['vec_return']*100:.1f}% | BT: {row['bt_return']*100:.1f}%"
        )
...
merged_df["final_calmar"] = merged_df[
    ["vec_calmar_ratio", "holdout_calmar_ratio", "bt_calmar_ratio"]
].min(axis=1)
merged_df["final_return"] = merged_df[
    ["vec_return", "holdout_return", "bt_return"]
].min(axis=1)
merged_df["final_mdd"] = merged_df[
    ["vec_max_drawdown", "holdout_max_drawdown", "bt_max_drawdown"]
].max(axis=1)
...
print("=" * 80)
print(
    f"{'排名':<4} | {'最低Calmar':<10} | {'最低收益':<9} | {'最大MDD':<9} | {'因子数':<6} | {'组合'}"
)
print("-" * 80)

for i, (_, row) in enumerate(final_ranking.head(20).iterrows(), 1):
    print(
        f"{i:<4} | {row['final_calmar']:>10.3f} | {row['final_return']*100:>8.1f}% | {row['final_mdd']*100:>8.1f}% | "
...
try:
    ...
    # Holdout vs BT
    axes[0, 1].scatter(
        merged_df["holdout_return"], merged_df["bt_return"], alpha=0.5, s=20
    )
    axes[0, 1].plot(
        [0, merged_df["holdout_return"].max()],
        [0, merged_df["holdout_return"].max()],
        "r--",
        lw=1,
    )
    ...
    # Calmar 对比
    axes[1, 0].scatter(
        merged_df["vec_calmar_ratio"], merged_df["bt_calmar_ratio"], alpha=0.5, s=20
    )
    max_calmar = max(
        merged_df["vec_calmar_ratio"].max(), merged_df["bt_calmar_ratio"].max()
    )
    ...
    top20_for_plot["short_name"] = (
        top20_for_plot["combo"].str.split(" + ").str[0] + "..."
    )
    x = range(len(top20_for_plot))
    axes[1, 1].barh(x, top20_for_plot["final_calmar"], alpha=0.7)
    axes[1, 1].set_yticks(x)
    axes[1, 1].set_yticklabels(
        [f"#{i+1}" for i in range(len(top20_for_plot))], fontsize=8
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "final_analysis_comparison.png", dpi=150, bbox_inches="tight"
    )
