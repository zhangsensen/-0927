...
def main():
    ...
    for w in windows:
        train_mask = (daily_returns_df.index >= w["train_start"]) & (
            daily_returns_df.index <= w["train_end"]
        )
        test_mask = (daily_returns_df.index >= w["test_start"]) & (
            daily_returns_df.index <= w["test_end"]
        )
        ...
        train_stats_df = pd.DataFrame(train_stats).sort_values(
    ...
    if oos_returns:
        ...
        full_oos_ret = full_oos_ret[
            ~full_oos_ret.index.duplicated(keep="first")
        ].sort_index()
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            top_singles[
                ["combo", "ann_return", "sharpe", "max_dd", "ret_2022", "ret_2025"]
        ...
        f.write(
            pairs_df.head(5)[
                ["combo_a", "combo_b", "correlation", "ann_return", "sharpe", "max_dd"]
            ].to_markdown(index=False, floatfmt=".2%")
        )
        f.write("\n\n")
