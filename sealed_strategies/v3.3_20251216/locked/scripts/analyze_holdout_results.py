...
def main() -> None:
    ...
    parser.add_argument(
        "--output", default=None, help="output markdown path (default next to input)"
    )
    parser.add_argument(
        "--top", type=int, default=30, help="rows to include in each shortlist"
    )
    ...
    out_path = (
        Path(args.output) if args.output else input_path.with_name("holdout_report.md")
    )
    ...
    df_conservative = df[df["holdout_max_drawdown"] <= 0.12].sort_values(
        "calmar_ratio_stability", ascending=False
    )
    ...
    lines.append(
        f"- Holdout positive return: {n_pos_hold} ({n_pos_hold / max(n_total, 1) * 100:.1f}%)\n"
    )
