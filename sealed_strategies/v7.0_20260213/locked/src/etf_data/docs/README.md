...
```bash
# 查看ETF清单
uv run python src/etf_data/scripts/download_etf_manager.py --action list

# 下载核心ETF
uv run python src/etf_data/scripts/download_etf_manager.py --action download-core

# 按优先级下载
uv run python src/etf_data/scripts/download_etf_manager.py --action download-priority --priority high

# 下载指定ETF
uv run python src/etf_data/scripts/download_etf_manager.py --action download-specific --etf-codes 510300 510500

# 更新单个ETF
uv run python src/etf_data/scripts/download_etf_manager.py --action update --etf-code 510300

# 验证数据完整性
uv run python src/etf_data/scripts/download_etf_manager.py --action validate

# 使用自定义配置
uv run python src/etf_data/scripts/download_etf_manager.py --config custom --config-file my_config.yaml
```

### 方式3: 交互式批量下载

```bash
uv run python src/etf_data/scripts/batch_download.py
```
...
```bash
# 使用快速配置
uv run python src/etf_data/scripts/download_etf_manager.py --config quick

# 使用完整配置
uv run python src/etf_data/scripts/download_etf_manager.py --config full
```
...
```python
from etf_data import ETFDownloadManager, ETFConfig, ETFListManager
