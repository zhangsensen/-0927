...
def compute_dual_gate_raw(
    ...
    *,
    ...
    thresholds_pct: Sequence[float] = (25, 30, 40),
    exposures_uptrend: Sequence[float] = (1.0, 0.7, 0.5, 0.3),
    exposures_downtrend: Sequence[float] = (0.7, 0.4, 0.2, 0.1),
) -> pd.Series:
    """Compute raw (unshifted) dual-dimension (trend + volatility) exposure gate.

    Combines trend detection (SMA cross) with volatility percentile to produce
    a 2D lookup:

        - Uptrend (SMA_short >= SMA_long): use exposures_uptrend
        - Downtrend (SMA_short < SMA_long): use exposures_downtrend

    Within each trend state, volatility thresholds determine the exposure level
    (same logic as compute_volatility_gate_raw).

    Parameters
    ----------
    close_df : DataFrame
        Close prices, columns = ETF codes.
    proxy_symbol : str
        Market proxy for vol and trend computation.
    vol_window : int
        Window for realized volatility.
    trend_short_window : int
        Short SMA window for trend detection.
    trend_long_window : int
        Long SMA window for trend detection.
    thresholds_pct : sequence of float
        Volatility percentile thresholds (same as volatility mode).
    exposures_uptrend : sequence of float
        Exposure levels for uptrend: len = len(thresholds_pct) + 1.
    exposures_downtrend : sequence of float
        Exposure levels for downtrend: len = len(thresholds_pct) + 1.

    Returns
    -------
    pd.Series
        Exposure values in [0, 1], indexed by close_df.index.
    """
    _validate_thresholds_and_exposures(thresholds_pct, exposures_uptrend)
    _validate_thresholds_and_exposures(thresholds_pct, exposures_downtrend)

    proxy_close = _resolve_market_proxy(close_df, proxy_symbol)
    rets = proxy_close.pct_change()

    # Volatility (same as volatility mode)
    hv = rets.rolling(window=vol_window, min_periods=vol_window).std() * np.sqrt(252) * 100
    hv_5d = hv.shift(5)
    regime_vol = (hv + hv_5d) / 2

    # Trend detection: SMA cross
    sma_short = proxy_close.rolling(window=trend_short_window, min_periods=trend_short_window).mean()
    sma_long = proxy_close.rolling(window=trend_long_window, min_periods=trend_long_window).mean()
    is_uptrend = (sma_short >= sma_long).values  # bool array

    # Build exposure array
    vol_vals = regime_vol.values.astype(np.float64)
    exp = np.full(len(vol_vals), np.nan, dtype=np.float64)

    for i in range(len(vol_vals)):
        if is_uptrend[i]:
            exps = exposures_uptrend
        else:
            exps = exposures_downtrend

        # Default: first exposure (lowest vol bucket)
        exp[i] = float(exps[0])
        for j, thr in enumerate(thresholds_pct):
            if vol_vals[i] >= float(thr):
                exp[i] = float(exps[j + 1])

    exp_s = pd.Series(exp, index=close_df.index)
    # Fill NaN from warmup period with default uptrend exposure
    exp_s = exp_s.fillna(float(exposures_uptrend[0]))
    return exp_s
...
def compute_regime_gate_arr(
    ...
    *,
    ...
    if mode == "volatility":
        ...
        dual_cfg = gate_cfg.get("dual") or {}
        proxy_symbol = str(dual_cfg.get("proxy_symbol", "510300"))
        vol_window = int(dual_cfg.get("vol_window", 20))
        trend_short_window = int(dual_cfg.get("trend_short_window", 20))
        trend_long_window = int(dual_cfg.get("trend_long_window", 60))
        shift_days = int(dual_cfg.get("shift_days", 1))
        thresholds_pct = tuple(dual_cfg.get("thresholds_pct", [25, 30, 40]))
        exposures_uptrend = tuple(
            dual_cfg.get("exposures_uptrend", [1.0, 0.7, 0.5, 0.3])
        )
        exposures_downtrend = tuple(
            dual_cfg.get("exposures_downtrend", [0.7, 0.4, 0.2, 0.1])
        )

        raw_gate = compute_dual_gate_raw(
            close_df,
            proxy_symbol=proxy_symbol,
            ...
            thresholds_pct=thresholds_pct,
            exposures_uptrend=exposures_uptrend,
            exposures_downtrend=exposures_downtrend,
        )
        default_fill = float(exposures_uptrend[0])
