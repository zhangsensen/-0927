...
class WFORealBacktestCalibrator:
    ...
    def fit(
        ...
        # 合并WFO和回测数据
        merged = wfo_df.merge(
            backtest_df[["combo", target_metric]], on="combo", how="inner"
        )
        logger.info(
            f"训练集: {len(merged)} 组合 (WFO: {len(wfo_df)}, 回测: {len(backtest_df)})"
        )
        ...
        logger.info(
            f"  训练集RMSE: {train_rmse:.4f}, R²: {train_r2:.4f}, Spearman: {train_corr:+.4f}"
        )
    ...
    def calibrate_and_rerank(
        ...
        # 按校准Sharpe重新排序
        calibrated_top = wfo_df.nlargest(top_k, "calibrated_sharpe").reset_index(
            drop=True
        )

        logger.info(f"✅ WFO结果已校准，TopK={top_k}")
        logger.info(
            f"  校准后Sharpe: min={calibrated_top['calibrated_sharpe'].min():.3f}, "
...
def main():
    ...
    backtest_path = Path(
        "results_combo_wfo/20251108_193712_20251108_195135/"
