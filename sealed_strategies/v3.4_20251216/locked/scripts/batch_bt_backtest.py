...
def process_combo(row_data):
    ...
    # 运行回测
    bt_return, margin_failures, risk_metrics, daily_returns_s, orders, trades = (
        run_bt_backtest(
            combined_score_df,
            timing_series,
            vol_regime_series,
...
def main():
    ...
    # 清理 results 中的 orders/trades 字段以免污染主结果表(太大)
    clean_results = []
    for res in results:
        combo = res["combo"]
        # Orders
        if "bt_orders" in res and res["bt_orders"]:
            for o in res["bt_orders"]:
                o_dict = o.copy()
                o_dict["combo"] = combo
                all_orders.append(o_dict)

        # Trades
        if "bt_trades" in res and res["bt_trades"]:
            for t in res["bt_trades"]:
                t_dict = t.copy()
                t_dict["combo"] = combo
        ...
        # 移除大对象以便保存主结果
        res_clean = {
            k: v for k, v in res.items() if k not in ["bt_orders", "bt_trades"]
        }
    ...
    if all_orders:
        ...
        df_orders.to_csv(output_dir / "bt_orders.csv", index=False)
        df_orders.to_parquet(output_dir / "bt_orders.parquet", index=False)
        print(f"   ✅ 订单详情已保存: {len(df_orders)} 条 -> bt_orders.csv")
    ...
    if all_trades:
        ...
        df_trades.to_csv(output_dir / "bt_trades.csv", index=False)
        df_trades.to_parquet(output_dir / "bt_trades.parquet", index=False)
        print(f"   ✅ 交易盈亏已保存: {len(df_trades)} 条 -> bt_trades.csv")
