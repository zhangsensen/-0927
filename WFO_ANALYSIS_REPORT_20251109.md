...
7. **20251112_170333对齐复盘**：修复时间对齐后，全局Spearman降至0.039，但Top/Bottom桶净年化分别保持在19.1%与8.1%，桶内排序依旧有效。
...
### 5.4 20251112_170333 对齐修复回归

- **运行来源**：`results/run_20251112_170333`（应用 `ComboWFOOptimizer` 时间对齐与权重符号修复）。
- **全局指标**：`analysis_outputs/ic_curve/20251112_170333_20251112_170425/ic_curve_summary.json` 显示 Spearman 0.0387、Pearson -0.0189，相比旧版（0.47/0.55）显著回落。
- **原因拆解**：
   1. **技术层面**：旧版本让 `signal[t]` 直接对应 `returns[t]`，存在一天偷看；调整为 `t → t+1` 后相关度自然回归。
   2. **市场层面**：2024Q4→2025Q3 震荡-反弹交替，真实收益已处历史“倒U”区间右侧，组合的绝对收益在走低，但排序的分层特征仍然稳定。
- **分层验证**（`analysis_outputs/return_deciles/20251112_170333_20251112_170425/return_bucket_summary.json`）：
   - Bottom 桶净年化均值 8.09%，Top 桶净年化均值 19.12%，桶间差值 11pp。
   - 桶内 IC 均值约 0.109，说明“分层有效 + 全局相关度变弱”确实是“倒U + 分层”并存的结构性现象。
- **结论**：全局Spearman回落属于对齐修复后的正常化表现，配合市场进入“倒U”尾段导致 rank ↔ return 的整体相关度被稀释；然而分桶仍能维持显著差距，可继续作为校准器与上线闸门的业务基线。

### 5.5 校准器扩展与门控策略

- **模型版本**：`results/calibrator_gbdt_full.joblib`（2025-11-12），基于 run `20251112_170333` + Top2000 真实回测训练。
- **特征清单**：对齐 `features/wfo_serving_features.txt`，涵盖四大类：
  1. **OOS时序统计**：`oos_ic_*`、`oos_ir_*`、`pos_rate_*` 系列，覆盖均值/波动/趋势/熵等 60+ 指标。
  2. **组合结构与频率**：`combo_size`、`factor_diversity_score`、`freq_consistency` 等，解析组合因子类型与换仓节奏。
  3. **市场环境适配**：`market_ic_bull/bear`、`market_vol_regime_stability`、`market_alpha_vs_market` 等，与 ETF 大盘 regime 对齐。
  4. **风险与成本代理**：`dd_recovery_ratio`、`turnover_efficiency_rank`、`tail_ratio`、`cost_drag` 等，避免过度换手/尾部风险。
- **门控规则**（写入 `results/calibrator_gbdt_full_eval.json`）：
  - Spearman 校准后需 ≥0.3 且比基线高出至少 0.1（本次增幅 +0.87）。
  - Top100/200/500 平均 Sharpe 相较 IC 基线至少提升 0.05（本次分别 +0.37/+0.35/+0.23）。
  - 若任一条件未通过，`core/combo_wfo_optimizer` 自动回退至 IC 排序。
- **上线判断**：本次门控 `passed=true`，并将校准排序对接 `run_combo_wfo.py`；后续训练若 gating 失败，将保留原始 IC 排序，避免负迁移。
