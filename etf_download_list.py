...
def get_high_priority_etfs():
    """获取高优先级ETF列表"""
    return [etf for etf in get_all_etfs() if etf['priority'] in ['must_have', 'core', 'high']]
...
def get_etf_summary():
    ...
    summary = {
        ...
        'completed_downloads': len([etf for etf in NEW_ETFS + RECOMMENDED_ETFS + OPTIONAL_ETFS if etf.get('download_status') == 'completed']),
        'pending_downloads': len([etf for etf in NEW_ETFS + RECOMMENDED_ETFS + OPTIONAL_ETFS if etf.get('download_status') != 'completed']),
        'categories': {}
    }
...
def print_etf_list_by_priority():
    ...
    for etf in core_etfs:
        ...
        print(f"  {status} {etf['code']} - {etf['name']} ({etf['category']}) - {etf.get('note', '')}")
    ...
    # 推荐配置
    recommended_etfs = [etf for etf in get_all_etfs() if etf['priority'] == 'recommended']
...
if __name__ == "__main__":
    ...
    for etf in must_have:
        status = "✅" if etf.get('file_exists', False) or etf.get('download_status') == 'completed' else "❌"
