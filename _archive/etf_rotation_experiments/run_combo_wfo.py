...
try:
    from apply_ranker import apply_ltr_ranking
...
def _run_backtests(run_dir: Path, ranking_files, topk: int = None, slippage_bps: int = 2):
    ...
    backtest_script = Path(__file__).parent / "real_backtest" / "run_profit_backtest.py"
    ...
    for rf in ranking_files:
        ...
        proc = subprocess.run(cmd, cwd=Path(__file__).parent, capture_output=True, text=True)
        meta = {
