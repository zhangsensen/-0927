...
class LTRRanker:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 可选：如果用户希望使用 GPU 加速 LightGBM（前提是 lightgbm 已编译/安装为 GPU 版本）
        self.use_gpu = bool(use_gpu)
        if self.use_gpu:
            # 常用的 GPU 参数，具体取决于 LightGBM 的构建方式（CUDA / OpenCL）
            # 如果使用 CUDA 编译的 LightGBM，可以设置 `device` 为 `gpu`。
            # 也可设置 `gpu_platform_id` / `gpu_device_id` 以选择设备。
            self.params.update({
                "device": "gpu",
                "gpu_platform_id": 0,
                "gpu_device_id": 0,
