...
def calculate_multi_period_ic(
    panel: pd.DataFrame, price_dir: str, periods: List[int] = [1, 5, 10, 20]
) -> pd.DataFrame:
    """多周期IC分析（完全向量化）"""
    print(f"\n🔬 多周期IC分析: {periods}")
...
def remove_correlated_factors(
    ic_df: pd.DataFrame, panel: pd.DataFrame, max_corr: float = 0.8
) -> pd.DataFrame:
...
def main():
    ...
    parser.add_argument(
        "--min-ic", type=float, default=0.005, help="最小IC (ETF现实标准0.5%)"
    )
    parser.add_argument("--min-ir", type=float, default=0.05, help="最小IR (实用标准)")
    parser.add_argument(
        "--max-pvalue", type=float, default=0.2, help="最大p值 (小样本适用)"
    )
    ...
    # IC分析
    ic_df = calculate_multi_period_ic(
        panel, args.price_dir, [1, 5, 10, args.future_periods]
    )
    ...
    if len(passed) > 0:
        ...
        print(
            passed.head(10)[
                ["factor", "ic_mean", "ic_ir", "ic_positive_rate", "p_value"]
