...
def main():
    ...
    # 支持通过环境变量覆盖配置文件路径（与 real_backtest/run_production_backtest 保持一致风格）
    # 用法示例： RB_CONFIG_FILE=configs/combo_wfo_top1000.yaml python run_combo_wfo.py
    env_cfg = os.environ.get("RB_CONFIG_FILE")
    config_path = Path(env_cfg).expanduser() if env_cfg else Path("configs/combo_wfo_config.yaml")
    if not config_path.exists():
        logger.error(f"配置文件不存在: {config_path} (可通过 RB_CONFIG_FILE 覆盖)")
        sys.exit(1)
    ...
    logger.info(f'  - OOS IC: {summary["best_combo"]["ic"]:.4f}')
