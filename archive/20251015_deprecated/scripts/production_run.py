...
def main():
    ...
    try:
        ...
        all_from_registry = sorted(
            set(list(registry.factors.keys()) + list(registry.metadata.keys()))
        )
    ...
    for timeframe in timeframes:
        ...
        try:
            ...
            if timeframe != "daily":
                categories = {
                    fid: registry.metadata.get(fid, {}).get("category", "")
                    for fid in factor_ids
                }
                current_factor_ids = [
                    fid
                    for fid in factor_ids
                    if not categories.get(fid, "").startswith(
                        ("money_flow", "money_flow_enhanced")
                    )
                    and not fid.lower().startswith(("moneyflow", "money_flow"))
                ]
                logger.info(
                    f"⛳ 过滤资金流因子: {len(factor_ids)} → {len(current_factor_ids)}"
                )
            ...
            with open(report_file, "w", encoding="utf-8") as f:
                ...
                # A股分钟级数据：校验每日K线数
                if timeframe != "daily" and any(
                    s.endswith((".SH", ".SZ")) for s in symbols
                ):
                    expected_bars = {
                        "1min": 241,
                        "5min": 48,
                    ...
                    if expected:
                        ...
                        for symbol in symbols:
                            if not symbol.endswith((".SH", ".SZ")):
                                continue
                            symbol_data = result.xs(symbol, level="symbol")
                            daily_counts = symbol_data.groupby(
                                symbol_data.index.date
                            ).size()
                            avg_bars = daily_counts.mean()
                            status = "✅" if abs(avg_bars - expected) < 0.1 else "❌"
                            f.write(
                                f"| {symbol} | {len(symbol_data):,} | {avg_bars:.1f} | {expected} | {status} |\n"
                            )
