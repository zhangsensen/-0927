...
class QualityDashboard:
    ...
    def generate_html_dashboard(self, days: int = 30) -> str:
        ...
        dashboard_file = (
            self.reports_dir
            / f"quality_dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
    ...
    def _parse_pyscn_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "complexity" in data:
            ...
            metrics["medium_risk_functions"] = len(
        ...
        if "clones" in data:
            ...
            metrics["high_similarity_clones"] = len(
                [c for c in clones if c.get("similarity", 0) > 0.9]
            )
    ...
    def _create_html_dashboard(
        self, trends_data: List[Dict], current_metrics: Dict, days: int
    ) -> str:
        """创建 HTML 仪表板"""
        html = f"""
<!DOCTYPE html>
...
"""
    ...
    def _generate_metric_cards(self, metrics: Dict) -> str:
        ...
        quality_class = (
            "quality-excellent"
            if quality_score >= 80
            else "quality-good" if quality_score >= 60 else "quality-poor"
        )
...
def main():
    ...
    parser.add_argument("--open", action="store_true", help="生成后自动打开仪表板")
