...
def generate_moneyflow_factors(symbol: str, output_dir: Path):
    ...
    print(
        f"     ✅ MainNetInflow_Rate: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ LargeOrder_Ratio: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ SuperLargeOrder_Ratio: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ OrderConcentration: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ MoneyFlow_Hierarchy: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ MoneyFlow_Consensus: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ MainFlow_Momentum: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ Flow_Price_Divergence: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ Institutional_Absorption: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
    ...
    print(
        f"     ✅ Flow_Reversal_Ratio: {valid}/{len(merged)} ({valid/len(merged)*100:.1f}%)"
    )
