...
class TestFactorConsistency:
    ...
    def test_rsi_consistency(self, test_data):
        ...
        # 3. 验证一致性
        if 'RSI14' in gen_result.columns and not engine_result.empty and 'RSI14' in engine_result.columns:
            ...
            if len(gen_rsi_valid) > 0:
                ...
                logger.info(f"  样本数: {len(gen_rsi_valid)}, 覆盖率: {len(gen_rsi_valid)/len(gen_rsi)*100:.1f}%")
    ...
    def test_willr_consistency(self, test_data):
        ...
        # 3. 验证
        if 'WILLR14' in gen_result.columns and not engine_result.empty and 'WILLR14' in engine_result.columns:
    ...
    def test_macd_consistency(self, test_data):
        ...
        if 'MACD_12_26_9' in gen_result.columns and not engine_result.empty and 'MACD_12_26_9' in engine_result.columns:
...
def test_shared_calculator_usage():
    ...
    logger.info(f"✓ {shared_calc_count}/{total_count}个因子使用SHARED_CALCULATORS ({percentage:.1f}%)")
