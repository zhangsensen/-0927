...
class TestFactorEngineConsistency:
    ...
    def factor_generation_calculator(self):
        ...
        # 使用默认配置
        from factor_system.factor_generation.enhanced_factor_calculator import FactorCalculatorConfig
    ...
    def test_rsi_consistency(self, sample_data):
        ...
        try:
            vbt_rsi = get_vectorbt_adapter().calculate_rsi(sample_data['close'], window=14)
            ...
            if valid_mask.sum() > 0:
                # 使用相对误差容忍度
                relative_diff = abs(rsi_engine[valid_mask] - vbt_rsi[valid_mask]) / (abs(vbt_rsi[valid_mask]) + 1e-10)
                max_relative_diff = relative_diff.max()

                print(f"RSI相对误差: {max_relative_diff:.6f}")
                assert max_relative_diff < 1e-10, f"RSI计算不一致，最大相对误差: {max_relative_diff}"
            else:
    ...
    def test_macd_consistency(self, sample_data):
        ...
        try:
            ...
            if valid_mask.sum() > 0:
                relative_diff = abs(macd_engine[valid_mask] - vbt_macd[valid_mask]) / (abs(vbt_macd[valid_mask]) + 1e-10)
                max_relative_diff = relative_diff.max()

                print(f"MACD相对误差: {max_relative_diff:.6f}")
                assert max_relative_diff < 1e-10, f"MACD计算不一致，最大相对误差: {max_relative_diff}"
            else:
    ...
    def test_stoch_consistency(self, sample_data):
        ...
        try:
            ...
            if valid_mask.sum() > 0:
                relative_diff = abs(stoch_engine[valid_mask] - vbt_stoch[valid_mask]) / (abs(vbt_stoch[valid_mask]) + 1e-10)
                max_relative_diff = relative_diff.max()

                print(f"STOCH相对误差: {max_relative_diff:.6f}")
                assert max_relative_diff < 1e-10, f"STOCH计算不一致，最大相对误差: {max_relative_diff}"
            else:
    ...
    def test_sma_consistency(self, sample_data):
        ...
        try:
            vbt_sma = get_vectorbt_adapter().calculate_sma(sample_data['close'], window=20)

            # 比较结果
            valid_mask = ~sma_engine.isna() & ~vbt_sma.isna()

            if valid_mask.sum() > 0:
                relative_diff = abs(sma_engine[valid_mask] - vbt_sma[valid_mask]) / (abs(vbt_sma[valid_mask]) + 1e-10)
                max_relative_diff = relative_diff.max()

                print(f"SMA相对误差: {max_relative_diff:.6f}")
                assert max_relative_diff < 1e-10, f"SMA计算不一致，最大相对误差: {max_relative_diff}"
            else:
    ...
    def test_ema_consistency(self, sample_data):
        ...
        try:
            vbt_ema = get_vectorbt_adapter().calculate_ema(sample_data['close'], window=20)

            # 比较结果
            valid_mask = ~ema_engine.isna() & ~vbt_ema.isna()

            if valid_mask.sum() > 0:
                relative_diff = abs(ema_engine[valid_mask] - vbt_ema[valid_mask]) / (abs(vbt_ema[valid_mask]) + 1e-10)
                max_relative_diff = relative_diff.max()

                print(f"EMA相对误差: {max_relative_diff:.6f}")
                assert max_relative_diff < 1e-10, f"EMA计算不一致，最大相对误差: {max_relative_diff}"
            else:
    ...
    def test_willr_consistency(self, sample_data):
        ...
        try:
            ...
            if valid_mask.sum() > 0:
                relative_diff = abs(willr_engine[valid_mask] - vbt_willr[valid_mask]) / (abs(vbt_willr[valid_mask]) + 1e-10)
                max_relative_diff = relative_diff.max()

                print(f"WILLR相对误差: {max_relative_diff:.6f}")
                assert max_relative_diff < 1e-10, f"WILLR计算不一致，最大相对误差: {max_relative_diff}"
            else:
    ...
    def test_cci_consistency(self, sample_data):
        ...
        try:
            ...
            if valid_mask.sum() > 0:
                relative_diff = abs(cci_engine[valid_mask] - vbt_cci[valid_mask]) / (abs(vbt_cci[valid_mask]) + 1e-10)
                max_relative_diff = relative_diff.max()

                print(f"CCI相对误差: {max_relative_diff:.6f}")
                assert max_relative_diff < 1e-10, f"CCI计算不一致，最大相对误差: {max_relative_diff}"
            else:
            ...
            if factor_id in test_factors_engine.columns:
                try:
                    ...
                    if factor_id == "RSI":
                        ...
                        vbt_result = adapter.calculate_stoch(sample_data['high'], sample_data['low'], sample_data['close'])
                    elif factor_id == "WILLR":
                        vbt_result = adapter.calculate_willr(sample_data['high'], sample_data['low'], sample_data['close'])
                    elif factor_id == "CCI":
                        vbt_result = adapter.calculate_cci(sample_data['high'], sample_data['low'], sample_data['close'])
                    elif factor_id == "SMA":
                    ...
                    if valid_mask.sum() > 0:
                        relative_diff = abs(engine_values[valid_mask] - vbt_result[valid_mask]) / (abs(vbt_result[valid_mask]) + 1e-10)
                        max_relative_diff = relative_diff.max()
    ...
    def test_api_vs_adapter_consistency(self, sample_data):
        ...
        if valid_mask.sum() > 0:
            relative_diff = abs(rsi_api[valid_mask] - rsi_adapter[valid_mask]) / (abs(rsi_adapter[valid_mask]) + 1e-10)
            max_relative_diff = relative_diff.max()

            print(f"API vs 适配器相对误差: {max_relative_diff:.6f}")
            assert max_relative_diff < 1e-12, f"API与适配器不一致，相对误差: {max_relative_diff}"
        else:
