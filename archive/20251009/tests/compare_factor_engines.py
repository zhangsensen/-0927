    ...
    for i, line in enumerate(lines):
        if "factor_calculations.append(" in line:
            ...
            if start != -1:
                ...
                if end != -1:
                    factor_calc = line[
                        start + len("factor_calculations.append((") : end
                    ].strip()
                    if "," in factor_calc:
...
def analyze_calculation_methods():
    ...
    # 读取VectorBT适配器
    adapter_file = Path(
        "/Users/zhangshenshen/深度量化0927/factor_system/factor_engine/core/vectorbt_adapter.py"
    )
...
def generate_comparison_report():
    ...
    report = {
        ...
        "consistency_analysis": {
            ...
            "consistency_ratio": (
                naming_comparison.get("common_count", 0)
                / max(fe_data.get("total_count", 1), 1)
                if fe_data.get("total_count", 0) > 0
                else 0
            ),
        },
    }
    ...
    print(
        f"  5. 仅factor_generation有的因子: {naming_comparison.get('fg_only_count', 0)}个"
    )
...
def main():
    ...
    try:
        ...
        # 保存报告
        report_file = Path(
            "/Users/zhangshenshen/深度量化0927/factor_engine_consistency_report.json"
        )
