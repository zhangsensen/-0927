...
def debug_stoch_result():
    ...
    try:
        ...
        print(
            f"STOCH结果属性: {[attr for attr in dir(result) if not attr.startswith('_')]}"
        )
        ...
        for attr in ["stoch_k", "stoch_d", "slowk", "slowd", "k", "d"]:
            if hasattr(result, attr):
                ...
                print(
                    f"  - {attr}: {type(data)}, shape={getattr(data, 'shape', 'N/A')}, 非空值={data.notna().sum() if hasattr(data, 'notna') else 'N/A'}"
                )
