...
def main():
    ...
    if validator.validate_and_sync():
        ...
        print("❌ 一致性验证失败")
        print(
            "💡 这是正常的，因为我们需要确保FactorEngine只包含factor_generation中的因子"
        )
    ...
    print(
        f"   一致性状态: {'✅ 通过' if report['consistency_analysis']['is_consistent'] else '❌ 失败'}"
    )
    ...
    if report["consistency_analysis"]["extra_in_engine"]:
        print(
            f"   FactorEngine多余: {report['consistency_analysis']['extra_in_engine']}"
        )
    ...
    print(f"   # 创建基准快照")
    print(
        f"   python factor_system/factor_engine/factor_consistency_guard.py create-baseline"
    )
    print(f"   # 强制同步")
