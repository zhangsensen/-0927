...
def debug_macd_difference():
    ...
    print(f"TA-Lib MACD: 非空值={talib_macd[0].notna().sum()}, 前5个值={talib_macd[0].dropna().head().values}")

    # 方法2: 共享计算器
    shared_macd = SHARED_CALCULATORS.calculate_macd(price, fastperiod=12, slowperiod=26, signalperiod=9)
    print(f"共享计算器MACD: 非空值={shared_macd['macd'].notna().sum()}, 前5个值={shared_macd['macd'].dropna().head().values}")

    # 方法3: VectorBT
    adapter = get_vectorbt_adapter()
    vbt_macd = adapter.calculate_macd(price, fast_period=12, slow_period=26, signal_period=9)
    print(f"VectorBT MACD: 非空值={vbt_macd.notna().sum()}, 前5个值={vbt_macd.dropna().head().values}")
