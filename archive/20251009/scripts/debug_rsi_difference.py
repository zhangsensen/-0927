...
def debug_rsi_difference():
    ...
    print(f"TA-Lib RSI: 非空值={talib_rsi.notna().sum()}, 前5个值={talib_rsi.dropna().head().values}")

    # 方法2: 共享计算器
    shared_rsi = SHARED_CALCULATORS.calculate_rsi(price, period=14)
    print(f"共享计算器RSI: 非空值={shared_rsi.notna().sum()}, 前5个值={shared_rsi.dropna().head().values}")

    # 方法3: VectorBT
    adapter = get_vectorbt_adapter()
    vbt_rsi = adapter.calculate_rsi(price, timeperiod=14)
    print(f"VectorBT RSI: 非空值={vbt_rsi.notna().sum()}, 前5个值={vbt_rsi.dropna().head().values}")
