...
def debug_stoch_final():
    ...
    for i, component in enumerate(talib_stoch):
        if component is not None:
            print(
                f"  组件{i}: 非空值={component.notna().sum()}, 前5个值={component.dropna().head().values}"
            )

    # 方法2: 共享计算器
    shared_stoch = SHARED_CALCULATORS.calculate_stoch(
        high, low, close, fastk_period=5, slowk_period=3, slowd_period=3
    )
    print(f"共享计算器STOCH: {len(shared_stoch)} 个组件")
    for key, value in shared_stoch.items():
        print(
            f"  {key}: 非空值={value.notna().sum()}, 前5个值={value.dropna().head().values}"
        )

    # 方法3: VectorBT
    adapter = get_vectorbt_adapter()
    vbt_stoch = adapter.calculate_stoch(
        high, low, close, fastk_period=5, slowk_period=3, slowd_period=3
    )
    print(
        f"VectorBT STOCH: 非空值={vbt_stoch.notna().sum()}, 前5个值={vbt_stoch.dropna().head().values}"
    )
