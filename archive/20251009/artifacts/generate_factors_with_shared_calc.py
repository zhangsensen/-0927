...
class SharedCalculatorFactorGenerator:
    ...
    def extract_all_factors(self) -> Dict[str, Dict]:
        ...
        factors_df = calculator.calculate_comprehensive_factors(
            test_data, TimeFrame.MIN_5
        )
    ...
    def _determine_category(self, factor_name: str) -> str:
        ...
        if any(
            p in name_upper
            for p in ["MA", "EMA", "SMA", "WMA", "DEMA", "TEMA", "BB_", "BOLB"]
        ):
            return "overlap"
    ...
    def _extract_parameters(self, factor_name: str) -> Dict:
        ...
        if "MACD" in name_upper and len(numbers) >= 3:
            params = {
                "fastperiod": int(numbers[0]),
                "slowperiod": int(numbers[1]),
                "signalperiod": int(numbers[2]),
            }
        elif "STOCH" in name_upper and len(numbers) >= 2:
            params = {
                "fastk_period": int(numbers[0]),
                "slowk_period": int(numbers[1]),
                "slowd_period": int(numbers[1]),
            }
        elif any(
            ind in name_upper
            for ind in [
            ...
            params = {
                "timeperiod": int(numbers[0]),
                "nbdevup": float(numbers[1]) / 10.0,
                "nbdevdn": float(numbers[1]) / 10.0,
            }
...
"""
            ...
            any(x in name_upper for x in ["MA", "EMA", "SMA", "WMA"])
            and "MACD" not in name_upper
        ):
            period = params.get("timeperiod", 20)
...
"""
    ...
    def _generate_category_file(
        self, category: str, factors: List[Tuple[str, Dict]]
    ) -> str:
        """生成类别文件"""
        content = f'''"""
自动生成的{category}类因子
...
"""
    ...
    def run(self) -> List[str]:
        ...
        logger.info(
            f"✅ 完成! 生成 {len(generated_files)} 个文件，{len(factor_info)} 个因子"
        )
        return generated_files
