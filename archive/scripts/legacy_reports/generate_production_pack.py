...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to a run manifest.yaml containing protocol dates (start_date/training_end_date/end_date)",
    )
    ...
    # Optional protocol dates (for audit-grade reporting only)
    protocol = None
    if args.manifest:
    ...
    with open(report_path, "w") as f:
        ...
        f.write("### Data Splits\n")
        if protocol:
            start_date = protocol.get("start_date", "2020-01-01")
            training_end_date = protocol.get("training_end_date", "2025-04-30")
            end_date = protocol.get("end_date", "")
            f.write(
                f"- **Train**: {start_date} → {training_end_date} (from manifest)\n"
            )
            f.write(
                f"- **Holdout**: {pd.to_datetime(training_end_date).date() + pd.Timedelta(days=1)} → {end_date} (from manifest)\n\n"
            )
        else:
            f.write("- **Train**: 2020-01-01 → 2025-04-30 (default)\n")
            f.write("- **Holdout**: 2025-05-01 → (see run manifest)\n\n")
