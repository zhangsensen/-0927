...
> **⚠️ 研究归档**
>
> 此文档基于 **v4.2 基线 (FREQ=3)**。v5.0 生产版已采用 **FREQ=5 + Exp4 hysteresis (delta_rank=0.10, min_hold_days=9)**，因子评估结论可能不同。
> 最新因子列表见 `configs/combo_wfo_config.yaml`（23 个活跃因子：17 OHLCV + 6 non-OHLCV）。
...
注: QDII 仅 8 只 (v5.0), 截面 IC 不稳定, 仅供参考方向。
