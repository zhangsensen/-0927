...
def main():
    ...
    try:
        ...
        provider = MoneyFlowProvider(
            ...
            enforce_t_plus_1=True  # 强制T+1时序安全
        )
        ...
        print(f"   ✅ 时序安全: {df['temporal_safe'].all() if 'temporal_safe' in df.columns else 'N/A'}")
        print()
        ...
        for factor_name, factor in factors.items():
            ...
            try:
                ...
                if len(valid_values) > 0:
                    ...
                    print(f"      📉 均值: {valid_values.mean():.4f}, 标准差: {valid_values.std():.4f}")
                else:
                    print(f"      ⚠️ 无有效值")
                print()
                ...
                print(f"      ❌ 计算失败: {e}")
                factor_results[factor_name] = pd.Series(np.nan, index=df.index, name=factor_name)
                print()
        ...
        output_file = f"factor_output/moneyflow_factors_{test_symbol}_{timestamp}.parquet"
