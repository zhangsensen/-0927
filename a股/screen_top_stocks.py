...
def main():
    ...
    if len(buy_candidates) < 6:
        ...
        hold_candidates = results_df[results_df["recommendation"] == "持有"].head(
            6 - len(buy_candidates)
        )
...
"""
        ...
        for i, (_, row) in enumerate(results_df.iterrows(), 1):
            f.write(
                f"| {i} | {row['stock_code']} | {row['composite_score']:.2f} | {row['recommendation']} | {row['total_return']:.2f}% | {row['sharpe_ratio']:.2f} | {row['current_price']:.2f}元 |\n"
            )
