...
class MultiTimeframe300450Analyzer:
    ...
    def analyze_timeframe_alignment(
        ...
        # 获取关键指标对齐情况
        daily_rsi = (
            daily_factors[[col for col in daily_factors.columns if "RSI_14" in col]]
            .iloc[-1]
            .iloc[0]
            if [col for col in daily_factors.columns if "RSI_14" in col]
            else 50
        )
        hourly_rsi = (
            hourly_factors[[col for col in hourly_factors.columns if "RSI_14" in col]]
            .iloc[-1]
            .iloc[0]
            if [col for col in hourly_factors.columns if "RSI_14" in col]
            else 50
        )
        ...
        daily_macd_cols = [
            ...
            and "MACD" in col
            and "Signal" not in col
            and "Histogram" not in col
        ]
        hourly_macd_cols = [
            col
        ...
        daily_macd = (
            daily_factors[daily_macd_cols].iloc[-1].iloc[0] if daily_macd_cols else 0
        )
        hourly_macd = (
            hourly_factors[hourly_macd_cols].iloc[-1].iloc[0] if hourly_macd_cols else 0
        )

        # 趋势对齐分析
        daily_ma20 = (
            daily_factors[[col for col in daily_factors.columns if "MA_20" in col]]
            .iloc[-1]
            .iloc[0]
            if [col for col in daily_factors.columns if "MA_20" in col]
            else latest_daily_price
        )
        hourly_ma20 = (
            hourly_factors[[col for col in hourly_factors.columns if "MA_20" in col]]
            .iloc[-1]
            .iloc[0]
            if [col for col in hourly_factors.columns if "MA_20" in col]
            else latest_hourly_price
        )
        ...
        return {
            ...
            "trend_alignment": {
                ...
                "trend_consistent": (latest_daily_price > daily_ma20)
                == (latest_hourly_price > hourly_ma20),
            },
        }
    ...
    def detect_multi_timeframe_signals(
        ...
        if daily_rsi > 50 and hourly_rsi > 50:
            signals["bullish_signals"].append(
                f"日线RSI({daily_rsi:.1f})和小时线RSI({hourly_rsi:.1f})均强势"
            )
            signals["signal_strength"] += 1.5
        elif daily_rsi < 50 and hourly_rsi < 50:
            signals["bearish_signals"].append(
                f"日线RSI({daily_rsi:.1f})和小时线RSI({hourly_rsi:.1f})均弱势"
            )
            signals["signal_strength"] -= 1.5
        else:
        ...
        # 5. 成交量确认
        recent_hourly_volume = (
            df_hourly["volume"].tail(24).mean()
        )  # 最近24小时平均成交量
    ...
    def generate_comprehensive_signals(
        ...
        alignment = self.analyze_timeframe_alignment(
            df_daily, df_hourly, daily_factors, hourly_factors
        )
        signals = self.detect_multi_timeframe_signals(
            df_daily, df_hourly, daily_factors, hourly_factors, alignment
        )
        ...
        # 综合建议
        if signals["signal_strength"] >= 2 and signals["confidence_level"] in [
            "high",
            "medium",
        ]:
            ...
            recommendation = "buy"
        elif signals["signal_strength"] <= -2 and signals["confidence_level"] in [
            "high",
            "medium",
        ]:
    ...
    def generate_comprehensive_report(
        ...
        comprehensive_signals = self.generate_comprehensive_signals(
            df_daily, df_hourly, daily_factors, hourly_factors
        )
        ...
        daily_change_5d = (
            (df_daily["close"].iloc[-1] / df_daily["close"].iloc[-6] - 1) * 100
            if len(df_daily) >= 6
            else 0
        )
        ...
        hourly_change_4h = (
            (df_hourly["close"].iloc[-1] / df_hourly["close"].iloc[-5] - 1) * 100
            if len(df_hourly) >= 5
            else 0
        )
...
def main():
    ...
    try:
        ...
        # 计算技术指标
        daily_factors, hourly_factors = analyzer.calculate_multi_timeframe_indicators(
            df_daily, df_hourly
        )
        ...
        analysis_data = {
            ...
            "latest_signals": analyzer.generate_comprehensive_signals(
                df_daily, df_hourly, daily_factors, hourly_factors
            ),
        }
