...
def parse_arguments():
    ...
    parser.add_argument('--data-dir', default='/Users/zhangshenshen/深度量化0927/a股',
                       help='数据文件目录路径')
    parser.add_argument('--output-dir', default='/Users/zhangshenshen/深度量化0927',
                       help='分析报告输出目录')
...
def detect_macd_divergence(df, lookback=20):
    ...
    for i in range(lookback, len(df)):
        ...
        if len(price_highs) >= 2:
            ...
            price_prev = price_highs.iloc[-5:-1].max() if len(price_highs) > 5 else price_highs.iloc[:-1].max()

            macd_curr = macd_highs.iloc[-1]
            macd_prev = macd_highs.iloc[-5:-1].max() if len(macd_highs) > 5 else macd_highs.iloc[:-1].max()
        ...
        if len(price_lows) >= 2:
            ...
            price_prev = price_lows.iloc[-5:-1].min() if len(price_lows) > 5 else price_lows.iloc[:-1].min()

            macd_curr = macd_lows.iloc[-1]
            macd_prev = macd_lows.iloc[-5:-1].min() if len(macd_lows) > 5 else macd_lows.iloc[:-1].min()
...
def cluster_support_resistance(df, n_clusters=5, window=10):
    ...
    if len(resistance_candidates) > n_clusters:
        kmeans_resistance = KMeans(n_clusters=min(n_clusters, len(resistance_candidates)), random_state=42)
    ...
    if len(support_candidates) > n_clusters:
        kmeans_support = KMeans(n_clusters=min(n_clusters, len(support_candidates)), random_state=42)
...
def find_support_resistance(df, window=20):
    ...
    # 使用聚类算法识别动态支撑阻力位
    try:
        clustered_resistance, clustered_support = cluster_support_resistance(df, n_clusters=4, window=window)
    except ImportError:
    ...
    nearest_resistance_high = pivot_highs[pivot_highs > current_price].min() if (pivot_highs > current_price).any() else current_price * 1.05
    nearest_support_low = pivot_lows[pivot_lows < current_price].max() if (pivot_lows < current_price).any() else current_price * 0.95

    # 计算枢轴点 (Pivot Points)
    current_pivot = (df['High'].iloc[-1] + df['Low'].iloc[-1] + df['Close'].iloc[-1]) / 3
...
def generate_trading_recommendation(df, metrics, current_state, indicators):
    ...
    if current_atr:
        if recommendation in ["强烈买入", "买入"]:
            stop_loss = max(nearest_support if nearest_support else current_price * 0.95,
                          current_price - 2 * current_atr)
            take_profit = min(nearest_resistance if nearest_resistance else current_price * 1.05,
                            current_price + 3 * current_atr)
        else:
            stop_loss = min(nearest_resistance if nearest_resistance else current_price * 1.05,
                          current_price + 2 * current_atr)
            take_profit = max(nearest_support if nearest_support else current_price * 0.95,
                            current_price - 3 * current_atr)
    else:
        # 传统百分比止损
        if recommendation in ["强烈买入", "买入"]:
    ...
    # 生成信号列表用于显示
    signals = []
    signals.extend([f"RSI: {momentum_signals['rsi']}", f"MACD: {momentum_signals['macd']}"])
    signals.extend([f"ADX: {trend_signals['adx_strength']} {trend_signals['adx_direction']}", f"均线: {trend_signals['ma_system']}"])
...
def main():
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='通用股票技术分析工具')
    parser.add_argument('stock_code', nargs='?', default='300450.SZ', help='股票代码 (例如: 300450.SZ)')
    parser.add_argument('--data-dir', default='/Users/zhangshenshen/深度量化0927/a股',
                       help='数据文件目录路径')
    parser.add_argument('--output-dir', default='/Users/zhangshenshen/深度量化0927',
                       help='分析报告输出目录')
