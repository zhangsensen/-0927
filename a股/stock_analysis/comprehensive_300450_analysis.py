...
class Comprehensive300450Analyzer:
    ...
    def load_all_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        print(
            f"✅ 日线数据加载完成: {len(df_daily)}条记录 ({df_daily.index[0]} 到 {df_daily.index[-1]})"
        )
        print(
            f"✅ 小时线数据加载完成: {len(df_hourly)}条记录 ({df_hourly.index[0]} 到 {df_hourly.index[-1]})"
        )

        return df_daily, df_hourly

    def calculate_comprehensive_indicators(
        self, df_daily: pd.DataFrame, df_hourly: pd.DataFrame
        ...
        # 计算日线指标
        print("📊 计算日线综合技术指标...")
        daily_factors = calculator.calculate_comprehensive_factors(
            df_daily, TimeFrame.DAILY
        )

        # 计算小时线指标
        print("📊 计算小时线综合技术指标...")
        hourly_factors = calculator.calculate_comprehensive_factors(
            df_hourly, TimeFrame.DAILY
        )
    ...
    def perform_comprehensive_analysis(
        ...
        analysis = {
            ...
            "trend_analysis": self.analyze_trend_patterns(
                df_daily, df_hourly, daily_factors, hourly_factors
            ),
            "momentum_analysis": self.analyze_momentum_indicators(
            ...
            "multi_timeframe_signals": self.generate_multi_timeframe_signals(
                df_daily, df_hourly, daily_factors, hourly_factors
            ),
        }
    ...
    def analyze_price_behavior(
        ...
        try:
            ...
            # 多周期表现
            daily_5d = (
                (daily_latest["close"] / df_daily["close"].iloc[-6] - 1) * 100
                if len(df_daily) >= 6
                else 0
            )
            daily_20d = (
                (daily_latest["close"] / df_daily["close"].iloc[-21] - 1) * 100
                if len(df_daily) >= 21
                else 0
            )
            ...
            price_position = (
                (daily_latest["close"] - daily_low_20d)
                / (daily_high_20d - daily_low_20d)
                * 100
            )
    ...
    def analyze_trend_patterns(
        ...
        try:
            ...
            return {
                ...
                "primary_trend": self.determine_primary_trend(
                    ma_trends, trend_strength
                ),
    ...
    def analyze_momentum_indicators(
        ...
        try:
            ...
            for rsi_period in [14, 20, 30, 60]:
                rsi_cols = [
                    col for col in daily_factors.columns if f"RSI_{rsi_period}" in col
                ]
                if rsi_cols:
                    rsi_indicators[f"daily_rsi_{rsi_period}"] = daily_factors[
                        rsi_cols[0]
                    ].iloc[-1]

            for rsi_period in [14, 20]:
                rsi_cols = [
                    col for col in hourly_factors.columns if f"RSI_{rsi_period}" in col
                ]
                if rsi_cols:
                    rsi_indicators[f"hourly_rsi_{rsi_period}"] = hourly_factors[
                        rsi_cols[0]
                    ].iloc[-1]
            ...
            if macd_daily_cols:
                macd_indicators["daily_macd"] = daily_factors[macd_daily_cols[0]].iloc[
                    -1
                ]

            macd_hourly_cols = [
                col
                for col in hourly_factors.columns
                if "MACD" in col and "Signal" not in col and "Histogram" not in col
            ]
            if macd_hourly_cols:
                macd_indicators["hourly_macd"] = hourly_factors[
                    macd_hourly_cols[0]
                ].iloc[-1]
            ...
            stoch_indicators = {}
            stoch_daily_cols = [
                col for col in daily_factors.columns if "STOCH" in col and "%K" in col
            ]
            if stoch_daily_cols:
                stoch_indicators["daily_stoch_k"] = daily_factors[
                    stoch_daily_cols[0]
                ].iloc[-1]
            ...
            # 综合动量评级
            momentum_indicators["overall_momentum"] = self.assess_overall_momentum(
                momentum_indicators
            )
        ...
        try:
            ...
            if bb_cols:
                ...
                if bb_upper_cols and bb_lower_cols:
                    bb_width = (
                        (
                            daily_factors[bb_upper_cols[0]].iloc[-1]
                            - daily_factors[bb_lower_cols[0]].iloc[-1]
                        )
                        / current_price
                        * 100
                    )
    ...
    def analyze_volume_patterns(
        ...
        try:
            ...
            volume_analysis = {
                ...
                "volume_signal": self.assess_volume_signal(
                    daily_volume_ratio, hourly_volume_ratio
                ),
            }
    ...
    def analyze_chart_patterns(
        ...
        try:
            ...
            # 整理形态
            patterns["consolidation_patterns"] = self.identify_consolidation_patterns(
                df_daily
            )
            ...
            # 持续形态
            patterns["continuation_patterns"] = self.identify_continuation_patterns(
                df_daily
            )
    ...
    def identify_support_resistance(
        ...
        try:
            ...
            support_resistance = {
                ...
                "nearest_resistance": self.find_nearest_resistance(
                    current_price, recent_highs.head(5).tolist()
                ),
                "nearest_support": self.find_nearest_support(
                    current_price, recent_lows.head(5).tolist()
                ),
                "price_position": self.assess_price_position(
                    current_price,
                    recent_lows.head(5).tolist(),
                    recent_highs.head(5).tolist(),
                ),
            }
    ...
    def generate_multi_timeframe_signals(
        ...
        try:
            signals = {
                "daily_signals": self.generate_timeframe_signals(
                    df_daily, daily_factors, "日线"
                ),
                "hourly_signals": self.generate_timeframe_signals(
                    df_hourly, hourly_factors, "小时线"
                ),
                "combined_signals": {},
                "signal_strength": 0,
            ...
            # 整合多时间框架信号
            daily_bullish = len(
                [s for s in signals["daily_signals"] if s["type"] == "bullish"]
            )
            daily_bearish = len(
                [s for s in signals["daily_signals"] if s["type"] == "bearish"]
            )
            hourly_bullish = len(
                [s for s in signals["hourly_signals"] if s["type"] == "bullish"]
            )
            hourly_bearish = len(
                [s for s in signals["hourly_signals"] if s["type"] == "bearish"]
            )
            ...
            signals["combined_signals"] = {
                ...
                "timeframe_agreement": self.check_timeframe_agreement(
                    daily_bullish - daily_bearish, hourly_bullish - hourly_bearish
                ),
            }
            ...
            # 确定置信度
            if (
                abs(signals["signal_strength"]) >= 2
                and signals["combined_signals"]["timeframe_agreement"]
            ):
    ...
    def generate_timeframe_signals(
        self, df: pd.DataFrame, factors: pd.DataFrame, timeframe_name: str
    ) -> List[Dict]:
        """生成单个时间框架的信号"""
        signals = []
        try:
                    ...
                    signals.append(
                        {
                            ...
                            "indicator": "RSI",
                            "value": rsi_value,
                            "message": f"{timeframe_name}RSI超买",
                        }
                    )
                elif rsi_value < 30:
                    signals.append(
                        {
                            ...
                            "indicator": "RSI",
                            "value": rsi_value,
                            "message": f"{timeframe_name}RSI超卖",
                        }
                    )
                elif rsi_value > 50:
                    signals.append(
                        {
                            ...
                            "indicator": "RSI",
                            "value": rsi_value,
                            "message": f"{timeframe_name}RSI强势",
                        }
                    )
            ...
            signal_cols = [
                col for col in factors.columns if "MACD" in col and "Signal" in col
            ]
                    ...
                    signals.append(
                        {
                            ...
                            "indicator": "MACD",
                            "value": macd_value,
                            "message": f"{timeframe_name}MACD多头",
                        }
                    )
                    ...
                    signals.append(
                        {
                            ...
                            "indicator": "MACD",
                            "value": macd_value,
                            "message": f"{timeframe_name}MACD空头",
                        }
                    )
                    ...
                    signals.append(
                        {
                            ...
                            "indicator": "MA20",
                            "value": ma20_value,
                            "message": f"{timeframe_name}价格位于MA20上方",
                        }
                    )
                else:
                    signals.append(
                        {
                            ...
                            "indicator": "MA20",
                            "value": ma20_value,
                            "message": f"{timeframe_name}价格位于MA20下方",
                        }
                    )
        ...
        try:
            ...
            for period in [1, 5, 10, 20]:
                if len(df_daily) > period:
                    momentum = (
                        df_daily["close"].iloc[-1] / df_daily["close"].iloc[-period - 1]
                        - 1
                    ) * 100
    ...
    def calculate_ma_slope(
        self, factors: pd.DataFrame, ma_col: str, periods: int
    ) -> float:
        """计算移动平均斜率"""
        try:
    ...
    def analyze_trend_consistency(
        ...
        try:
            # 简化的趋势一致性分析
            daily_trend = (
                df_daily["close"].tail(5).is_monotonic_increasing
                or df_daily["close"].tail(5).is_monotonic_decreasing
            )
            hourly_trend = (
                df_hourly["close"].tail(24).is_monotonic_increasing
                or df_hourly["close"].tail(24).is_monotonic_decreasing
            )
    ...
    def determine_primary_trend(self, ma_trends: Dict, trend_strength: Dict) -> str:
        ...
        try:
            bullish_count = sum(
                1 for trend in ma_trends.values() if trend.get("above", False)
            )
    ...
    def identify_continuation_patterns(self, df: pd.DataFrame) -> List[str]:
        ...
        try:
            ...
            if (recent_highs.iloc[-3:] - recent_highs.iloc[:3]).mean() < 0 and (
                recent_lows.iloc[-3:] - recent_lows.iloc[:3]
            ).mean() > 0:
    ...
    def assess_price_position(
        ...
        try:
            ...
            if nearest_support and nearest_resistance:
                ...
                resistance_distance = (
                    (nearest_resistance - current_price) / nearest_resistance * 100
                )
    ...
    def check_timeframe_agreement(self, daily_signal: int, hourly_signal: int) -> bool:
        """检查时间框架一致性"""
        return (daily_signal > 0 and hourly_signal > 0) or (
            daily_signal < 0 and hourly_signal < 0
        )
    ...
    def generate_comprehensive_report(
        ...
        current_price = analysis["price_analysis"].get(
...
def main():
    ...
    try:
        ...
        # 计算综合技术指标
        daily_factors, hourly_factors = analyzer.calculate_comprehensive_indicators(
            df_daily, df_hourly
        )
        ...
        analysis_data = {
            ...
            "key_insights": {
                ...
                "overall_momentum": analysis["momentum_analysis"].get(
                    "overall_momentum"
                ),
                "volatility_level": analysis["volatility_analysis"]
                .get("daily_atr", {})
        ...
        print(
            f"📈 主要趋势: {analysis['trend_analysis'].get('primary_trend', 'unknown')}"
        )
        print(
            f"🚀 整体动量: {analysis['momentum_analysis'].get('overall_momentum', 'unknown')}"
        )
        print(
            f"⚠️ 综合风险: {analysis['risk_assessment'].get('overall_risk', {}).get('risk_level', 'unknown')}"
        )
