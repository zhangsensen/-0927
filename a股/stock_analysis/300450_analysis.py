...
def analyze_short_term_trend(df):
    ...
    return {
        ...
        'trend_strength': '强' if abs(ma5_slope) > 0.5 else '中等' if abs(ma5_slope) > 0.2 else '弱'
    }

def generate_trading_recommendation(df, metrics, current_state, indicators, trend_analysis):
    """生成交易建议"""
    signals = []
    score = 0
    ...
    resistance_levels = [
        ...
        max(indicators['resistance_levels']) if indicators['resistance_levels'] else current_price * 1.08
    ]
    ...
    support_levels = [
        ...
        min(indicators['support_levels']) if indicators['support_levels'] else current_price * 0.92
    ]
...
def main():
    ...
    # 生成交易建议
    recommendation = generate_trading_recommendation(df_with_indicators, metrics, current_state, indicators, trend_analysis)
    ...
    print(f"🔺 价格相对MA10: {'在上方' if current_state['Price_vs_MA10'] else '在下方'}")
    ...
    print(f"📊 当前价格位置:")
    print(f"   距离MA5: {((latest['Close'] - latest['MA5']) / latest['MA5'] * 100):+.2f}%")
    print(f"   距离MA20: {((latest['Close'] - latest['MA20']) / latest['MA20'] * 100):+.2f}%")
    print(f"   距离布林带上轨: {((latest['Close'] - latest['BB_Upper']) / latest['BB_Upper'] * 100):+.2f}%")
    print(f"   距离布林带下轨: {((latest['Close'] - latest['BB_Lower']) / latest['BB_Lower'] * 100):+.2f}%")
