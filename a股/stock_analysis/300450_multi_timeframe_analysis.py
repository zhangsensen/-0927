...
def load_stock_data(daily_file, hourly_file):
    ...
    print(f"日线数据: {len(daily_df)} 条记录 ({daily_df.index.min()} 到 {daily_df.index.max()})")
    print(f"小时线数据: {len(hourly_df)} 条记录 ({hourly_df.index.min()} 到 {hourly_df.index.max()})")
...
def calculate_technical_indicators(daily_df, hourly_df):
    ...
    daily_df['MACD'], daily_df['Signal'], daily_df['MACD_Hist'] = calculate_macd(daily_df)
    daily_df['BB_Upper'], daily_df['BB_Middle'], daily_df['BB_Lower'] = calculate_bollinger_bands(daily_df)
    ...
    hourly_df['MACD'], hourly_df['Signal'], hourly_df['MACD_Hist'] = calculate_macd(hourly_df)
    hourly_df['BB_Upper'], hourly_df['BB_Middle'], hourly_df['BB_Lower'] = calculate_bollinger_bands(hourly_df, period=50)
...
def analyze_timeframe_alignment(daily_df, hourly_df):
    ...
    # 日线状态
    daily_macd_state = "金叉" if latest_daily['MACD'] > latest_daily['Signal'] else "死叉"
    daily_rsi_state = "超买" if latest_daily['RSI'] > 70 else "超卖" if latest_daily['RSI'] < 30 else "正常"
    daily_ma_trend = "多头" if latest_daily['MA5'] > latest_daily['MA20'] else "空头"

    # 小时线状态
    hourly_macd_state = "金叉" if latest_hourly['MACD'] > latest_hourly['Signal'] else "死叉"
    hourly_rsi_state = "超买" if latest_hourly['RSI'] > 70 else "超卖" if latest_hourly['RSI'] < 30 else "正常"
    hourly_ma_trend = "多头" if latest_hourly['MA20'] > latest_hourly['MA60'] else "空头"
    ...
    if daily_macd_state == hourly_macd_state:
        ...
        alignment_signals.append(f"MACD时间框架分歧(日线{daily_macd_state}, 小时线{hourly_macd_state})")

    if daily_ma_trend == hourly_ma_trend:
        alignment_score += 2
        alignment_signals.append(f"均线趋势一致({daily_ma_trend})")
    else:
        alignment_signals.append(f"均线趋势分歧(日线{daily_ma_trend}, 小时线{hourly_ma_trend})")
    ...
    return {
        ...
        'alignment_strength': '强' if alignment_score >= 4 else '中等' if alignment_score >= 2 else '弱'
    }
...
def find_key_levels(daily_df, hourly_df):
    ...
    return {
        ...
        'hourly': {
            'resistance_5d': hourly_high,
            'support_5d': hourly_low
        },
        'fibonacci': {
            '38.2%': fib_382,
            '50.0%': fib_500,
...
def analyze_intraday_momentum(hourly_df):
    ...
    price_position = (recent_hours['Close'].iloc[-1] - recent_low) / (recent_high - recent_low)

    return {
        'momentum_score': momentum_score,
        'volume_momentum': volume_momentum,
        'price_position': price_position,
        'momentum_direction': '向上' if momentum_score > 0 else '向下' if momentum_score < 0 else '横盘'
    }

def generate_multi_timeframe_signals(daily_df, hourly_df, alignment, key_levels, momentum):
    """生成多时间框架交易信号"""
    signals = []
    buy_signals = 0
    sell_signals = 0
    ...
    # 日线趋势信号
    if alignment['daily']['ma_trend'] == "多头" and alignment['daily']['macd'] == "金叉":
        signals.append("🟢 日线趋势向上，中期看多")
        buy_signals += 2
    elif alignment['daily']['ma_trend'] == "空头" and alignment['daily']['macd'] == "死叉":
        signals.append("🔴 日线趋势向下，中期看空")
        sell_signals += 2

    # 小时线动量信号
    if alignment['hourly']['macd'] == "金叉" and momentum['momentum_direction'] == "向上":
        signals.append("🟡 小时线动量向上，短期偏多")
        buy_signals += 1
    elif alignment['hourly']['macd'] == "死叉" and momentum['momentum_direction'] == "向下":
    ...
    return {
        ...
        'signals': signals,
        'resistance_levels': sorted([x for x in resistance_levels if x > current_price]),
        'support_levels': sorted([x for x in support_levels if x < current_price], reverse=True)
    }
...
def main():
    ...
    # 加载数据
    daily_file = "/Users/zhangshenshen/深度量化0927/a股/300450.SZ/300450.SZ_1d_2025-09-30.csv"
    hourly_file = "/Users/zhangshenshen/深度量化0927/a股/300450.SZ/300450.SZ_1h_2025-09-30.csv"
    ...
    # 生成多时间框架信号
    signals = generate_multi_timeframe_signals(daily_df, hourly_df, alignment, key_levels, momentum)
    ...
    print(f"   RSI: {alignment['daily']['rsi_value']:.1f} ({alignment['daily']['rsi']})")
    print(f"   均线趋势: {alignment['daily']['ma_trend']}")

    print("\n🕒 小时线状态:")
    print(f"   价格: {alignment['hourly']['price']:.2f}元")
    print(f"   MACD: {alignment['hourly']['macd']}")
    print(f"   RSI: {alignment['hourly']['rsi_value']:.1f} ({alignment['hourly']['rsi']})")
    ...
    if signals['recommendation'] in ["强烈买入", "买入"]:
        print("🟢 入场策略:")
        print(f"   • 建议价格区间: {signals['support_levels'][0]:.2f} - {current_price:.2f}元")
        ...
        print("🟡 观望策略:")
        print(f"   • 等待更明确信号")
        print(f"   • 关注价格在 {signals['support_levels'][0]:.2f} - {signals['resistance_levels'][0]:.2f}元区间的突破")
        print(f"   • 当前适合高抛低吸")
