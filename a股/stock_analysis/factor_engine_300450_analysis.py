...
class FactorEngine300450Analyzer:
    ...
    def calculate_factor_engine_indicators(self, df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
        """使用FactorEngine计算技术指标因子"""
        print(f"正在使用FactorEngine计算{timeframe}技术因子...")

        # 设置环境变量
        import os
        os.environ['FACTOR_ENGINE_RAW_DATA_DIR'] = '/Users/zhangshenshen/深度量化0927/raw'
        ...
        key_factors = [
            'RSI14', 'RSI10', 'RSI7', 'STOCH_14_20', 'STOCH_10_14', 'STOCH_7_10',
            'WILLR14', 'WILLR9', 'CCI14', 'CCI10', 'CCI20', 'ATR14', 'ATR10', 'ATR7',
            'MSTD20', 'MSTD15', 'MSTD10', 'FIXLB10', 'FIXLB8', 'FIXLB5'
        ]
    ...
    def analyze_factor_signals(self, daily_factors: pd.DataFrame, hourly_factors: pd.DataFrame) -> Dict:
        """分析因子信号"""
        print("正在分析因子信号...")
        ...
        # 分析日线因子信号
        signals['daily_signals'] = self.analyze_single_timeframe_factors(daily_factors, '日线')

        # 分析小时线因子信号
        signals['hourly_signals'] = self.analyze_single_timeframe_factors(hourly_factors, '小时线')

        # 分析因子对齐情况
        signals['factor_alignment'] = self.analyze_factor_alignment(daily_factors, hourly_factors)
    ...
    def analyze_single_timeframe_factors(self, factors_df: pd.DataFrame, timeframe_name: str) -> Dict:
        """分析单个时间框架的因子信号"""
        signals = {
            'trend_signals': [],
            'momentum_signals': [],
        ...
        try:
            ...
            if 'RSI_14' in latest_data:
                ...
                if rsi_value > 70:
                    signals['momentum_signals'].append(f"{timeframe_name}RSI超买({rsi_value:.1f})")
                    signals['overall_score'] -= 0.5
                elif rsi_value < 30:
                    signals['momentum_signals'].append(f"{timeframe_name}RSI超卖({rsi_value:.1f})")
                    signals['overall_score'] += 0.5
                elif rsi_value > 50:
                    signals['momentum_signals'].append(f"{timeframe_name}RSI强势({rsi_value:.1f})")
                    signals['overall_score'] += 0.3
                else:
                    signals['momentum_signals'].append(f"{timeframe_name}RSI弱势({rsi_value:.1f})")
                    signals['overall_score'] -= 0.3
            ...
            # 随机指标信号分析
            if 'STOCH_14_3_3_SLOWK' in latest_data and 'STOCH_14_3_3_SLOWD' in latest_data:
                ...
                if k > d and k < 80:
                    ...
                    signals['momentum_signals'].append(f"{timeframe_name}KDJ超买({k:.1f})")
                    signals['overall_score'] -= 0.3
                elif k < 20:
                    signals['momentum_signals'].append(f"{timeframe_name}KDJ超卖({k:.1f})")
                    signals['overall_score'] += 0.3
            ...
            if 'WILLR_14' in latest_data:
                ...
                if willr > -20:
                    signals['momentum_signals'].append(f"{timeframe_name}Williams超买({willr:.1f})")
                    signals['overall_score'] -= 0.3
                elif willr < -80:
                    signals['momentum_signals'].append(f"{timeframe_name}Williams超卖({willr:.1f})")
                    signals['overall_score'] += 0.3

            # 布林带信号分析
            if 'BBANDS_20_2_UPPER' in latest_data and 'BBANDS_20_2_LOWER' in latest_data:
                upper = latest_data['BBANDS_20_2_UPPER']
                lower = latest_data['BBANDS_20_2_LOWER']
                close = latest_data['CLOSE']

                if close > upper:
                    signals['volatility_signals'].append(f"{timeframe_name}突破布林上轨")
                    signals['overall_score'] -= 0.5
                elif close < lower:
                    signals['volatility_signals'].append(f"{timeframe_name}跌破布林下轨")
                    signals['overall_score'] += 0.5
                else:
                    signals['volatility_signals'].append(f"{timeframe_name}价格在布林带内")

            # ADX趋势强度分析
            if 'ADX_14' in latest_data:
                adx = latest_data['ADX_14']
                if adx > 25:
                    signals['trend_signals'].append(f"{timeframe_name}ADX强趋势({adx:.1f})")
                elif adx > 20:
                    signals['trend_signals'].append(f"{timeframe_name}ADX中等趋势({adx:.1f})")
                else:
                    signals['trend_signals'].append(f"{timeframe_name}ADX弱趋势({adx:.1f})")
            ...
            if 'VOLUME' in latest_data:
                ...
                if len(factors_df) >= 20:
                    ...
                    if volume_ratio > 2.0:
                        signals['volume_signals'].append(f"{timeframe_name}成交量显著放大({volume_ratio:.1f}倍)")
                        signals['overall_score'] += 0.5
                    elif volume_ratio > 1.5:
                        signals['volume_signals'].append(f"{timeframe_name}成交量放大({volume_ratio:.1f}倍)")
                        signals['overall_score'] += 0.3
                    elif volume_ratio < 0.5:
                        signals['volume_signals'].append(f"{timeframe_name}成交量萎缩({volume_ratio:.1f}倍)")
    ...
    def analyze_factor_alignment(self, daily_factors: pd.DataFrame, hourly_factors: pd.DataFrame) -> Dict:
        """分析因子对齐情况"""
        alignment = {
        ...
        try:
            ...
            if 'RSI_14' in daily_latest and 'RSI_14' in hourly_latest:
                ...
                if (daily_rsi > 50 and hourly_rsi > 50) or (daily_rsi < 50 and hourly_rsi < 50):
                    alignment['aligned_factors'].append(f"RSI一致(日线:{daily_rsi:.1f}, 小时线:{hourly_rsi:.1f})")
                    alignment['overall_alignment_score'] += 1
                else:
                    alignment['conflicting_factors'].append(f"RSI分歧(日线:{daily_rsi:.1f}, 小时线:{hourly_rsi:.1f})")
            ...
            if 'MACD_12_26_9' in daily_latest and 'MACD_12_26_9' in hourly_latest:
                ...
                if (daily_macd > 0 and hourly_macd > 0) or (daily_macd < 0 and hourly_macd < 0):
                    alignment['aligned_factors'].append(f"MACD一致(日线:{daily_macd:+.4f}, 小时线:{hourly_macd:+.4f})")
                    alignment['overall_alignment_score'] += 1
                else:
                    alignment['conflicting_factors'].append(f"MACD分歧(日线:{daily_macd:+.4f}, 小时线:{hourly_macd:+.4f})")

            # 趋势对齐
            daily_above_ma20 = ('SMA_20' in daily_latest and daily_latest['CLOSE'] > daily_latest['SMA_20'])
            hourly_above_ma20 = ('SMA_20' in hourly_latest and hourly_latest['CLOSE'] > hourly_latest['SMA_20'])
    ...
    def generate_combined_signals(self, signals: Dict) -> Dict:
        ...
        for signal_type in ['trend_signals', 'momentum_signals', 'volume_signals']:
            ...
            for signal in daily_signals + hourly_signals:
                if any(keyword in signal for keyword in ['上方', '强势', '金叉', '放大', '突破']):
                    combined['bullish_signals'].append(signal)
                elif any(keyword in signal for keyword in ['下方', '弱势', '死叉', '萎缩', '跌破']):
    ...
    def generate_comprehensive_analysis(self, df_daily: pd.DataFrame, df_hourly: pd.DataFrame,
                                      daily_factors: pd.DataFrame, hourly_factors: pd.DataFrame,
                                      signals: Dict) -> Dict:
        """生成综合分析"""
        print("正在生成FactorEngine综合分析...")

        current_price = df_hourly['close'].iloc[-1] if 'close' in df_hourly.columns else df_hourly['CLOSE'].iloc[-1]
    ...
    def calculate_technical_levels(self, df: pd.DataFrame, factors: pd.DataFrame) -> Dict:
        """计算技术位"""
        try:
            current_price = df['close'].iloc[-1] if 'close' in df.columns else df['CLOSE'].iloc[-1]

            # 支撑阻力位
            recent_highs = df['high'].tail(20).nlargest(5) if 'high' in df.columns else factors['HIGH'].tail(20).nlargest(5)
            recent_lows = df['low'].tail(20).nsmallest(5) if 'low' in df.columns else factors['LOW'].tail(20).nsmallest(5)

            # ATR计算
            if len(df) >= 14:
                high_low = (df['high'] - df['low']) if 'high' in df.columns else (factors['HIGH'] - factors['LOW'])
    ...
    def generate_factor_summary(self, signals: Dict) -> Dict:
        """生成因子摘要"""
        return {
            'total_aligned_factors': len(signals['factor_alignment']['aligned_factors']),
            'total_conflicting_factors': len(signals['factor_alignment']['conflicting_factors']),
            ...
            'key_conflicting_factors': signals['factor_alignment']['conflicting_factors'][:3]
        }

    def generate_factor_engine_report(self, df_daily: pd.DataFrame, df_hourly: pd.DataFrame,
                                    daily_factors: pd.DataFrame, hourly_factors: pd.DataFrame,
                                    analysis: Dict) -> str:
        """生成FactorEngine分析报告"""
...
def main():
    ...
    try:
        ...
        # 生成综合分析
        analysis = analyzer.generate_comprehensive_analysis(df_daily, df_hourly, daily_factors, hourly_factors, signals)

        # 生成报告
        report = analyzer.generate_factor_engine_report(df_daily, df_hourly, daily_factors, hourly_factors, analysis)
