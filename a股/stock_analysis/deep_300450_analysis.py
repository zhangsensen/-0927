...
class Deep300450Analyzer:
    ...
    def calculate_volume_profile(self, df: pd.DataFrame) -> Dict:
        ...
        for price in price_levels:
            # 找到该价格附近的成交量
            nearby_volume = df[
                (df["close"] >= price - 0.5) & (df["close"] <= price + 0.5)
            ]["volume"].sum()
    ...
    def analyze_market_structure(
        ...
        # 4. 波动性分析
        atr_values = factors_df[
            [col for col in factors_df.columns if "ATR" in col]
        ].iloc[-1]
        ...
        bb_upper = (
            factors_df[bb_upper_cols].iloc[-1].iloc[0]
            if bb_upper_cols
            else current_price * 1.1
        )
        bb_lower = (
            factors_df[bb_lower_cols].iloc[-1].iloc[0]
            if bb_lower_cols
    ...
    def analyze_risk_reward_scenarios(
        self,
        df: pd.DataFrame,
        ...
        scenarios = {"bullish": {}, "bearish": {}, "neutral": {}, "risk_levels": {}}
        ...
        if market_structure["recent_highs"]:
            nearest_resistance = min(
                [h for h in market_structure["recent_highs"] if h > current_price],
        ...
        scenarios["bullish"] = {
            "target_price": (
                nearest_resistance if nearest_resistance else current_price * 1.1
            ),
            "potential_return": (
            ...
            "probability": (
                "high" if self._is_uptrend_confirmed(market_structure) else "medium"
            ),
        }
        ...
        if market_structure["recent_lows"]:
            nearest_support = max(
                [l for l in market_structure["recent_lows"] if l < current_price],
        ...
        scenarios["bearish"] = {
            ...
            "potential_return": (
                (nearest_support / current_price - 1) * 100 if nearest_support else -10
            ),
            "probability": (
                "high" if self._is_downtrend_confirmed(market_structure) else "medium"
            ),
        }
        ...
        scenarios["neutral"] = {
            "target_range": (
                f"{nearest_support:.2f} - {nearest_resistance:.2f}"
                if nearest_support and nearest_resistance
                else f"{current_price*0.95:.2f} - {current_price*1.05:.2f}"
            ),
            "expected_return": 0,
            "probability": "medium",
        }
        ...
        scenarios["risk_levels"] = {
            ...
            "overall_risk": self._calculate_overall_risk(
                volatility_risk, trend_risk, volume_risk
            ),
        }
    ...
    def _calculate_overall_risk(
        self, volatility_risk: str, trend_risk: str, volume_risk: str
    ) -> str:
        """计算总体风险等级"""
    ...
    def explain_stop_loss_logic(
        ...
        if market_structure["recent_lows"]:
            important_lows = [
                low for low in market_structure["recent_lows"] if low < current_price
            ]
            if important_lows:
        ...
        # b) 上升趋势线
        if (
            market_structure["uptrend_line"]
            and market_structure["uptrend_line"] < current_price
        ):
            potential_stops.append(
                {
                    ...
                    "strength": "medium",
                    "distance_pct": (
                        market_structure["uptrend_line"] / current_price - 1
                    )
                    * 100,
                }
            )
        ...
        potential_stops.sort(
            key=lambda x: (x["strength"], -abs(x["distance_pct"])), reverse=True
        )
        ...
        if potential_stops:
            ...
            stop_loss_explanation["reasoning"].append(
                f"选择{selected_stop['type']}作为止损位"
            )
            stop_loss_explanation["reasoning"].append(
                f"该位置强度：{selected_stop['strength']}"
            )
            stop_loss_explanation["reasoning"].append(
                f"距离当前价格：{selected_stop['distance_pct']:+.2f}%"
            )

            # 技术因素
            if selected_stop["type"] == "近期低点":
                stop_loss_explanation["technical_factors"].append(
                    "近期低点代表市场参与者认可的心理支撑位"
                )
                stop_loss_explanation["technical_factors"].append(
                    "跌破此位可能引发更多止损和抛售"
            ...
            if volatility_ratio > 0.08:  # 8%以上波动
                stop_loss_explanation["risk_considerations"].append(
                    f"高波动股票（ATR比率{volatility_ratio*100:.1f}%）需要较大止损空间"
                )
                stop_loss_explanation["risk_considerations"].append(
                    "小止损容易被市场噪音触发"
                )
    ...
    def explain_take_profit_logic(
        ...
        if market_structure["recent_highs"]:
            recent_high_targets = [
                high
                for high in market_structure["recent_highs"]
        ...
        if rsi_cols:
            current_rsi = (
                factors_df["RSI14"].iloc[-1] if "RSI14" in factors_df.columns else 50
            )
        ...
        # 4. 斐波那契扩展目标
        recent_swing_low = (
            min(market_structure["recent_lows"][:3])
            if market_structure["recent_lows"]
            else current_price * 0.9
        )
        recent_swing_high = (
            max(market_structure["recent_highs"][:3])
            if market_structure["recent_highs"]
            else current_price * 1.1
        )
        ...
        take_profit_explanation["reasoning"].append(
        ...
        take_profit_explanation["technical_factors"].append(
            "• 防止趋势反转带来的利润回吐"
        )
        take_profit_explanation["technical_factors"].append(
            "• 保留部分仓位，捕捉更大涨幅"
    ...
    def generate_deep_analysis_report(
        ...
        risk_scenarios = self.analyze_risk_reward_scenarios(
            df, factors_df, market_structure, volume_profile
        )
        stop_loss_explanation = self.explain_stop_loss_logic(
            df, factors_df, market_structure, volume_profile
        )
        take_profit_explanation = self.explain_take_profit_logic(
            df, factors_df, market_structure, volume_profile
        )
...
"""
        ...
        # 风险回报分析
        if (
            stop_loss_explanation["recommended_stop"]
            and take_profit_explanation["targets"]
        ):
            risk = abs(
                (stop_loss_explanation["recommended_stop"] / current_price - 1) * 100
            )
...
def main():
    ...
    try:
        ...
        risk_scenarios = analyzer.analyze_risk_reward_scenarios(
            df, factors_df, market_structure, volume_profile
        )
        stop_loss_logic = analyzer.explain_stop_loss_logic(
            df, factors_df, market_structure, volume_profile
        )
        take_profit_logic = analyzer.explain_take_profit_logic(
            df, factors_df, market_structure, volume_profile
        )
