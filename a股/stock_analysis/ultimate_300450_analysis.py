...
class Ultimate300450Analyzer:
    ...
    def calculate_factors_with_calculator(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if factors_df is not None:
                ...
                print(f"  指标数据范围: {factors_df.index.min().date()} 到 {factors_df.index.max().date()}")

                # 显示一些关键指标
                print("  关键指标示例:")
                key_indicators = {
                    'RSI': [col for col in factors_df.columns if 'RSI14' in col],
                    'MACD': [col for col in factors_df.columns if 'MACD' in col and 'Signal' not in col],
    ...
    def analyze_comprehensive_signals(self, df: pd.DataFrame, factors_df: pd.DataFrame) -> Dict:
        """综合分析65个技术指标信号"""
        ...
        # 1. 趋势分析 (权重: 30%)
        trend_score, trend_signals = self._analyze_trend_signals(df, factors_df, latest, prev_1, prev_5, prev_10)
        signals['buy_signals'].extend(trend_signals['buy'])
        signals['sell_signals'].extend(trend_signals['sell'])
        trend_weight = 0.3

        # 2. 动量分析 (权重: 25%)
        momentum_score, momentum_signals = self._analyze_momentum_signals(factors_df, latest, prev_1, prev_3, prev_5)
        signals['buy_signals'].extend(momentum_signals['buy'])
        signals['sell_signals'].extend(momentum_signals['sell'])
        momentum_weight = 0.25

        # 3. 超买超卖分析 (权重: 20%)
        oscillator_score, oscillator_signals = self._analyze_oscillator_signals(factors_df, latest, prev_1)
        ...
        # 4. 成交量分析 (权重: 15%)
        volume_score, volume_signals = self._analyze_volume_signals(df, latest, prev_1, prev_5)
        ...
        # 5. 波动性分析 (权重: 10%)
        volatility_score, volatility_signals = self._analyze_volatility_signals(factors_df, latest, prev_5)
        ...
        # 计算置信度
        signals['buy_confidence'] = min(len([s for s in signals['buy_signals'] if s['strength'] >= 7]) / 10, 1.0)
        signals['sell_confidence'] = min(len([s for s in signals['sell_signals'] if s['strength'] >= 7]) / 10, 1.0)
    ...
    def _analyze_trend_signals(self, df: pd.DataFrame, factors_df: pd.DataFrame, latest, prev_1, prev_5, prev_10) -> tuple:
        """分析趋势信号"""
        score = 0
        buy_signals = []
        sell_signals = []
        ...
        # 移动平均线分析
        ma_cols = [col for col in factors_df.columns if col.startswith('MA') and col[2:].isdigit()]
        ...
        if 'MA10' in factors_df.columns and 'MA20' in factors_df.columns:
            ...
            if not (pd.isna(ma10_latest) or pd.isna(ma20_latest) or pd.isna(ma10_prev) or pd.isna(ma20_prev)):
                # 金叉
        ...
        # EMA趋势分析
        ema_cols = [col for col in factors_df.columns if col.startswith('EMA') and col[3:].isdigit()]
        if ema_cols:
            ema_above = 0
        ...
        # MACD趋势
        macd_cols = [col for col in factors_df.columns if 'MACD' in col and 'Signal' not in col]
        if macd_cols:
    ...
    def _analyze_momentum_signals(self, factors_df: pd.DataFrame, latest, prev_1, prev_3, prev_5) -> tuple:
        """分析动量信号"""
        score = 0
        buy_signals = []
        sell_signals = []
    ...
    def _analyze_oscillator_signals(self, factors_df: pd.DataFrame, latest, prev_1) -> tuple:
        """分析超买超卖信号"""
        score = 0
        buy_signals = []
        sell_signals = []
        ...
        # 随机指标分析
        stoch_k_cols = [col for col in factors_df.columns if 'STOCH' in col and 'K' in col]
    ...
    def _analyze_volume_signals(self, df: pd.DataFrame, latest, prev_1, prev_5) -> tuple:
        """分析成交量信号"""
        score = 0
        buy_signals = []
        sell_signals = []
        ...
        if obv_cols:
            ...
            if not pd.isna(obv_value) and not pd.isna(obv_prev_5):
                obv_trend = (obv_value - obv_prev_5) / abs(obv_prev_5) if obv_prev_5 != 0 else 0
        ...
        return score / 15, {'buy': buy_signals, 'sell': sell_signals, 'neutral': neutral_signals}  # 归一化到[-1, 1]

    def _analyze_volatility_signals(self, factors_df: pd.DataFrame, latest, prev_5) -> tuple:
        """分析波动性信号"""
        score = 0
        neutral_signals = []
    ...
    def _calculate_risk_management(self, trading_points: dict, signals: dict) -> dict:
        ...
        risk_mgmt['time_stop'] = '20个交易日' if signals['signal_strength'] in ['weak', 'very_weak'] else '40个交易日'

        return risk_mgmt

    def generate_ultimate_report(self, df: pd.DataFrame, factors_df: pd.DataFrame, signals: dict, trading_points: dict) -> str:
        """生成300450终极分析报告"""
        ...
        max_drawdown = ((df['close'].expanding().max() - df['close']) / df['close'].expanding().max()).max() * 100
...
────────────────────────────────────────────────────────────────────────────"""
        ...
        # 显示关键均线
        ma20 = df[[col for col in df.columns if 'MA20' in col]].iloc[-1].iloc[0] if any('MA20' in col for col in df.columns) else None
        ma60 = df[[col for col in df.columns if 'MA60' in col]].iloc[-1].iloc[0] if any('MA60' in col for col in df.columns) else None
  ...
  📍 MA60均线: ¥{ma60:.2f} ({ma60_signal})"""
        ...
        # 布林带信息
        bb_upper = factors_df[[col for col in factors_df.columns if 'BB_Upper_20_2' in col]].iloc[-1].iloc[0] if any('BB_Upper_20_2' in col for col in factors_df.columns) else None
        bb_lower = factors_df[[col for col in factors_df.columns if 'BB_Lower_20_2' in col]].iloc[-1].iloc[0] if any('BB_Lower_20_2' in col for col in factors_df.columns) else None
  ...
  📍 当前位置: 布林带{bb_position:.1f}%位置"""
        ...
        # RSI值
        rsi14 = factors_df[[col for col in factors_df.columns if 'RSI14' in col]].iloc[-1].iloc[0] if any('RSI14' in col for col in factors_df.columns) else None
        if rsi14:
...
def main():
    ...
    try:
        ...
        if factors_df is not None:
            ...
            # 5. 生成终极报告
            report = analyzer.generate_ultimate_report(df, factors_df, signals, trading_points)
            ...
            combined_file = f"/Users/zhangshenshen/深度量化0927/a股/300450_factors_{timestamp}.csv"
