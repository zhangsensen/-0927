...
def main():
    ...
    # ETF代码列表
    etf_codes = [
        '510300.SH', '588000.SH', '512480.SH', '515790.SH', '515030.SH',
        '512010.SH', '515210.SH', '159998.SZ', '159915.SZ', '510500.SH',
        '512100.SH', '512660.SH', '512690.SH', '512880.SH', '518880.SH',
        '159992.SZ', '159819.SZ', '516160.SH', '159883.SZ'
    ]
    ...
    try:
        ...
        for i in range(0, len(date_list), batch_size):
            ...
            print(f"  下载第 {i//batch_size + 1} 批数据 ({len(batch_dates)} 个交易日)...")
        ...
        if market_data:
            ...
            # 保存全市场数据
            market_file = data_dir / "moneyflow_market" / f"market_moneyflow_{start_date_tushare}_{end_date_tushare}.parquet"
            ...
            # 分析数据
            print(f"\n  📊 全市场资金流向分析:")
            print(f"    日期范围: {market_df['trade_date'].min()} ~ {market_df['trade_date'].max()}")
            print(f"    总净流入: {market_df['net_mf_amount'].sum():,.0f} 万元")
            print(f"    日均净流入: {market_df['net_mf_amount'].mean():,.0f} 万元")
            print(f"    流入天数: {(market_df['net_mf_amount'] > 0).sum():,}/{len(market_df):,} ({(market_df['net_mf_amount'] > 0).sum()/len(market_df)*100:.1f}%)")

            # 按净流入排序
            top_inflows = market_df.nlargest(10, 'net_mf_amount')
            print(f"\n  📈 净流入排行榜 (前10):")
            for _, row in top_inflows.iterrows():
                print(f"    {row['ts_code']} ({row['trade_date'].strftime('%Y-%m-%d')}): {row['net_mf_amount']:,.0f} 万元")
    ...
    try:
        hsgt_df = pro.moneyflow_hsgt(start_date=start_date_tushare, end_date=end_date_tushare)
        ...
        if not hsgt_df.empty:
            ...
            # 保存数据
            hsgt_file = data_dir / "moneyflow_market" / f"hsgt_moneyflow_{start_date_tushare}_{end_date_tushare}.parquet"
            ...
            # 分析数据
            print(f"\n  📊 沪深港通资金流向分析:")
            print(f"    日期范围: {hsgt_df['trade_date'].min()} ~ {hsgt_df['trade_date'].max()}")
            print(f"    北向资金净流入均值: {hsgt_df['north_money'].mean():.2f} 百万元")
            print(f"    南向资金净流入均值: {hsgt_df['south_money'].mean():.2f} 百万元")
            print(f"    北向资金流入天数: {(hsgt_df['north_money'] > 0).sum()}/{len(hsgt_df)} ({(hsgt_df['north_money'] > 0).sum()/len(hsgt_df)*100:.1f}%)")
        ...
        for file_path in etf_files:
            try:
                ...
                if etf_code:
                    ...
                    # 资金流向估算 - 多维度方法
                    # 方法1: 基于成交量和价格变化
                    df['moneyflow_basic'] = df['amount'] * df['volume_ratio'] * np.sign(df['pct_chg'])

                    # 方法2: 基于成交额异常
                    amount_std = df['amount'].rolling(20).std()
                    df['amount_anomaly'] = (df['amount'] - df['amount_ma5']) / amount_std
                    df['moneyflow_anomaly'] = np.where(df['amount_anomaly'] > 2,
                                                   df['amount'] * np.sign(df['pct_chg']), 0)

                    # 方法3: 大单净流入估算
                    volume_std = df['vol'].rolling(20).std()
                    df['large_order_signal'] = df['vol'] > (df['volume_ma20'] + 1.5 * volume_std)
                    df['moneyflow_large'] = np.where(df['large_order_signal'],
                                                    df['amount'] * 0.6 * np.sign(df['pct_chg']), 0)

                    # 综合资金流向指标
                    df['estimated_moneyflow'] = (df['moneyflow_basic'] * 0.4 +
                                                df['moneyflow_anomaly'] * 0.3 +
                                                df['moneyflow_large'] * 0.3)

                    # 保存估算数据
                    estimate_df = df[['trade_date', 'close', 'vol', 'amount', 'pct_chg',
                                     'volume_ratio', 'amount_ratio', 'price_change_5d', 'price_change_20d',
                                     'moneyflow_basic', 'moneyflow_anomaly', 'large_order_signal',
                                     'moneyflow_large', 'estimated_moneyflow']].copy()

                    # 清理数据
                    estimate_df = estimate_df.dropna()

                    estimate_file = data_dir / "moneyflow" / f"{symbol}_moneyflow_estimated_{start_date_tushare}_{end_date_tushare}.parquet"
    ...
    print(f"df = pd.read_parquet('raw/ETF/moneyflow/510300_moneyflow_estimated_20241014_20251014.parquet')")
    print(f"")
    print(f"# 分析资金流向趋势")
    ...
    print(f"")
    print(f"# 识别资金流向信号")
    print(f"buy_signals = df[df['estimated_moneyflow'] > df['estimated_moneyflow'].quantile(0.8)]")
