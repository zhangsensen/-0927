...
def apply_filters(
    ...
    mask = (df['train_composite'] >= train_threshold) & (df['hold_composite'] >= hold_threshold)
    df_filtered = df[mask].copy()
    print(f"✓ 双重挡板 (训练>{train_pct:.0%} ∩ Holdout>{hold_pct:.0%}): {len(df_filtered)} 个")
    ...
    if whitelist:
        ...
        if require_all_whitelist:
            ...
            print(f"✓ 白名单过滤 (必含全部: {', '.join(whitelist)}): {len(df_filtered[mask])} 个")
        else:
            mask = df_filtered['combo'].apply(has_any_whitelisted)
            print(f"✓ 白名单过滤 (至少一个: {', '.join(whitelist)}): {len(df_filtered[mask])} 个")
...
def summarize_results(df):
    ...
    print(f"  训练期收益: 均值={df['vec_return'].mean():.2%}, 中位={df['vec_return'].median():.2%}")
    print(f"  Holdout收益: 均值={df['hold_return'].mean():.2%}, 中位={df['hold_return'].median():.2%}")
    print(f"  Holdout Sharpe: 均值={df['hold_sharpe'].mean():.4f}, 中位={df['hold_sharpe'].median():.4f}")
    print(f"  Holdout MaxDD: 均值={df['hold_max_dd'].mean():.2%}, 中位={df['hold_max_dd'].median():.2%}")
...
def display_top_n(df, n=20):
    ...
    for idx, row in df_sorted.head(n).iterrows():
        print(f"{idx+1:4d} {row['hold_return']:+11.2%} {row['hold_sharpe']:13.4f} {row['hold_max_dd']:13.2%} {int(row['size_train']):6d} {row['combo']}")


def main():
    parser = argparse.ArgumentParser(description='双重挡板+约束筛选')
    parser.add_argument('--train_pct', type=float, default=0.70, help='训练期分位数阈值')
    parser.add_argument('--hold_pct', type=float, default=0.80, help='Holdout期分位数阈值')
    ...
    if args.output:
        ...
        output_path = ROOT / f'results/vec_from_wfo_20251211_205649/filtered_top{args.top_n}_for_bt.csv'
    ...
    print(f"  2. 执行BT小规模审计: uv run python scripts/batch_bt_backtest.py --input {output_path}")
