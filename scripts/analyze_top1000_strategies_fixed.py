...
def load_and_combine_all_batches():
    ...
    if all_data:
        ...
        # 按夏普比率排序
        combined_df = combined_df.sort_values('sharpe', ascending=False).reset_index(drop=True)
        print(f"   夏普比率范围: {combined_df['sharpe'].min():.4f} - {combined_df['sharpe'].max():.4f}")
...
def parse_weights_data(df):
    ...
    valid_df = pd.concat([valid_df.reset_index(drop=True), weights_df.reset_index(drop=True)], axis=1)
...
def analyze_performance_overview(df):
    ...
    # 基本统计
    metrics = ['sharpe', 'annual_return', 'max_drawdown', 'calmar', 'win_rate', 'turnover']
    ...
    for metric in metrics:
        if metric in df.columns:
            ...
            if metric == 'annual_return' or metric == 'max_drawdown':
                print(f"  {metric:12s}: {mean_val:.4f} ({mean_val*100:.2f}%) ± {std_val:.4f}, 范围: [{min_val:.4f}, {max_val:.4f}]")
            elif metric == 'turnover':
                print(f"  {metric:12s}: {mean_val:.2f} ± {std_val:.2f}, 范围: [{min_val:.2f}, {max_val:.2f}]")
            else:
                print(f"  {metric:12s}: {mean_val:.4f} ± {std_val:.4f}, 范围: [{min_val:.4f}, {max_val:.4f}]")

    # Top-N分析
    if 'top_n' in df.columns:
        top_n_stats = df.groupby('top_n').agg({
            'sharpe': ['count', 'mean', 'std'],
            'annual_return': 'mean'
        }).round(4)
        ...
        for top_n in sorted(df['top_n'].unique()):
            ...
            print(f"  Top-{int(top_n)}: {len(subset)} 个策略, "
...
def analyze_factor_importance(df, factor_names):
    ...
    for i, factor_name in enumerate(factor_names):
        ...
        stats = {
            ...
            'avg_non_zero_weight': non_zero_weights.mean() if len(non_zero_weights) > 0 else 0,
            'usage_frequency': len(non_zero_weights) / len(weights) * 100  # 使用频率百分比
        }
    ...
    for category, factors in categories.items():
        if factors:
            ...
            if category_weights:
                ...
                non_zero_ratio = sum(1 for w in category_weights if w > 0) / len(category_weights)
                print(f"  {category:8s}: 平均权重 {avg_weight:.4f}, 使用率 {non_zero_ratio:.2%}")
...
def analyze_strategy_patterns(df, factor_names, weights_data):
    ...
    print(f"  中集中度(0.3≤HHI<0.5): {np.sum((concentration_scores >= 0.3) & (concentration_scores < 0.5))} 个策略")
    ...
    print(f"  使用10-20个因子: {np.sum((factor_counts >= 10) & (factor_counts < 20))} 个策略")
    ...
    complexity_performance = df.groupby('factor_count').agg({
...
def identify_top_performers(df, factor_names, n=100):
    ...
    print(f"Top {n} 策略性能:")
    print(f"  夏普比率: {top_strategies['sharpe'].mean():.4f} ± {top_strategies['sharpe'].std():.4f}")
    print(f"  年化收益: {top_strategies['annual_return'].mean():.4f} ± {top_strategies['annual_return'].std():.4f}")
    print(f"  最大回撤: {top_strategies['max_drawdown'].mean():.4f} ± {top_strategies['max_drawdown'].std():.4f}")
    print(f"  换手率: {top_strategies['turnover'].mean():.2f} ± {top_strategies['turnover'].std():.2f}")
    ...
    for i, (col, mean_weight) in enumerate(top_factor_means.items()):
        ...
        print(f"  {i+1:2d}. {factor_name:20s}: "
              f"平均权重 {mean_weight:.4f}, "
              f"使用率 {usage_rate:.1f}%")

    # 风险调整收益分析
    top_strategies['risk_adjusted_return'] = top_strategies['annual_return'] / top_strategies['max_drawdown']
    print(f"\n风险调整收益分析:")
...
def save_comprehensive_results(df, factor_names, factor_df):
    ...
    import json
    with open("strategies/results/top1000_strategy_insights.json", 'w', encoding='utf-8') as f:
...
def main():
    ...
    # 策略模式分析
    concentration_scores, factor_counts = analyze_strategy_patterns(df, factor_names, weights_data)
