...
def test_candlestick_patterns():
    ...
    for pattern_code, pattern_name in patterns_to_test:
        try:
            ...
            logger.info(f"  ✅ {pattern_name}: 信号数={total_signals}, 信号率={total_signals/len(test_data)*100:.1f}%")
...
def generate_test_report(results):
    ...
    logger.info(f"信号频率范围: {min(r['signal_rate'] for r in results.values()):.1f}% - {max(r['signal_rate'] for r in results.values()):.1f}%")
