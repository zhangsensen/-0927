...
class QuantQualityChecker:
    ...
    def _analyze_json_results(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "complexity" in data:
            ...
            results["summary"]["complexity"] = {
                ...
                "high_risk": len([f for f in complexity_data.get("functions", [])
                               if f.get("complexity", 0) > 10]),
                "medium_risk": len([f for f in complexity_data.get("functions", [])
                                 if 5 < f.get("complexity", 0) <= 10])
            }
            results["issues"]["complexity"] = [
                f for f in complexity_data.get("functions", [])
        ...
        if "clones" in data:
            ...
            results["summary"]["clones"] = {
                ...
                "high_similarity": len([c for c in clones_data.get("clones", [])
        ...
        if "dead_code" in data:
            ...
            results["summary"]["dead_code"] = {
                ...
                "critical": len([i for i in dead_code_data.get("issues", [])
    ...
    def generate_quality_report(self, results: Dict[str, Any], output_file: Optional[str] = None) -> str:
        """生成质量报告"""
        if not output_file:
    ...
    def _create_markdown_report(self, results: Dict[str, Any]) -> str:
        ...
        if issues.get("complexity"):
            ...
            for issue in issues["complexity"]:
                report.append(f"### {issue.get('name', 'Unknown')} (复杂度: {issue.get('complexity', 'N/A')})")
                report.append(f"- **位置**: {issue.get('file', 'N/A')}:{issue.get('line', 'N/A')}")
                report.append("")
        ...
        if issues.get("clones"):
            ...
            for i, clone in enumerate(issues["clones"][:10], 1):  # 只显示前10个
                ...
                if len(fragments) >= 2:
                    report.append(f"- **位置1**: {fragments[0].get('file', 'N/A')}:{fragments[0].get('start_line', 'N/A')}")
                    report.append(f"- **位置2**: {fragments[1].get('file', 'N/A')}:{fragments[1].get('start_line', 'N/A')}")
                report.append("")
