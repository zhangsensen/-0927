...
def load_all_etf_symbols():
    ...
    logger.info(
        f"ETF列表: {symbols[:10]}..." if len(symbols) > 10 else f"ETF列表: {symbols}"
    )
...
def validate_etf_data(symbols):
    ...
    for symbol in symbols:
        ...
        try:
            ...
            # 统一列名
            df = df.rename(
                columns={"trade_date": "date", "vol": "volume", "ts_code": "symbol"}
            )
...
def build_full_cross_section(manager, symbols, date):
    ...
    legacy_factors = [
        ...
        if not f.startswith("VBT_") and not f.startswith("TALIB_")
    ]
    dynamic_factors = [
        f for f in available_factors if f.startswith("VBT_") or f.startswith("TALIB_")
    ]
    ...
    for i in range(0, len(available_factors), batch_size):
        ...
        try:
            result = manager.build_cross_section(
                date=date, symbols=symbols, factor_ids=batch_factors
            )

            if (
                result.cross_section_df is not None
                and not result.cross_section_df.empty
            ):
                all_results.append(result.cross_section_df)
                logger.info(
                    f"✅ 批次 {batch_num} 完成: {result.cross_section_df.shape}"
                )
            else:
...
def analyze_factor_effectiveness(cross_section_df):
    ...
    partial_factors = stats_df[
        (stats_df["valid_rate"] > 0) & (stats_df["valid_rate"] < 50.0)
    ]
    invalid_factors = stats_df[stats_df["valid_rate"] == 0]

    logger.info(f"\n因子生效情况:")
    logger.info(
        f"  ✅ 完全生效 (≥50%): {len(effective_factors)} 个 ({len(effective_factors)/total_factors*100:.1f}%)"
    )
    logger.info(
        f"  ⚠️ 部分生效 (<50%): {len(partial_factors)} 个 ({len(partial_factors)/total_factors*100:.1f}%)"
    )
    logger.info(
        f"  ❌ 未生效 (0%):    {len(invalid_factors)} 个 ({len(invalid_factors)/total_factors*100:.1f}%)"
    )
    ...
    if len(effective_factors) > 0:
        ...
        for idx, row in effective_factors.head(20).iterrows():
            logger.info(
                f"  {row['factor_id']}: {row['valid_rate']:.1f}% 有效, "
...
def main():
    ...
    try:
        ...
        cross_section_df = build_full_cross_section(
            manager=manager, symbols=valid_symbols, date=cross_section_date
        )
