...
_PKG_ROOT = _HERE.parent  # etf_strategy
...
def _numba_warmup():
    ...
    try:
        ...
        from etf_strategy.core.ic_calculator_numba import compute_spearman_ic_numba as _csi
        sig = factors[:, :, 0]
        _ = _csi(sig, rets)
...
def _compute_or_load_daily_ic_memmap(factors_data_full: np.ndarray, returns: np.ndarray, stable_rank: bool) -> np.ndarray:
    ...
    环境变量:
        RB_DAILY_IC_MEMMAP_DIR: 目录（默认 etf_strategy/.cache）
    ...
    """
    memmap_dir = os.environ.get("RB_DAILY_IC_MEMMAP_DIR", "etf_strategy/.cache").strip()
