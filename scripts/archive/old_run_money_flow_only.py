...
def main():
    ...
    provider = MoneyFlowDataProvider(money_flow_dir=money_flow_dir, enforce_t_plus_1=True)

    money_flow_data = provider.load_money_flow_data(symbols, "1day", start_date, end_date)
            ...
            for symbol in symbols:
                try:
                    ...
                    if values is not None and isinstance(values, pd.Series):
                        ...
                        factor_df = pd.DataFrame({factor_name: values.values}, index=multi_idx)
