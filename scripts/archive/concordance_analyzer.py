...
Assumptions:
  ...
  - Whitelist file: etf_strategy/results/run_<wfo_ts>/whitelist_8d_wfo_qualified_<wfo_ts>.txt
"""
...
def load_whitelist_rank_map(wfo_ts: str) -> Dict[str, int]:
    ...
    candidates = [
        os.path.join("etf_strategy", "results", f"run_{wfo_ts}", f"whitelist_8d_wfo_qualified_{wfo_ts}.txt"),
        os.path.join("results", f"run_{wfo_ts}", f"whitelist_8d_wfo_qualified_{wfo_ts}.txt"),
        os.path.join("etf_strategy", "results", f"run_{wfo_ts}", f"whitelist_{wfo_ts}.txt"),
    ]
    wl_path = None
    for c in candidates:
        if os.path.exists(c):
