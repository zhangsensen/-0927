...
def compute_adx(high, low, close, window):
    ...
    plus_di = 100 * (plus_dm.ewm(span=window, adjust=False, min_periods=window).mean() / (atr + 1e-10))
    minus_di = 100 * (minus_dm.ewm(span=window, adjust=False, min_periods=window).mean() / (atr + 1e-10))
...
class Top1Validator:
    ...
    def run_vec_backtest(self, factors_3d, factor_indices, slippage_model=None):
        ...
        equity = validation_backtest_kernel(
            factors_3d, close, open_, high, low, self.timing_arr,
            factor_indices, self.rebalance_schedule, self.POS_SIZE, 1_000_000.0, 0.0002,
            0.0, 0, False, False, 0.0, np.zeros((self.T, self.N)), 0.0, False,
            np.ones((self.T, self.N), dtype=bool), False,
    ...
    def step1_bt_reproduction(self):
        ...
        target_factors = ['ADX_14D', 'MAX_DD_60D', 'PRICE_POSITION_120D', 'PV_CORR_20D', 'SHARPE_RATIO_20D']
    ...
    def step2_perturbation(self):
        ...
        for k, vals in variations.items():
            for v in vals:
                if k == 'ADX': cache[f"{k}_{v}"] = compute_adx(self.ohlcv['high'], self.ohlcv['low'], self.ohlcv['close'], v)
                elif k == 'MAX_DD': cache[f"{k}_{v}"] = -compute_max_dd(self.ohlcv['close'], v) # Invert here
                elif k == 'PRICE_POS': cache[f"{k}_{v}"] = compute_price_position(self.ohlcv['close'], self.ohlcv['high'], self.ohlcv['low'], v)
                elif k == 'PV_CORR': cache[f"{k}_{v}"] = compute_pv_corr(self.ohlcv['close'], self.ohlcv['volume'], v)
                elif k == 'SHARPE': cache[f"{k}_{v}"] = compute_sharpe(self.ohlcv['close'], v)
                
                # Z-Score immediately
                mean = cache[f"{k}_{v}"].mean(axis=1)
                std = cache[f"{k}_{v}"].std(axis=1)
                cache[f"{k}_{v}"] = cache[f"{k}_{v}"].sub(mean, axis=0).div(std + 1e-10, axis=0)
    ...
    def run(self):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(f"- **VEC Return**: {(vec_equity[-1]/1e6)**(252/len(vec_equity))-1:.2%}\n")
            ...
            f.write(perturb_df.sort_values('ann_return', ascending=False).head(5).to_markdown())
            f.write("\n\n")
            ...
            f.write("- **VaR 99%**: The 1st percentile of the simulated 1-year returns (i.e., 99% of the time, the annual return is better than this).\n")
            ...
            f.write("| :--- | :--- | :--- |\n")
            f.write(f"| 99% VaR (1Y) | {diag_res['iid_var_99']:.2%} | {diag_res['block_var_99']:.2%} |\n")
            f.write(f"| 95% VaR (1Y) | {diag_res['iid_var_95']:.2%} | {diag_res['block_var_95']:.2%} |\n")
            f.write(f"| Mean Simulated Return | {diag_res['iid_mean']:.2%} | {diag_res['block_mean']:.2%} |\n\n")
            
            f.write("### 5.4 Interpretation\n")
            f.write("- **i.i.d. Assumption**: The i.i.d. bootstrap destroys the serial correlation (trends) that the strategy relies on. By shuffling daily returns randomly, it simulates a 'random walk' market where trends do not persist, causing the strategy to underperform significantly.\n")
            f.write("- **Block Bootstrap**: By preserving 21-day (approx 1 month) blocks, we retain short-term trends. The Block VaR is expected to be closer to historical reality.\n")
            if diag_res['block_var_99'] > diag_res['iid_var_99']:
                f.write("- **Conclusion**: The -27% VaR was largely an artifact of the i.i.d. assumption breaking the trend structure. The Block VaR provides a more realistic (though still conservative) risk estimate.\n")
            else:
                f.write("- **Conclusion**: The risk remains high even with block bootstrapping, suggesting the strategy has genuine tail risk in certain market regimes.\n")
