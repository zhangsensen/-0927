...
all_factors_stack_full = np.stack([std_factors[f].values for f in factor_names_list], axis=-1)
...
for rb_date in rebalance_dates[:3]:
    ...
    if len(rb_idx) > 0:
        ...
        print(f"  {rb_date}: 非NaN数量={(~np.isnan(factor_vals)).sum()}/43, 均值={np.nanmean(factor_vals):.4f}")
