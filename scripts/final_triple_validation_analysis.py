...
def main():
    ...
    # 计算训练集期的对齐差异
    df_merged['train_return_diff'] = abs(df_merged['vec_return'] - df_merged['bt_return'])
    df_merged['train_mdd_diff'] = abs(df_merged['vec_max_drawdown'] - df_merged['bt_max_drawdown'])
    ...
    print(f"  <0.5% 差异: {(df_merged['train_return_diff'] < 0.005).sum()} / {len(df_merged)} ({(df_merged['train_return_diff'] < 0.005).mean()*100:.1f}%)")
    ...
    print(f"  <0.5% 差异: {(df_merged['train_mdd_diff'] < 0.005).sum()} / {len(df_merged)} ({(df_merged['train_mdd_diff'] < 0.005).mean()*100:.1f}%)")
    ...
    # 综合稳定性得分（三期最小 Calmar）
    df_merged['triple_stable_score'] = df_merged[['vec_calmar_ratio', 'holdout_calmar_ratio', 'bt_calmar_ratio']].min(axis=1)
    df_merged = df_merged.sort_values('triple_stable_score', ascending=False)
    
    print("\n🏆 三重验证 Top 20 (按最小 Calmar 排序):")
    print(f"{'排名':<4} | {'训练Calmar':<11} | {'Holdout':<11} | {'BT':<11} | {'最小值':<8} | {'组合'}")
    print('-' * 100)
    
    for i, (_, row) in enumerate(df_merged.head(20).iterrows(), 1):
        print(f"{i:<4} | {row['vec_calmar_ratio']:>10.3f} | {row['holdout_calmar_ratio']:>10.3f} | "
    ...
    # 训练集 vs Holdout
    train_holdout_ratio = df_merged['holdout_calmar_ratio'] / df_merged['vec_calmar_ratio']
    # 训练集 vs BT
    ...
    print(f"  0.8-1.2 (稳定): {((train_holdout_ratio >= 0.8) & (train_holdout_ratio <= 1.2)).sum()} / {len(df_merged)}")
    
    print("\n训练集(VEC) vs 训练集(BT):")
    print(f"  平均比值: {train_bt_ratio.mean():.3f}")
    print(f"  中位数比值: {train_bt_ratio.median():.3f}")
    print(f"  0.95-1.05 (高度对齐): {((train_bt_ratio >= 0.95) & (train_bt_ratio <= 1.05)).sum()} / {len(df_merged)}")
    ...
    print(f"\n三重验证表现:")
    print(f"  训练集(VEC):  Calmar={top1['vec_calmar_ratio']:.3f}, Return={top1['vec_return']*100:.2f}%, MDD={top1['vec_max_drawdown']*100:.2f}%")
    print(f"  Holdout:      Calmar={top1['holdout_calmar_ratio']:.3f}, Return={top1['holdout_return']*100:.2f}%, MDD={top1['holdout_max_drawdown']*100:.2f}%")
    print(f"  训练集(BT):   Calmar={top1['bt_calmar_ratio']:.3f}, Return={top1['bt_return']*100:.2f}%, MDD={top1['bt_max_drawdown']*100:.2f}%")
    print(f"\n稳定性评价:")
    print(f"  最小 Calmar: {top1['triple_stable_score']:.3f}")
    print(f"  VEC vs BT 收益差异: {abs(top1['vec_return'] - top1['bt_return'])*100:.3f}%")
    ...
    # 保存完整对比
    df_output = df_merged[['combo', 'combo_size', 
                           'vec_calmar_ratio', 'vec_return', 'vec_max_drawdown',
                           'holdout_calmar_ratio', 'holdout_return', 'holdout_max_drawdown',
                           'bt_calmar_ratio', 'bt_return', 'bt_max_drawdown',
                           'triple_stable_score',
                           'train_return_diff', 'train_mdd_diff']].copy()
    ...
    print('=' * 80)
    print(f"\n核心结论:")
    print(f"  1. VEC vs BT 平均对齐差异: {df_merged['train_return_diff'].mean()*100:.3f}% (收益率)")
