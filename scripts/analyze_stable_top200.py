...
def main():
    ...
    print(f"  >1.0 (Holdout更好): {(calmar_ratio_change > 1.0).sum()} / 200 ({(calmar_ratio_change > 1.0).mean()*100:.1f}%)")
    print(f"  0.8-1.2 (稳定): {((calmar_ratio_change >= 0.8) & (calmar_ratio_change <= 1.2)).sum()} / 200")
    ...
    for size in sorted(size_dist.index):
        ...
        avg_stable = top200[top200['combo_size'] == size]['calmar_ratio_stability'].mean()
    ...
    for factor in sorted(all_factor_names):
        ...
        if abs(diff) > 20:
            ...
            print(f"{marker} {factor:<38} | {top200_rate:>8.1f}% | {all_rate:>9.1f}% | {diff:>+9.1f}%")
    ...
    # 保存 Top 200 详情
    top200_output = top200[['combo', 'combo_size', 'vec_calmar_ratio', 'holdout_calmar_ratio', 
                             'calmar_ratio_stability', 'vec_return', 'holdout_return',
                             'vec_max_drawdown', 'holdout_max_drawdown']].copy()
    ...
    ax = axes[1, 0]
    ax.scatter(top200['vec_calmar_ratio'], top200['holdout_calmar_ratio'], 
               alpha=0.6, c=top200['combo_size'], cmap='viridis', s=50)
    ...
    # 8.4 稳定性得分分布
    ax = axes[1, 1]
    ax.hist(top200['calmar_ratio_stability'], bins=30, color='green', alpha=0.7, edgecolor='black')
    ax.axvline(top200['calmar_ratio_stability'].median(), color='red', 
               linestyle='--', linewidth=2, label=f'中位数: {top200["calmar_ratio_stability"].median():.3f}')
