...
def main():
    ...
    df = df.replace([np.inf, -np.inf], np.nan).dropna(subset=["bt_annual_return", "bt_sharpe_ratio", "bt_max_drawdown"])
    ...
    df["score_rank"] = 0.4 * df["rank_ret"] + 0.3 * df["rank_sharpe"] + 0.3 * df["rank_dd"]
    ...
    with open(summary_path, "w") as f:
        ...
        cols = ["combo", "grade", "bt_annual_return", "bt_max_drawdown", "bt_sharpe_ratio", "bt_calmar_ratio"]
