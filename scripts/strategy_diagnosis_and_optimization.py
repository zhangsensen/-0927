...
def enhanced_filtering(df: pd.DataFrame):
    ...
    # Remove Inf/NaN
    df_clean = df_clean.replace([np.inf, -np.inf], np.nan).dropna(subset=['ann_ret', 'max_dd', 'sharpe'])
    ...
    if 'num_trades' in df_clean.columns:
        ...
        filter_log.append(f"Removed {before - len(df_clean)} strategies with < 20 trades.")
    
    # 2.1 Existing Logic
    # Ann Ret > 12%
    before = len(df_clean)
    df_clean = df_clean[df_clean['ann_ret'] > 0.12]
    filter_log.append(f"Removed {before - len(df_clean)} strategies with Ann Ret <= 12%.")
    
    # Max DD > -30% (i.e., <= 30% loss)
    before = len(df_clean)
    df_clean = df_clean[df_clean['max_dd'] > -0.30]
    filter_log.append(f"Removed {before - len(df_clean)} strategies with Max DD <= -30%.")
    
    # Positive Years
    if 'ret_2023' in df_clean.columns:
        before = len(df_clean)
        df_clean = df_clean[df_clean['ret_2023'] > 0]
        filter_log.append(f"Removed {before - len(df_clean)} strategies with Negative 2023.")
        
    if 'ret_2024' in df_clean.columns:
        before = len(df_clean)
        df_clean = df_clean[df_clean['ret_2024'] > 0]
        filter_log.append(f"Removed {before - len(df_clean)} strategies with Negative 2024.")
    ...
    if 'win_rate' in df_clean.columns:
        ...
        filter_log.append(f"Removed {before - len(df_clean)} strategies with Win Rate <= 45%.")
    ...
    filter_log.append(f"Removed {before - len(df_clean)} strategies with Calmar <= 0.3.")
