...
def validation_backtest_kernel(
    ...
    holdings_history = np.zeros((T, N), dtype=np.float64) # ✅ NEW: Track holdings history
...
def main():
    ...
    for idx, row in tqdm(filtered.iterrows(), total=len(filtered)):
        ...
        # Run Backtest
        (equity_curve, holdings_hist, total_ret, num_trades, filled_val, max_dd, ann_ret, sharpe) = validation_backtest_kernel(
        ...
        # Avg Equity
        avg_equity = np.mean(valid_equity[start_idx:]) if len(valid_equity[start_idx:]) > 0 else 1000000.0
        trading_years = (T - start_idx) / 252.0
        turnover_ann = (filled_val / avg_equity) / trading_years if trading_years > 0 else 0.0
        ...
        res['cost_sensitivity'] = ret_impact / abs(ann_ret) if abs(ann_ret) > 0.01 else 1.0
    ...
    with open(report_path, "w") as f:
        ...
        if not pass_df.empty:
            ...
            cols = ['combo', 'ann_return', 'sharpe', 'max_dd', 'cost_sensitivity', 'avg_adv_weighted']
            # Format columns
            ...
            disp_df['cost_sensitivity'] = disp_df['cost_sensitivity'].apply(lambda x: f"{x:.2%}")
            disp_df['avg_adv_weighted'] = disp_df['avg_adv_weighted'].apply(lambda x: f"{x/1e6:.1f}M")
            ...
            for idx, row in top10.head(5).iterrows():
                f.write(f"### `{row['combo']}`\n")
                f.write(f"- **Ann Ret**: {row['ann_return']:.2%} | **Sharpe**: {row['sharpe']:.3f} | **MaxDD**: {row['max_dd']:.2%}\n")
                f.write(f"- **Liquidity**: {row['avg_adv_weighted']/1e6:.1f}M | **Concentration**: {row['top_symbol_weight']:.1%}\n")
