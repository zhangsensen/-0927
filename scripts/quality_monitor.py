...
class QualityMonitor:
    ...
    def _extract_metrics(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "complexity" in data:
            ...
            if functions:
                metrics["avg_complexity"] = sum(f.get("complexity", 0) for f in functions) / len(functions)
    ...
    def generate_trend_report(self, days: int = 30) -> str:
        ...
        if len(trends) >= 2:
            ...
            report.append("| 日期 | Commit | 质量分数 | 复杂度问题 | 克隆问题 | 死代码 |\n")
            report.append("|------|--------|----------|------------|----------|--------|\n")

            for trend in trends:
                date = datetime.fromisoformat(trend["timestamp"]).strftime("%m-%d %H:%M")
                report.append(f"| {date} | {trend['git_commit']} | {trend['quality_score']:.1f} | "
            ...
            score_trend = "📈 上升" if latest["quality_score"] > earliest["quality_score"] else "📉 下降"
            report.append(f"- **质量分数**: {score_trend} ({earliest['quality_score']:.1f} → {latest['quality_score']:.1f})")

            complexity_trend = "📈 减少" if latest["complexity_issues"] < earliest["complexity_issues"] else "📉 增加"
            report.append(f"- **复杂度问题**: {complexity_trend} ({earliest['complexity_issues']} → {latest['complexity_issues']})")

            clone_trend = "📈 减少" if latest["clone_issues"] < earliest["clone_issues"] else "📉 增加"
            report.append(f"- **代码克隆**: {clone_trend} ({earliest['clone_issues']} → {latest['clone_issues']})")

            # 建议
            report.append("\n## 💡 改进建议\n")
            if latest["quality_score"] < 80:
                report.append("- 当前质量分数偏低，建议重点解决高复杂度函数和代码克隆问题")
    ...
    def setup_monitoring(self, interval_hours: int = 24) -> None:
        ...
        print(f"   0 */{interval_hours} * * * cd {Path.cwd()} && python scripts/quality_monitor.py --capture")
...
def main():
    ...
    parser.add_argument(
        "--target",
        default="factor_system/",
        help="分析目标路径 (默认: factor_system/)"
    )
    ...
    if args.capture:
        ...
        if trends["trends"]:
            ...
            for trend in trends["trends"][-5:]:
                print(f"  {trend['timestamp'][:16]} | {trend['git_commit']} | "
                      f"质量分数: {trend['quality_score']:.1f}")
    elif args.report:
        report = monitor.generate_trend_report(args.days)
        report_file = Path("reports") / f"quality_trends_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
