...
def main() -> None:
    ...
    parser.add_argument("--date", required=True, help="Release date yyyymmdd, e.g. 20251214")
    parser.add_argument("--final-candidates", required=True, help="Path to final_candidates.parquet")
    parser.add_argument("--bt-results", required=True, help="Path to bt_results.parquet")
    parser.add_argument("--production-dir", required=True, help="Path to production pack directory")
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    _require_columns(cand, ["combo", "vec_return", "holdout_return", "composite_score"], "final_candidates")
    ...
    add("final_candidates", inputs.final_candidates, "artifacts/final_candidates.parquet")
    ...
    all_files = sorted(
        [p for p in sealed_dir.rglob("*") if p.is_file() and p.name != "CHECKSUMS.sha256"]
    )
    for fpath in all_files:
        checksum_lines.append(
            f"{_sha256_file(fpath)}  {fpath.relative_to(sealed_dir)}"
        )
