        ...
        try:
            ...
            if column_mapping:
                ...
                if execute:
                    ...
                    logger.info(
                        f"迁移完成: {file_path.name} - {len(column_mapping)} 列"
                    )
    ...
    def migrate_json_file(self, file_path: Path, execute: bool = False) -> Dict:
        ...
        try:
            ...
            def replace_factor_names(obj):
                ...
                if isinstance(obj, dict):
                    for key, value in obj.items():
                        if key in ["factor_id", "factor_name", "name"] and isinstance(
                            value, str
                        ):
                            new_name, params = self.extract_factor_name_and_params(
                                value
                            )
                            if new_name != value:
                                obj[key] = new_name
                                changed = True
                                result["migrated_references"].append(
                                    f"{value} -> {new_name}"
                                )
                        else:
            ...
            if changed and execute:
                ...
                logger.info(
                    f"迁移完成: {file_path.name} - {len(result['migrated_references'])} 引用"
                )
    ...
    def migrate_all(self, execute: bool = False) -> bool:
        ...
        for file_type in ["parquet", "csv", "json"]:
            for result in migration_results[file_type]:
                migration_results["total_migrated_columns"] += len(
                    result.get("migrated_columns", [])
                ) + len(result.get("migrated_references", []))
...
def main():
    ...
    parser.add_argument(
        "--base-dir", type=Path, default=Path("."), help="项目根目录路径"
    )
    parser.add_argument(
        "--backup-dir",
        type=Path,
