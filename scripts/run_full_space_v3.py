...
def main():
    ...
    combos_df = pd.read_csv(ROOT / 'results/ARCHIVE_unified_wfo_43etf_best/all_combos.csv')
    ...
    for idx, row in tqdm(combos_df.iterrows(), total=len(combos_df)):
        ...
        kernel_res = vec_backtest_kernel(
            ...
            INITIAL_CAPITAL,
            COMMISSION,
            0.20, 20, False, # Dynamic Leverage (Disabled)
            False, 0.0, atr_arr, 3.0, False, # Stop Loss (Disabled)
            individual_trend_arr, False, # Individual Trend (Disabled)
            profit_ladder_thresholds, profit_ladder_stops, profit_ladder_multipliers, # Profit Ladder (Disabled)
            0.0, 0.0, 5, # Circuit Breaker (Disabled)
            0, # Cooldown
            1.0 # Leverage Cap
        )
