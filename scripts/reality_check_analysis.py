...
def get_etf_name(ticker):
    ...
    names = {
        ...
        '513100': '纳指ETF', '513500': '标普500ETF', '512880': '证券ETF',
        '512000': '券商ETF', '512660': '军工ETF', '512010': '医药ETF',
        '512800': '银行ETF', '512690': '酒ETF', '512480': '半导体ETF',
        '512100': '1000ETF', '512070': '非银ETF', '515000': '科技ETF',
        '588000': '科创50ETF', '159915': '创业板ETF', '159949': '创业板50',
        '518880': '黄金ETF', '513050': '中概互联', '513330': '恒生科技',
        '515030': '新能源车ETF', '515790': '光伏ETF', '510330': '华夏300'
    }
...
class RealityCheck(Top1Validator):
    ...
    def prepare_data(self):
        print(f"DEBUG: self.T={self.T}, len(rebalance_schedule)={len(self.rebalance_schedule)}")
        ...
        # Market Context Data
        self.hs300 = self.ohlcv['close']['510300'] if '510300' in self.tickers else self.ohlcv['close'].iloc[:, 0]
        ...
        for i in range(self.T):
            ...
            if i > 0 and current_holdings:
                ...
                for ticker in current_holdings:
                    ret = self.ohlcv['close'][ticker].iloc[i] / self.ohlcv['close'][ticker].iloc[i-1] - 1
                    day_ret += ret
            ...
            if i in rebalance_indices_set:
                ...
                if not valid_scores.empty:
                    # Sort descending
                    top_picks = valid_scores.sort_values(ascending=False).head(self.POS_SIZE).index.tolist()
    ...
    def task1_random_replay(self):
        ...
        for idx in selected_indices:
            ...
            # 1. Market Context
            hs300_ret = self.hs300.iloc[idx] / self.hs300.iloc[idx-1] - 1 if idx > 0 else 0
            ...
            print(f"   策略Top5选股:")
            print(f"   {'代码':<8} {'名称':<10} {'总分':<8} {'ADX':<8} {'MaxDD':<8} {'Pos120':<8} {'PVCorr':<8} {'Sharpe':<8}")
            for ticker in top5:
                name = get_etf_name(ticker)
                score = scores[ticker]
                f_vals = [self.raw_factors_dict[f][ticker].iloc[idx] for f in ['ADX_14D', 'MAX_DD_60D', 'PRICE_POSITION_120D', 'PV_CORR_20D', 'SHARPE_RATIO_20D']]
                # Note: Raw factors might be NaN or different scales. Just showing them.
                print(f"   {ticker:<8} {name:<10} {score:>6.2f} {f_vals[0]:>8.2f} {f_vals[1]:>8.2f} {f_vals[2]:>8.2f} {f_vals[3]:>8.2f} {f_vals[4]:>8.2f}")
            ...
            for ticker in top5[:self.POS_SIZE]:
                ...
                issues = []
                if vol == 0: issues.append("停牌")
                if high == low and close > prev_close: issues.append("一字涨停")
                if high == low and close < prev_close: issues.append("一字跌停")
                amount = vol * close
                if amount < 50_000_000: issues.append(f"流动性低({amount/1e6:.1f}M)")
    ...
    def task3_failures(self):
        ...
        for date in worst_days:
            ...
            # Why loss?
            for t in tickers:
                r = self.ohlcv['close'][t].iloc[idx] / self.ohlcv['close'][t].iloc[idx-3] - 1
                print(f"   -> {t}: {r:.2%}")
    ...
    def task4_risks(self):
        ...
        print(f"流动性不足 (<5000万) 次数: {low_liq_count} ({low_liq_count/total_trades:.1%})")
        print(f"无法执行 (涨跌停/停牌) 次数: {limit_count} ({limit_count/total_trades:.1%})")
