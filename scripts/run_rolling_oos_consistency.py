...
def _window_metrics(equity_curve: np.ndarray, start_idx: int, end_idx: int) -> dict[str, float]:
...
def _build_segments(
    ...
    while start < dates.size - 1:
        ...
        if end - start > 1:
            ...
            segments.append(Segment(label=label, start_idx=start, end_idx=end, is_holdout=is_holdout))
        start = end
...
def process_combo(
    ...
    try:
        ...
        summary.update({f"all_{k}": v for k, v in _summarize_segments(seg_metrics_all).items()})
        summary.update({f"holdout_{k}": v for k, v in _summarize_segments(seg_metrics_holdout).items()})
...
def _write_report(out_dir: Path, summary_df: pd.DataFrame, top: int = 30) -> None:
    ...
    if "all_segment_worst_calmar" in summary_df.columns:
        table(summary_df.sort_values("all_segment_worst_calmar", ascending=False), "Top by worst-segment Calmar (all segments)")
    ...
    if "all_segment_positive_rate" in summary_df.columns:
        table(summary_df.sort_values(["all_segment_positive_rate", "all_segment_worst_calmar"], ascending=False), "Top by segment positive rate")

    (out_dir / "rolling_oos_report.md").write_text("\n".join(lines), encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Rolling OOS consistency validation (Parquet-first)")
    parser.add_argument("--input", required=True, help="training results table (.parquet/.csv) containing at least 'combo'")
    parser.add_argument(
        "--top-n",
        type=int,
        default=None,
    ...
    if args.top_n is not None:
        ...
        df_in = df_in.sort_values("vec_calmar_ratio", ascending=False).head(int(args.top_n)).copy()
    ...
    # Timing signal
    timing_module = LightTimingModule(extreme_threshold=EXTREME_THRESHOLD, extreme_position=EXTREME_POSITION)
    timing_series = timing_module.compute_position_ratios(ohlcv["close"])
    timing_arr_raw = timing_series.reindex(all_dates).fillna(1.0).values.astype(np.float64)
    timing_arr = shift_timing_signal(timing_arr_raw)

    effective_start_idx = int(backtest_config["lookback"])
    segments = _build_segments(all_dates, effective_start_idx, args.segment, training_end)
    print(f"Segmentation: {args.segment}, segments={len(segments)} (effective_start_idx={effective_start_idx})")
