...
def main():
    ...
    for g in ['A', 'B', 'C', 'D']:
        if len(df[df['grade'] == g]) > 0:
            s = pd.Series(factors_by_grade[g]).value_counts() / len(df[df['grade'] == g])
            factor_freq[g] = s
    ...
    top_candidates = df[df['grade'].isin(['A', 'B'])].sort_values(
        ['grade', 'bt_b_sharpe'], ascending=[True, False]
    ).head(20)
    
    print("\n🏆 Top Robust Candidates (Sorted by Grade then BT_B Sharpe):")
    cols = ['combo', 'vec_ann_ret', 'bt_a_ann_ret', 'bt_b_ann_ret', 
            'bt_b_max_dd', 'bt_b_sharpe', 'exec_return_ratio', 'grade']
