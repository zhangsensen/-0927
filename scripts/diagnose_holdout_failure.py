...
print(f"  满仓天数: {(train_timing_shifted >= 0.95).sum()} / {len(train_timing_shifted)} ({(train_timing_shifted >= 0.95).mean():.1%})")
print(f"  低仓位天数 (<50%): {(train_timing_shifted < 0.5).sum()} / {len(train_timing_shifted)} ({(train_timing_shifted < 0.5).mean():.1%})")
print(f"  极端低仓 (<15%): {(train_timing_shifted <= 0.15).sum()} / {len(train_timing_shifted)} ({(train_timing_shifted <= 0.15).mean():.1%})")

print(f"\nHoldout择时信号统计 ({config['data']['training_end_date']} 至 {config['data']['end_date']}):")
print(f"  平均仓位: {holdout_timing_shifted.mean():.2%}")
print(f"  满仓天数: {(holdout_timing_shifted >= 0.95).sum()} / {len(holdout_timing_shifted)} ({(holdout_timing_shifted >= 0.95).mean():.1%})")
print(f"  低仓位天数 (<50%): {(holdout_timing_shifted < 0.5).sum()} / {len(holdout_timing_shifted)} ({(holdout_timing_shifted < 0.5).mean():.1%})")
print(f"  极端低仓 (<15%): {(holdout_timing_shifted <= 0.15).sum()} / {len(holdout_timing_shifted)} ({(holdout_timing_shifted <= 0.15).mean():.1%})")
...
print(f"  训练集正收益ETF: {(train_etf_rets > 0).sum()} / {len(train_etf_rets)} ({(train_etf_rets > 0).mean():.1%})")
print(f"  Holdout正收益ETF: {(holdout_etf_rets > 0).sum()} / {len(holdout_etf_rets)} ({(holdout_etf_rets > 0).mean():.1%})")
