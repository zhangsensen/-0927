...
def main():
    ...
    # Subset Data
    ohlcv_liquid = {
        k: v[available_liquid] for k, v in ohlcv.items()
    }
    ...
    for combo_str in tqdm(combos):
        ...
        # Get indices
        try:
            f_indices = np.array([factor_name_to_idx[f] for f in factors], dtype=np.int64)
        except KeyError:
            continue
        ...
        equity_curve, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = vec_backtest_kernel(
            ...
            1_000_000.0, # Capital
            0.0002, # Commission
            0.0, 0, False, # Vol target
            False, 0.0, atr_arr, 0.0, False, # Stop loss
            individual_trend_arr, False, # Trend filter
            profit_ladder_thresholds, profit_ladder_stops, profit_ladder_multipliers,
            0.0, 0.0, 0, # Circuit breaker
            0, # Cooldown
            1.0 # Leverage cap
        )
        ...
        if len(valid_equity) > 0:
            ...
            recent_mask = (dates >= pd.Timestamp('2025-01-01')) & (dates <= pd.Timestamp('2025-05-31'))
    ...
    out_dir = ROOT / "results" / f"vec_liquid_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    filtered = res_df[
        (res_df['Recent_Ret_5M'] > 0) & 
        (res_df['Recent_MDD_5M'] < 0.08)
    ].copy()
    ...
    if len(filtered) > 0:
        ...
        print(top5[['combo', 'vec_return', 'vec_sharpe_ratio', 'Recent_Ret_5M']].to_markdown(index=False, floatfmt=".2%"))
