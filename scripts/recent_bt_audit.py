...
def run_bt_with_daily_equity(
    ...
    if not daily_ret.empty:
        ...
        equity = pd.Series([start_val, end_val], index=[pd.Timestamp("1970-01-01"), pd.Timestamp("1970-01-02")])
...
def main():
    parser = argparse.ArgumentParser(description="近期窗口 BT 审计：验证最近市场阶段表现是否稳健")
    parser.add_argument("--topn", type=int, default=5, help="从最终排名中选取前 N 个组合进行审计")
    parser.add_argument(
        "--windows",
        type=int,
        nargs="+",
    ...
    # 1. 读取最终排名结果
    ranking_path = ROOT / "results" / "stable_top200_analysis" / "final_ranking_top200.csv"
    ...
    print(f"✅ 加载最终排名: {len(ranking_df)} 条，选取 Top-{args.topn} 进行近期窗口审计")
    ...
    summary_path = output_dir / f"recent_bt_summary_top{args.topn}_windows_{windows_tag}d.csv"
