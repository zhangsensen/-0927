...
def build_cross_section(combined_df, target_date):
    ...
    if isinstance(combined_df.index, pd.MultiIndex):
        try:
            ...
            closest_date = min(
                dates, key=lambda d: abs((d - target_date).total_seconds())
            )
...
def analyze_factors(cross_section):
    ...
    for factor_id in cross_section.columns:
        ...
        # 🔥 修复：检测全零列（失败因子）
        is_all_zero = (values == 0).all() or values.isna().all()
        non_zero_count = ((values != 0) & values.notna()).sum()
        ...
        factor_stats.append(
            {
                ...
                "non_zero_count": non_zero_count,  # 非零数据点
                "is_all_zero": is_all_zero,  # 是否全零（失败）
    ...
    # 🔥 修复：使用non_zero_count重新分类
    truly_effective = stats_df[stats_df["non_zero_count"] >= total_etfs * 0.5]
    partial = stats_df[(stats_df["non_zero_count"] > 0) & (stats_df["non_zero_count"] < total_etfs * 0.5)]
    failed = stats_df[stats_df["is_all_zero"]]

    logger.info("\n🔍 因子真实生效情况:")
    logger.info(
        f"  ✅ 真正有效 (≥50%非零): {len(truly_effective)} 个 ({len(truly_effective)/total_factors*100:.1f}%)"
    )
    logger.info(
        f"  ⚠️  部分有效 (<50%非零): {len(partial)} 个 ({len(partial)/total_factors*100:.1f}%)"
    )
    logger.info(
        f"  ❌ 计算失败 (全零/全空): {len(failed)} 个 ({len(failed)/total_factors*100:.1f}%)"
    )

    # 显示生效因子
    if len(truly_effective) > 0:
        logger.info("\n✅ 真正有效的因子 (前30个):")
        for _, row in truly_effective.head(30).iterrows():
            logger.info(
                f"  {row['factor_id']}: {row['non_zero_count']}/{total_etfs} 非零"
            )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"   动态因子: {len(manager.factor_registry.list_factors(is_dynamic=True))} 个"
        )
        logger.info(
            f"   传统因子: {len(available_factors) - len(manager.factor_registry.list_factors(is_dynamic=True))} 个"
        )
        ...
        output_file = (
            output_dir / f"cross_section_{target_date.strftime('%Y%m%d')}.parquet"
        )
