...
def analyze_period_stats(etf_returns, start_date, end_date, period_name):
    ...
    for idx, row in top10.iterrows():
        print(f"  {row['code']:12} {row['total_return']:+7.2%} | Sharpe={row['sharpe']:6.2f} | MaxDD={row['max_dd']:7.2%}")
...
def compare_periods(train_stats, hold_stats):
    ...
    print(f"\n【平均指标变化】")
    print(f"  收益率: 训练={merged['total_return_train'].mean():.2%} → Holdout={merged['total_return_hold'].mean():.2%}")
    print(f"  波动率: 训练={merged['volatility_train'].mean():.2%} → Holdout={merged['volatility_hold'].mean():.2%}")
    print(f"  Sharpe: 训练={merged['sharpe_train'].mean():.4f} → Holdout={merged['sharpe_hold'].mean():.4f}")
    print(f"  最大回撤: 训练={merged['max_dd_train'].mean():.2%} → Holdout={merged['max_dd_hold'].mean():.2%}")
    ...
    print(f"  训练Top20在Holdout平均收益: {train_top20['total_return_hold'].mean():.2%}")
    print(f"  训练Top20在Holdout正收益占比: {(train_top20['total_return_hold'] > 0).mean():.2%}")
    ...
    for idx, row in train_top20_sorted.iterrows():
        ...
        print(f"{row['code']:12} {int(row['rank_train']):10d} {int(row['rank_hold']):10d} {rank_change:+10d} {row['total_return_hold']:+9.2%}")
