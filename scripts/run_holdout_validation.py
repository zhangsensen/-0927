...
def _window_metrics(equity_curve: np.ndarray, start_idx: int, end_idx: int | None = None) -> dict:
...
def process_combo(
    ...
    try:
        equity_curve, total_return, win_rate, profit_factor, num_trades, rounding_diag, risk_metrics = run_vec_backtest(
            current_factors,
            close_prices,
            open_prices,
            high_prices,
...
def main():
    ...
    parser.add_argument("--training-results", type=str, required=True, 
    ...
    if holdout_start_candidates.size == 0:
        raise ValueError(f"Holdout 起始点不存在：training_end_date={training_end} 已到数据末尾")
    ...
    print(f"有效交易起始索引: {effective_start_idx} (lookback={backtest_config['lookback']})")
    
    if holdout_start_idx < effective_start_idx:
        print(f"⚠️  警告: Holdout 起始点 ({holdout_start_idx}) 早于 lookback 窗口 ({effective_start_idx})")
    ...
    if args.top_n is not None:
        if "vec_calmar_ratio" not in training_df.columns:
            raise ValueError("training-results 缺少 vec_calmar_ratio 列，无法按 Top-N 过滤")
        training_df = training_df.sort_values("vec_calmar_ratio", ascending=False).head(int(args.top_n)).copy()
    print(f"\n✅ 加载训练集结果: {len(training_df)} 个组合")

    trailing_windows = [int(x) for x in str(args.trailing_windows).split(",") if str(x).strip()]
    ...
    # Merge on combo
    merged_df = training_df.merge(holdout_df, on='combo', how='inner', suffixes=('_train', '_holdout'))
    
    # 9. Calculate Stability Metrics
    # 双稳定性定义: 训练集和Holdout都表现良好
    merged_df['calmar_ratio_avg'] = (merged_df['vec_calmar_ratio'] + merged_df['holdout_calmar_ratio']) / 2
    merged_df['calmar_ratio_stability'] = merged_df[['vec_calmar_ratio', 'holdout_calmar_ratio']].min(axis=1)
    merged_df['return_avg'] = (merged_df['vec_return'] + merged_df['holdout_return']) / 2
    
    # 计算训练集和Holdout的排名
    merged_df['train_calmar_rank'] = merged_df['vec_calmar_ratio'].rank(ascending=False)
    merged_df['holdout_calmar_rank'] = merged_df['holdout_calmar_ratio'].rank(ascending=False)
    merged_df['rank_stability'] = (merged_df['train_calmar_rank'] - merged_df['holdout_calmar_rank']).abs()
    ...
    print('=' * 80)
    print(f"{'Rank':<4} | {'Size':<4} | {'Train_Calmar':<12} | {'Hold_Calmar':<12} | {'Stable':<8} | {'Combo'}")
    print('-' * 80)
    
    for i, (_, row) in enumerate(df_stable.head(20).iterrows()):
        combo_size = len(row['combo'].split(' + '))
        print(f"{i+1:<4} | {combo_size:<4} | {row['vec_calmar_ratio']:>11.3f} | "
    ...
    overfit_ratio = (merged_df['vec_calmar_ratio'] / merged_df['holdout_calmar_ratio']).median()
