...
def run_probe():
    ...
    for w_long in long_windows:
        for w_short in short_windows:
            ...
            # Backtest
            equity, _ = simple_vec_backtest(combined, close_np, rebalance_schedule, POS_SIZE, INITIAL_CAPITAL, COMMISSION_RATE)
    ...
    base_equity, base_holdings = simple_vec_backtest(base_combined, close_np, rebalance_schedule, POS_SIZE, INITIAL_CAPITAL, COMMISSION_RATE)
    ...
    print(sens_df.pivot(index='w_long', columns='w_short', values='sharpe').to_markdown())
    print("\n(Values are Sharpe Ratio)")
    
    # Check Plateau
    base_sharpe = sens_df[(sens_df['w_long']==120) & (sens_df['w_short']==20)]['sharpe'].values[0]
    ...
    if is_robust:
        print("**STATUS: PASS**")
        print("Strategy shows robust characteristics across parameters and historical crises.")
    else:
