...
# ---------- 配置 ----------
COMBO_STR = "CORRELATION_TO_MARKET_20D + MAX_DD_60D + PRICE_POSITION_120D + PRICE_POSITION_20D"
FREQ = 8
POS_SIZE = 2
LOOKBACK = 252
...
# ---------- VEC 纯 Python 模拟器 ----------
def run_vec_diag(factors_3d, prices, timing_arr, vol_regime_arr, factor_indices, dates, etf_codes):
    ...
    rebalance = generate_rebalance_schedule(total_periods=T, lookback_window=LOOKBACK, freq=FREQ)
    for t in rebalance:
        ...
        print(f"[VEC] t={t}, date={dates[t].date()}, cash_before={cash_before:.2f}, total_before={total_before:.2f}, timing={timing:.4f}, vol_regime={vol_r:.4f}")
        ...
        print(f"[VEC] selected_idx={selected_idx}, selected_codes={selected_codes}, selected_scores={selected_scores}")
        ...
        buy_list = []
        print(f"[VEC] kept_value={kept_value:.2f}, target_exposure={target_exposure:.2f}, available={available:.2f}")
        if available > 0:
            ...
            for n in selected_idx:
                ...
                if cash >= cost - 1e-6:
                    cash -= cost
                    holdings[n] += shares
                    buy_list.append((etf_codes[n], target_pos_value, price, shares, cost))
                    print(f"[VEC] buy {etf_codes[n]} price={price:.4f} shares={shares:.4f} cost={cost:.2f}")
        ...
        log_rows.append({
            ...
            "sell_list": ";".join(f"{c}:{sh:.4f}:{p:.4f}:{pro:.2f}" for c, sh, p, pro in sell_list),
            "buy_list": ";".join(f"{c}:{tv:.2f}:{p:.4f}:{sh:.4f}:{co:.2f}" for c, tv, p, sh, co in buy_list),
...
class DiagnosticStrategy(bt.Strategy):
    ...
    def rebalance(self, bar_index: int):
        ...
        holdings_value_before = sum(holdings.get(d._name, 0.0) * d.close[-1] for d in self.datas)
        ...
        selected_scores = [scores[i] for i in selected_idx]
        print(f"[BT] t={bar_index}, date={dt}, cash_before={cash_before:.2f}, total_before={total_before:.2f}, timing={timing:.4f}, vol_regime={vol_r:.4f}")
        print(f"[BT] selected_idx={selected_idx}, selected_codes={selected_codes}, selected_scores={selected_scores}")
        ...
        kept_value = sum(self.getposition(self.etf_map[c]).size * self.etf_map[c].close[0] for c in selected_codes if c in holdings)
        target_exposure = (cash + kept_value) * timing_ratio
        available = max(0.0, target_exposure - kept_value)

        buy_list = []
        print(f"[BT] kept_value={kept_value:.2f}, target_exposure={target_exposure:.2f}, available={available:.2f}")
        if available > 0:
            target_pos_value = available / self.params.pos_size / (1 + self.params.commission)
            for code in selected_codes:
                ...
                if cash >= cost - 1e-6:
                    ...
                    holdings_after_log[code] = holdings_after_log.get(code, 0.0) + shares
                    self.shadow_holdings[code] = self.shadow_holdings.get(code, 0.0) + shares
                    print(f"[BT] buy {code} price={price:.4f} shares={shares:.4f} cost={cost:.2f}")

        # 日志中的持仓市值（使用本bar收盘价，包含计划买入的shares；已卖出的设为0）
        holdings_value_after_log = sum((holdings_after_log.get(d._name, 0.0)) * d.close[0] for d in self.datas)
    ...
    def stop(self):
        pd.DataFrame(self.daily_log).to_csv(OUTPUT_DIR / "bt_daily_log.csv", index=False)


def main():
    # 1) 数据与因子
    config = yaml.safe_load(Path("configs/combo_wfo_config.yaml").read_text())
    loader = DataLoader(data_dir=config["data"]["data_dir"], cache_dir=config["data"]["cache_dir"])
    ...
    vec_log = run_vec_diag(
        factors_3d, prices, timing_arr, vol_regime_arr, combo_indices, dates, cols.tolist()
    )
    ...
    rebalance_schedule = generate_rebalance_schedule(total_periods=len(dates), lookback_window=LOOKBACK, freq=FREQ)
    ...
    if len(first_total) > 0:
        plt.axvline(first_total.iloc[0]["date"], color="r", linestyle="--", label="first total divergence")
    ...
    report.append("")
    report.append("## 结果摘要")
    report.append(f"- 现金首次差异 > $1: {first_cash.iloc[0]['date'] if len(first_cash)>0 else '无'}")
    report.append(f"- 总资产首次差异 > $1: {first_total.iloc[0]['date'] if len(first_total)>0 else '无'}")
    report.append("")
    report.append("## 差异示例（前1条）")
    if len(first_cash) > 0:
        row = first_cash.iloc[0]
        report.append(f"- 日期: {row['date']}, total_diff={row.get('total_diff', 0):.2f}, cash_diff={row.get('cash_diff', 0):.2f}, selection_diff={row.get('selection_diff', False)}")
    else:
