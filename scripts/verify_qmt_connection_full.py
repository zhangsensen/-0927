...
async def main():
    ...
    try:
        ...
        print_result("get_positions", success, data=f"持仓数量: {len(res) if res else 0}", error=None if success else "返回类型错误")
        if success and res:
            print(f"   首个持仓: {res[0]}")
        results.append(success)
    except Exception as e:
    ...
    try:
        ...
        print_result("get_orders", success, data=f"委托结构: {type(res).__name__}, 数量: {count}")
        results.append(success)
    except Exception as e:
    ...
    try:
        ...
        print_result("get_trades", success, data=f"成交结构: {type(res).__name__}, 数量: {count}")
        results.append(success)
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if success: results.append(True)
        except:
            results.append(False)
    ...
    try:
        ...
        # 参数名修正: market
        res = await client.get_trading_calendar(market="SH", start_time=today, end_time=today)
        success = res is not None
