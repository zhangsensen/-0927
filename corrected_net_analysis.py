...
def main():
    ...
    for cost_rate in cost_scenarios:
        ...
        net_return = best_strategy['annual_return'] - annual_cost
        net_sharpe = best_strategy['sharpe'] - (annual_cost / (best_strategy['annual_return']/best_strategy['sharpe']))
        
        print(f"  成本率{cost_rate*100:.1f}%: 成本{annual_cost:.3f}, 净收益{net_return:.3f}, 净夏普{net_sharpe:.4f}")
    ...
    df['net_sharpe_approx'] = df['sharpe'] * (df['net_annual_return'] / df['annual_return'])
    
    print(f"平均年化成本: {df['annual_turnover_cost'].mean():.3f} ({df['annual_turnover_cost'].mean()*100:.2f}%)")
    print(f"成本中位数: {df['annual_turnover_cost'].median():.3f} ({df['annual_turnover_cost'].median()*100:.2f}%)")
    ...
    # 保存结果
    output_cols = ['combo_idx', 'annual_return', 'sharpe', 'turnover', 'annual_turnover_cost', 
                   'net_annual_return', 'net_sharpe_approx', 'gross_rank', 'net_rank', 'rank_change']
    
    df[output_cols].to_csv('/Users/zhangshenshen/深度量化0927/strategies/results/corrected_net_analysis.csv', index=False)
    
    # 显示净收益最佳策略
    best_net = df.loc[df['net_sharpe_approx'].idxmax()]
    print(f"\n净收益最佳策略: {best_net['combo_idx']}")
    print(f"  毛夏普: {best_net['sharpe']:.4f} -> 净夏普: {best_net['net_sharpe_approx']:.4f}")
    print(f"  换手: {best_net['turnover']:.1f}%, 成本: {best_net['annual_turnover_cost']:.3f}")
